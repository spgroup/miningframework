...
public class ClientUtils {
    ...
    public static List<BakedQuad> convertConnectionFromBlockstate(BlockPos here, Set<Connection.RenderData> data, TextureAtlasSprite t) {
        ...
        for (Connection.RenderData connData : data) {
            ...
            for (int segmentEndId = 1; segmentEndId <= connData.pointsToRenderSolid; segmentEndId++) {
                ...
                Vector3d[] vertices = { segmentEnd.add(cross), segmentEnd.subtract(cross), segmentStart.subtract(cross), segmentStart.add(cross) };
                ...
                vertices = new Vector3d[] { segmentEnd.add(cross), segmentEnd.subtract(cross), segmentStart.subtract(cross), segmentStart.add(cross) };
    ...
    public static Set<BakedQuad> createBakedBox(Vector3d from, Vector3d to, Matrix4 matrixIn, Direction facing, Function<Vector3d[], Vector3d[]> vertexTransformer, Function<Direction, TextureAtlasSprite> textureGetter, float[] colour) {
        ...
        Vector3d[] vertices = { new Vector3d(from.x, from.y, from.z), new Vector3d(from.x, from.y, to.z), new Vector3d(to.x, from.y, to.z), new Vector3d(to.x, from.y, from.z) };
        ...
        vertices = new Vector3d[] { new Vector3d(to.x, to.y, from.z), new Vector3d(to.x, from.y, from.z), new Vector3d(from.x, from.y, from.z), new Vector3d(from.x, to.y, from.z) };
        ...
        vertices = new Vector3d[] { new Vector3d(from.x, to.y, to.z), new Vector3d(from.x, from.y, to.z), new Vector3d(from.x, from.y, from.z), new Vector3d(from.x, to.y, from.z) };
    ...
    public static void renderModelTESRFancy(List<BakedQuad> quads, IVertexBuilder renderer, World world, BlockPos pos, boolean useCached, int color, int light) {
        if (IEClientConfig.disableFancyTESR.get())
            ...
            for (BakedQuad quad : quads) {
                ...
                Vector3d side1 = new Vector3d(quadCoords[1][0] - quadCoords[3][0], quadCoords[1][1] - quadCoords[3][1], quadCoords[1][2] - quadCoords[3][2]);
                Vector3d side2 = new Vector3d(quadCoords[2][0] - quadCoords[0][0], quadCoords[2][1] - quadCoords[0][1], quadCoords[2][2] - quadCoords[0][2]);
