...
public class RealmSourceCodeGenerator {
    ...
    public void generate() throws IOException, UnsupportedOperationException {
        ...
        writer.emitImports("io.realm.internal.ColumnType", "io.realm.internal.Table", "io.realm.internal.ImplicitTransaction", "io.realm.internal.Row", "io.realm.internal.LinkView", "io.realm.RealmList", "io.realm.RealmObject", "java.util.Date", "java.util.Arrays", packageName + ".*").emitEmptyLine();
        ...
        for (VariableElement field : fields) {
            ...
            if (JAVA_TO_REALM_TYPES.containsKey(fieldTypeCanonicalName)) {
                ...
                writer.endControlFlow();
                writer.emitStatement("table.addColumnLink(ColumnType.LINK, \"%s\", transaction.getTable(\"%s%s\"))", fieldName.toLowerCase(Locale.getDefault()), tablePrefix, fieldTypeName);
            } else if (typeUtils.isAssignable(field.asType(), realmList)) {
                String genericCanonicalType = ((DeclaredType) field.asType()).getTypeArguments().get(0).toString();
                String genericType;
                if (genericCanonicalType.contains(".")) {
                ...
                writer.endControlFlow();
                writer.emitStatement("table.addColumnLink(ColumnType.LINK_LIST, \"%s\", transaction.getTable(\"%s%s\"))", fieldName.toLowerCase(Locale.getDefault()), tablePrefix, genericType);
            }
        }
            ...
            if (HOW_TO_EQUAL.containsKey(fieldTypeCanonicalName)) {
                switch(HOW_TO_EQUAL.get(fieldTypeCanonicalName)) {
                    ...
                    case EQUALS_NULL:
                        ...
                        break;
                    case EQUALS_ARRAY:
                        writer.emitStatement("if (!Array.equals(get%s(), a%s.get%s()) return false", capFieldName, className, capFieldName);
                        break;
                    case EQUALS_COMPARE:
                        writer.emitStatement("if (%s.compare(get%s, a%s.get%s) != 0) return false", fieldTypeCanonicalName, capitaliseFirstChar(fieldName), className, capitaliseFirstChar(fieldName));
                        break;
                }
