...
public class MethodHandlesTest {
    ...
    public void printCounts() {
        if (verbosity >= 2 && (posTests | negTests) != 0) {
            System.out.println();
            if (posTests != 0)
                System.out.println("=== " + testName + ": " + posTests + " positive test cases run");
            if (negTests != 0)
                System.out.println("=== " + testName + ": " + negTests + " negative test cases run");
    ...
    public static void tearDownClass() throws Exception {
        ...
        if (verbosity >= 0 && (posTests | negTests) != 0) {
            System.out.println();
            if (posTests != 0)
                System.out.println("=== " + posTests + " total positive test cases");
            if (negTests != 0)
                System.out.println("=== " + negTests + " total negative test cases");
        }
    }
    ...
    static void assertCalled(String name, Object... args) {
        ...
        if (expected.equals(actual) && verbosity < 9)
            return;
    ...
    static String deepToString(Object x) {
        ...
        if (x instanceof Object[]) {
            ...
            for (int i = 0; i < ax.length; i++) ax[i] = deepToString(ax[i]);
    ...
    static final int ONE_MILLION = (1000 * 1000), TEN_BILLION = (10 * 1000 * 1000 * 1000), INITIAL_ARG_VAL = ONE_MILLION << 1;
    ...
    static Object[] randomArgs(Class<?>... params) {
        ...
        for (int i = 0; i < args.length; i++) args[i] = randomArg(params[i]);
        return args;
    }

    static Object[] randomArgs(int nargs, Class<?> param) {
        Object[] args = new Object[nargs];
        for (int i = 0; i < args.length; i++) args[i] = randomArg(param);
        return args;
    }
    ...
    static Object getClasses(Object x) {
        ...
        if (x instanceof List) {
            ...
            for (int i = 0; i < xa.length; i++) xa[i] = getClasses(xa[i]);
    ...
    static MethodHandle varargsList(int arity, Class<?> rtype) {
        ...
        if (List.class.isAssignableFrom(rtype) || rtype == void.class || rtype == Object.class) {
            ...
            if (LIST_TO_STRING == null)
                try {
                    LIST_TO_STRING = PRIVATE.findStatic(PRIVATE.lookupClass(), "listToString", MethodType.methodType(String.class, List.class));
            ...
            if (LIST_TO_INT == null)
                try {
                    LIST_TO_INT = PRIVATE.findStatic(PRIVATE.lookupClass(), "listToInt", MethodType.methodType(int.class, List.class));
                } catch (NoSuchMethodException | IllegalAccessException ex) {
                    throw new RuntimeException(ex);
                }
    ...
    private static String listToString(List<?> x) {
        return x.toString();
    }

    private static int listToInt(List<?> x) {
    ...
    static class Example implements IntExample {
        ...
        protected Example(int x) {
            this();
            called("protected <init>", this, x);
        }
        ...
        public Object v1(Object x) {
            return called("v1", this, x);
        }

        public Object v2(Object x, Object y) {
            return called("v2", this, x, y);
        }

        public Object v2(Object x, int y) {
            return called("v2", this, x, y);
        }

        public Object v2(int x, Object y) {
            return called("v2", this, x, y);
        }

        public Object v2(int x, int y) {
            return called("v2", this, x, y);
        }

        public static Object s1(Object x) {
            return called("s1", x);
        }

        public static Object s2(int x) {
            return called("s2", x);
        }

        public static Object s3(long x) {
            return called("s3", x);
        }

        public static Object s4(int x, int y) {
            return called("s4", x, y);
        }

        public static Object s5(long x, int y) {
            return called("s5", x, y);
        }

        public static Object s6(int x, long y) {
            return called("s6", x, y);
        }

        public static Object s7(float x, double y) {
            return called("s7", x, y);
        }

        public Example(String x, int y) {
            this.name = x + y;
            called("Example.<init>", x, y);
        }
        ...
        public Example(int x, long y) {
            this.name = x + "" + y;
            called("Example.<init>", x, y);
        }

        public Example(int x, float y) {
            this.name = x + "" + y;
            called("Example.<init>", x, y);
        }
        ...
        public Example(int x, int y, int z) {
            this.name = x + "" + y + "" + z;
            called("Example.<init>", x, y, z);
        }

        public Example(int x, int y, int z, int a) {
            this.name = x + "" + y + "" + z + "" + a;
            called("Example.<init>", x, y, z, a);
        }

        static final Lookup EXAMPLE = MethodHandles.lookup();
    }
    ...
    public static class PubExample extends Example {
        ...
        protected PubExample(String prefix) {
            super(prefix + "#" + nextArg());
        }

        protected void pro_v0() {
            called("Pub/pro_v0", this);
        }
    ...
    static class SubExample extends Example {
        ...
        @Override
        void pkg_v0() {
            called("Sub/pkg_v0", this);
        }

        @SuppressWarnings("LeakingThisInConstructor")
        private SubExample(int x) {
            called("<init>", this, x);
        }

        public SubExample() {
            super("SubExample#" + nextArg());
        }
    }
    ...
    static final Object[][][] ACCESS_CASES = { { { false, PUBLIC }, { false, SUBCLASS }, { false, PACKAGE }, { false, PRIVATE }, { false, EXAMPLE } }, { { false, PUBLIC }, { false, SUBCLASS }, { false, PACKAGE }, { true, PRIVATE }, { true, EXAMPLE } }, { { false, PUBLIC }, { false, SUBCLASS }, { true, PACKAGE }, { true, PRIVATE }, { true, EXAMPLE } }, { { false, PUBLIC }, { true, SUBCLASS }, { true, PACKAGE }, { true, PRIVATE }, { true, EXAMPLE } }, { { true, PUBLIC }, { true, SUBCLASS }, { true, PACKAGE }, { true, PRIVATE }, { true, EXAMPLE } } };

    static Object[][] accessCases(Class<?> defc, String name, boolean isSpecial) {
        Object[][] cases;
        if (name.contains("pri_") || isSpecial) {
            cases = ACCESS_CASES[1];
        } else if (name.contains("pkg_") || !Modifier.isPublic(defc.getModifiers())) {
            cases = ACCESS_CASES[2];
        } else if (name.contains("pro_")) {
            cases = ACCESS_CASES[3];
        } else {
            assertTrue(name.indexOf('_') < 0 || name.contains("fin_"));
            boolean pubc = Modifier.isPublic(defc.getModifiers());
            if (pubc)
                cases = ACCESS_CASES[4];
            else
                cases = ACCESS_CASES[2];
        }
    ...
    void testFindStatic(boolean positive, Lookup lookup, Class<?> defc, Class<?> ret, String name, Class<?>... params) throws Throwable {
        ...
        try {
            ...
            noAccess = ex;
            assertExceptionClass((name.contains("bogus") || INIT_REF_CAUSES_NSME && name.contains("<init>")) ? NoSuchMethodException.class : IllegalAccessException.class, noAccess);
            if (verbosity >= 5)
                ex.printStackTrace(System.out);
        }
        if (verbosity >= 3)
            System.out.println("findStatic " + lookup + ": " + defc.getName() + "." + name + "/" + type + " => " + target + (noAccess == null ? "" : " !! " + noAccess));
    ...
    static void assertExceptionClass(Class<? extends Throwable> expected, Throwable actual) {
    ...
    public void testFindVirtualClone() throws Throwable {
        ...
        for (Class<?> cls : new Class<?>[] { boolean[].class, long[].class, float[].class, char[].class }) testFindVirtual(true, PUBLIC, cls, Object.class, "clone");
    }
    ...
    void testFindVirtual(boolean positive, Lookup lookup, Class<?> rcvc, Class<?> defc, Class<?> ret, String name, Class<?>... params) throws Throwable {
        ...
        if (verbosity >= 3)
            System.out.println("findVirtual " + lookup + ": " + defc.getName() + "." + name + "/" + type + " => " + target + (noAccess == null ? "" : " !! " + noAccess));
        if (positive && noAccess != null)
            throw noAccess;
        assertEquals(positive ? "positive test" : "negative test erroneously passed", positive, target != null);
        if (!positive)
            return;
        Class<?> selfc = defc;
        if (lookup == SUBCLASS && name.contains("pro_") && selfc.isAssignableFrom(lookup.lookupClass())) {
            selfc = lookup.lookupClass();
            if (name.startsWith("Pub/"))
                name = "Rem/" + name.substring(4);
        }
        ...
        if (selfc.isAssignableFrom(rcvc) && rcvc != selfc)
            argsWithSelf[0] = randomArg(rcvc);
    ...
    void testFindSpecial(Class<?> specialCaller, Class<?> defc, Class<?> ret, String name, Class<?>... params) throws Throwable {
    ...
    void testFindSpecial(boolean positive, Lookup lookup, Class<?> specialCaller, Class<?> defc, Class<?> ret, String name, Class<?>... params) throws Throwable {
        ...
        try {
            ...
            noAccess = ex;
            assertExceptionClass((!specialAccessOK) ? IllegalAccessException.class : (name.contains("bogus") || INIT_REF_CAUSES_NSME && name.contains("<init>")) ? NoSuchMethodException.class : IllegalAccessException.class, noAccess);
            if (verbosity >= 5)
                ex.printStackTrace(System.out);
        }
        if (verbosity >= 3)
    ...
    void testFindConstructor(boolean positive, Lookup lookup, Class<?> defc, Class<?>... params) throws Throwable {
        countTest(positive);
        ...
        try {
            if (verbosity >= 4)
                System.out.println("lookup via " + lookup + " of " + defc + " <init>" + type);
        ...
        if (verbosity >= 3)
            System.out.println("findConstructor " + defc.getName() + ".<init>/" + type + " => " + target + (target == null ? "" : target.type()) + (noAccess == null ? "" : " !! " + noAccess));
        if (positive && noAccess != null)
            throw noAccess;
        assertEquals(positive ? "positive test" : "negative test erroneously passed", positive, target != null);
        if (!positive)
    ...
    void testBind(boolean positive, Lookup lookup, Class<?> defc, Class<?> ret, String name, Class<?>... params) throws Throwable {
        ...
        if (verbosity >= 3)
            System.out.println("bind " + receiver + "." + name + "/" + type + " => " + target + (noAccess == null ? "" : " !! " + noAccess));
        if (positive && noAccess != null)
            throw noAccess;
        assertEquals(positive ? "positive test" : "negative test erroneously passed", positive, target != null);
        if (!positive)
    ...
    void testUnreflectMaybeSpecial(Class<?> specialCaller, boolean positive, Lookup lookup, Class<?> defc, Class<?> rcvc, Class<?> ret, String name, Class<?>... params) throws Throwable {
        ...
        try {
            ...
            noAccess = ex;
            assertExceptionClass(IllegalAccessException.class, noAccess);
            if (verbosity >= 5)
                ex.printStackTrace(System.out);
        }
        if (verbosity >= 3)
    ...
    public static class HasFields {
        ...
        static {
            ...
            Object[][] types = { { 'L', Object.class }, { 'R', String.class }, { 'I', int.class }, { 'J', long.class }, { 'F', float.class }, { 'D', double.class }, { 'Z', boolean.class }, { 'B', byte.class }, { 'S', short.class }, { 'C', char.class } };
    ...
    static boolean testModeMatches(int testMode, boolean isStatic) {
        switch(testMode) {
            case TEST_FIND_STATIC:
                return isStatic;
            case TEST_FIND_FIELD:
                return !isStatic;
            case TEST_UNREFLECT:
                return true;
        }
    ...
    public void testGetter(boolean positive, MethodHandles.Lookup lookup, Object fieldRef, Object value, int testMode) throws Throwable {
        testAccessor(positive, lookup, fieldRef, value, testMode);
    }

    public void testAccessor(boolean positive0, MethodHandles.Lookup lookup, Object fieldRef, Object value, int testMode0) throws Throwable {
        if (verbosity >= 4)
        ...
        if (!testModeMatches(testMode, isStatic))
            return;
        if (f == null && testMode == TEST_UNREFLECT)
            return;
        if (testNPE && isStatic)
            return;
        countTest(positive);
        ...
        if (isStatic)
            expType = expType.dropParameterTypes(0, 1);
        Exception noAccess = null;
        MethodHandle mh;
        try {
            switch(testMode0 & ~(TEST_BOUND | TEST_NPE)) {
                case TEST_UNREFLECT:
                    ...
                    break;
                case TEST_FIND_FIELD:
                    mh = lookup.findGetter(fclass, fname, ftype);
                    break;
                case TEST_FIND_STATIC:
                    mh = lookup.findStaticGetter(fclass, fname, ftype);
                    break;
                case TEST_SETTER | TEST_UNREFLECT:
                    mh = lookup.unreflectSetter(f);
                    break;
                case TEST_SETTER | TEST_FIND_FIELD:
                    mh = lookup.findSetter(fclass, fname, ftype);
                    break;
                case TEST_SETTER | TEST_FIND_STATIC:
            ...
            mh = null;
            noAccess = ex;
            assertExceptionClass((fname.contains("bogus")) ? NoSuchFieldException.class : IllegalAccessException.class, noAccess);
            if (verbosity >= 5)
                ex.printStackTrace(System.out);
        }
        if (verbosity >= 3)
            System.out.println("find" + (isStatic ? "Static" : "") + (isGetter ? "Getter" : "Setter") + " " + fclass.getName() + "." + fname + "/" + ftype + " => " + mh + (noAccess == null ? "" : " !! " + noAccess));
        if (positive && !testNPE && noAccess != null)
            throw new RuntimeException(noAccess);
        ...
        if (ftype != int.class)
            vtype = Object.class;
        if (isGetter) {
            mh = mh.asType(mh.type().generic().changeReturnType(vtype));
        } else {
            int last = mh.type().parameterCount() - 1;
            mh = mh.asType(mh.type().generic().changeReturnType(void.class).changeParameterType(last, vtype));
        }
        if (f != null && f.getDeclaringClass() == HasFields.class) {
            assertEquals(f.get(fields), value);
        }
        ...
        if (isGetter) {
            ...
            for (int i = 0; i <= 1; i++) {
                ...
                if (f != null && f.getDeclaringClass() == HasFields.class && !Modifier.isFinal(f.getModifiers())) {
    ...
    public void testSetter(int testMode) throws Throwable {
        ...
        for (int isStaticN = 0; isStaticN <= 1; isStaticN++) {
            testSetter(false, lookup, new Object[] { (isStaticN != 0), System.class, "bogus", char.class }, null, testMode);
        }
    }

    public void testSetter(boolean positive, MethodHandles.Lookup lookup, Object fieldRef, Object value, int testMode) throws Throwable {
    ...
    public void testArrayElementGetterSetter(Object array, boolean testSetter, int negTest) throws Throwable {
        ...
        switch(negTest) {
            ...
            case TEST_ARRAY_ASE:
                ...
                if (elemType == Object.class)
                    arrayToMH = new StringBuffer[length];
                    ...
                    return;
                assert (arrayType.isInstance(arrayToMH)) : Arrays.asList(arrayType, arrayToMH.getClass(), testSetter, negTest);
                break;
        }
        countTest(positive);
        if (verbosity > 2)
            System.out.println("array type = " + array.getClass().getComponentType().getName() + "[" + length + "]" + (positive ? "" : " negative test #" + negTest + " using " + Arrays.deepToString(new Object[] { arrayToMH })));
        MethodType expType = !testSetter ? MethodType.methodType(elemType, arrayType, int.class) : MethodType.methodType(void.class, arrayType, int.class, elemType);
        MethodHandle mh = !testSetter ? MethodHandles.arrayElementGetter(arrayType) : MethodHandles.arrayElementSetter(arrayType);
        assertSame(mh.type(), expType);
        if (elemType != int.class && elemType != boolean.class) {
            MethodType gtype = mh.type().generic().changeParameterType(1, int.class);
            if (testSetter)
                gtype = gtype.changeReturnType(void.class);
            mh = mh.asType(gtype);
        }
        ...
        if (!positive) {
            ...
            switch(negTest) {
                case TEST_ARRAY_ASE:
                    reqType = ArrayStoreException.class;
                    break;
                case TEST_ARRAY_OOB:
                    reqType = ArrayIndexOutOfBoundsException.class;
                    break;
                case TEST_ARRAY_NPE:
                    reqType = NullPointerException.class;
                    break;
                default:
                    assert (false);
            }
    ...
    List<Object> array2list(Object array) {
        ...
        for (int i = 0; i < length; i++) model.add(Array.get(array, i));
        return model;
    }
    ...
    static class Callee {
        ...
        static Object id(Object x, Object y) {
            return called("id", x, y);
        }

        static Object id(Object x, Object y, Object z) {
            return called("id", x, y, z);
        }

        static Object id(Object... vx) {
            return called("id", vx);
        }
    ...
    void testConvert(boolean positive, MethodHandle id, Class<?> rtype, String name, Class<?>... params) throws Throwable {
        ...
        if (rtype == null)
            rtype = idType.returnType();
        for (int i = 0; i < params.length; i++) {
            if (params[i] == null)
                params[i] = idType.parameterType(i);
        }
        ...
        if (verbosity >= 3)
            System.out.println("convert " + id + " to " + newType + " => " + target + (error == null ? "" : " !! " + error));
        if (positive && error != null)
            throw error;
        assertEquals(positive ? "positive test" : "negative test erroneously passed", positive, target != null);
        if (!positive)
            return;
    ...
    public void testVarargsCollector() throws Throwable {
        ...
        MethodHandle vac0 = PRIVATE.findStatic(MethodHandlesTest.class, "called", MethodType.methodType(Object.class, String.class, Object[].class));
    ...
    public void testPermuteArguments(int max, Class<?> type1, int t2c, Class<?> type2, int dilution) throws Throwable {
        if (verbosity >= 2)
            System.out.println("permuteArguments " + max + "*" + type1.getName() + (t2c == 0 ? "" : "/" + t2c + "*" + type2.getName()) + (dilution > 0 ? " with dilution " + dilution : ""));
        int t2pos = t2c == 0 ? 0 : 1;
        for (int inargs = t2pos + 1; inargs <= max; inargs++) {
            ...
            for (int outargs = 0; outargs <= max; outargs++) {
                if (outargs - inargs >= MAX_ARG_INCREASE)
                    continue;
                int casStep = dilution + 1;
                while ((casStep > 2 && casStep % 2 == 0 && inargs % 2 == 0) || (casStep > 3 && casStep % 3 == 0 && inargs % 3 == 0)) casStep++;
                testPermuteArguments(args, types, outargs, numcases, casStep);
                numcases *= inargs;
                if (CAN_TEST_LIGHTLY && outargs < max - 2)
                    continue;
                if (dilution > 10 && outargs >= 4) {
                    ...
                    for (int i = 0; i < inargs; i++) {
                        ...
                        for (int d = 1; d <= 2; d++) {
                            if (i + d >= inargs)
                                continue;
                            for (int j = 1; j < outargs; j += 2) reorder[j] += 1;
                            testPermuteArguments(args, types, reorder);
                    ...
                    for (int i = 1; i < inargs; i++) {
                        for (int len = 3; len <= inargs; len++) {
                            for (int j = 0; j < outargs; j++) reorder[j] = (i + (j % len)) % inargs;
                            testPermuteArguments(args, types, reorder);
                            testPermuteArguments(args, types, reverse(reorder));
                        }
                    }
    ...
    public void testPermuteArguments(Object[] args, Class<?>[] types, int outargs, int numcases, int casStep) throws Throwable {
        ...
        for (int cas = 0; cas < numcases; cas += casStep) {
            ...
            if (CAN_TEST_LIGHTLY && outargs >= 3 && (reorder[0] == reorder[1] || reorder[1] == reorder[2]))
                continue;
            testPermuteArguments(args, types, reorder);
        }
    }
    ...
    public void testSpreadArguments() throws Throwable {
        ...
        for (Class<?> argType : new Class<?>[] { Object.class, Integer.class, int.class }) {
            ...
            for (int nargs = 0; nargs < 50; nargs++) {
                if (CAN_TEST_LIGHTLY && nargs > 11)
                    break;
                for (int pos = 0; pos <= nargs; pos++) {
                    if (CAN_TEST_LIGHTLY && pos > 2 && pos < nargs - 2)
                        continue;
    ...
    void testInsertArguments(int nargs, int pos, int ins) throws Throwable {
        ...
        @SuppressWarnings("cast")
        MethodHandle target2 = MethodHandles.insertArguments(target, pos, (Object[]) argsToInsert.toArray());
        argsToInsert.clear();
    ...
    public void testFilterReturnValue() throws Throwable {
        ...
        for (int nargs = 0; nargs <= 3; nargs++) {
            for (Class<?> rtype : new Class<?>[] { Object.class, List.class, int.class, byte.class, long.class, CharSequence.class, String.class }) {
    ...
    boolean mixArgs(List<Class<?>> argTypes, int mix, Map<Object, Integer> argTypesSeen) {
        ...
        if (prev != null) {
            if (verbosity >= 4)
                System.out.println("mix " + prev + " repeated " + mix + ": " + argTypes);
            return false;
        }
        if (verbosity >= 3)
            System.out.println("mix " + mix + " = " + argTypes);
        return true;
    }

    void testFoldOrCollectArguments(List<Class<?>> argTypes, int pos, int fold, Class<?> combineType, Class<?> lastType, boolean isCollect) throws Throwable {
    ...
    void testDropArguments(int nargs, int pos, int drop) throws Throwable {
        ...
        Object[] args = randomArgs(target.type().parameterArray());
        MethodHandle target2 = MethodHandles.dropArguments(target, pos, Collections.nCopies(drop, Object.class).toArray(new Class<?>[0]));
        List<Object> resList = Arrays.asList(args);
    ...
    public void testInvokers(MethodType type) throws Throwable {
        ...
        MethodHandle target = PRIVATE.findStatic(MethodHandlesTest.class, "invokee", MethodType.genericMethodType(0, true));
        ...
        for (int k = 0; k <= nargs; k++) {
            if (CAN_TEST_LIGHTLY && (k > 1 || k < nargs - 1))
                continue;
            countTest();
            calledLog.clear();
            inv = MethodHandles.spreadInvoker(type, k);
            MethodType expType = (type.dropParameterTypes(k, nargs).appendParameterTypes(Object[].class).insertParameterTypes(0, MethodHandle.class));
    ...
    void testGuardWithTest(int nargs, Class<?> argClass) throws Throwable {
        ...
        if (nargs <= 5 || nargs % 10 == 3) {
            for (int testDrops = 1; testDrops <= nargs; testDrops++) testGuardWithTest(nargs, testDrops, argClass);
        }
    }
    ...
    void testGuardWithTest(int nargs, int testDrops, Class<?> argClass) throws Throwable {
        ...
        while (test.type().parameterCount() > nargs) test = MethodHandles.insertArguments(test, 0, MISSING_ARG);
        ...
        Object[][] argLists = { {}, { "foo" }, { MISSING_ARG }, { "foo", "foo" }, { "foo", "bar" }, { "foo", "foo", "baz" }, { "foo", "bar", "baz" } };
        for (Object[] argList : argLists) {
            Object[] argList1 = argList;
            if (argList.length != nargs) {
                if (argList.length != nargs1)
                    continue;
            ...
            switch(nargs) {
                case 0:
                    ...
                    break;
                case 1:
                    equals = MISSING_ARG.equals(argList[0]);
                    break;
                default:
                    equals = argList[0].equals(argList[1]);
                    break;
            }
    ...
    void testCatchException(Class<?> returnType, Throwable thrown, int throwMode, int nargs) throws Throwable {
        ...
        if (nargs <= 5 || nargs % 10 == 3) {
            for (int catchDrops = 1; catchDrops <= nargs; catchDrops++) testCatchException(returnType, thrown, throwMode, nargs, catchDrops);
        }
    }
    ...
    void testCatchException(Class<?> returnType, Throwable thrown, int throwMode, int nargs, int catchDrops) throws Throwable {
        ...
        if (throwMode > THROW_CAUGHT)
            thrown = new UnsupportedOperationException("do not catch this");
        MethodHandle throwOrReturn = PRIVATE.findStatic(MethodHandlesTest.class, "throwOrReturn", MethodType.methodType(Object.class, Object.class, Throwable.class));
        if (throwMode == THROW_THROUGH_ADAPTER) {
            MethodHandle fakeIdentity = PRIVATE.findVirtual(MethodHandlesTest.class, "fakeIdentity", MethodType.methodType(Object.class, Object.class)).bindTo(this);
            for (int i = 0; i < 10; i++) throwOrReturn = MethodHandles.filterReturnValue(throwOrReturn, fakeIdentity);
        }
    ...
    public void testInterfaceCast() throws Throwable {
        ...
        for (MethodHandle mh : new MethodHandle[] { MethodHandles.identity(String.class), MethodHandles.identity(CharSequence.class), MethodHandles.identity(Iterable.class) }) {
            if (verbosity > 0)
                System.out.println("-- mh = " + mh);
            for (Class<?> ctype : new Class<?>[] { Object.class, String.class, CharSequence.class, Number.class, Iterable.class }) {
                if (verbosity > 0)
                    System.out.println("---- ctype = " + ctype.getName());
    ...
    public void testInterfaceCast(MethodHandle mh, Class<?> ctype, boolean doret, boolean docast) throws Throwable {
        MethodHandle mh0 = mh;
        if (verbosity > 1)
        ...
        MethodType mt0 = mt;
        if (doret)
            mt = mt.changeReturnType(ctype);
        else
            mt = mt.changeParameterType(0, ctype);
        if (docast)
        ...
        Class<?>[] step = { mt1.parameterType(0), mt0.parameterType(0), mt0.returnType(), mt1.returnType() };
    ...
    static class Surprise {
        ...
        static Object refIdentity(Object x) {
            trace("ref.x", x);
            return x;
        }

        static Integer boxIdentity(Integer x) {
            trace("box.x", x);
            return x;
        }

        static int intIdentity(int x) {
            trace("int.x", x);
            return x;
        }
        ...
        static {
            try {
                VALUE = PRIVATE.findVirtual(Surprise.class, "value", MethodType.methodType(Object.class, Object.class));
                REF_IDENTITY = PRIVATE.findStatic(Surprise.class, "refIdentity", MethodType.methodType(Object.class, Object.class));
                BOX_IDENTITY = PRIVATE.findStatic(Surprise.class, "boxIdentity", MethodType.methodType(Integer.class, Integer.class));
                INT_IDENTITY = PRIVATE.findStatic(Surprise.class, "intIdentity", MethodType.methodType(int.class, int.class));
            } catch (NoSuchMethodException | IllegalAccessException ex) {
                throw new RuntimeException(ex);
            }
        }
    ...
    public void testAsInterfaceInstance() throws Throwable {
        ...
        for (Class<?> intfc : new Class<?>[] { Runnable.class, Fooable.class }) {
            int badArity = 1;
            if (verbosity > 2)
...
class ValueConversions {
    ...
    static Object changeArrayType(Class<?> arrayType, Object[] a) {
        ...
        for (int i = 0; i < a.length; i++) java.lang.reflect.Array.set(b, i, a[i]);
        return b;
    }
