...
public final class ProtobufUtil {
    ...
    public static boolean isPBMagicPrefix(final byte[] bytes, int offset, int len) {
        if (bytes == null || len < ProtobufMagic.PB_MAGIC.length)
            return false;
        return Bytes.compareTo(ProtobufMagic.PB_MAGIC, 0, ProtobufMagic.PB_MAGIC.length, bytes, offset, ProtobufMagic.PB_MAGIC.length) == 0;
    }
    ...
    @Deprecated
    public static Result getRowOrBefore(final ClientService.BlockingInterface client, final byte[] regionName, final byte[] row, final byte[] family) throws IOException {
    ...
    public static CompactionDescriptor toCompactionDescriptor(HRegionInfo info, byte[] family, List<Path> inputPaths, List<Path> outputPaths, Path storeDir) {
        CompactionDescriptor.Builder builder = CompactionDescriptor.newBuilder().setTableName(ByteStringer.wrap(info.getTableName())).setEncodedRegionName(ByteStringer.wrap(info.getEncodedNameAsBytes())).setFamilyName(ByteStringer.wrap(family)).setStoreHomeDir(storeDir.getName());
    ...
    public static RegionEventDescriptor toRegionEventDescriptor(EventType eventType, HRegionInfo hri, long seqId, ServerName server, Map<byte[], List<Path>> storeFiles) {
        RegionEventDescriptor.Builder desc = RegionEventDescriptor.newBuilder().setEventType(eventType).setTableName(ByteStringer.wrap(hri.getTable().getName())).setEncodedRegionName(ByteStringer.wrap(hri.getEncodedNameAsBytes())).setLogSequenceNumber(seqId).setServer(toServerName(server));
