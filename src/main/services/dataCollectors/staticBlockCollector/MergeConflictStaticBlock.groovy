        ...
        while (mergeCodeLines.hasNext()) {
		  ...
		  if(!line.isEmpty()) { 
     		if(line!=null && linePrevious == null && linePreviousOfPrevious == null) {
	            	 ...
	            	 if((StringUtils.deleteWhitespace(linePreviousOfPrevious).contains(TOKEN_PARSER_TERMINATOR) || StringUtils.deleteWhitespace(linePreviousOfPrevious).contains(TOKEN_PARSER_KEYS) || StringUtils.deleteWhitespace(linePreviousOfPrevious).contains(MINE_CONFLICT_MARKER)) &&
							 (StringUtils.deleteWhitespace(linePrevious).contains(MINE_CONFLICT_MARKER) || StringUtils.deleteWhitespace(linePrevious).contains(CHANGE_CONFLICT_MARKER)) &&
							 (StringUtils.deleteWhitespace(line).contains(TOKEN_STATIC_BLOCK) || StringUtils.deleteWhitespace(line).equals(TOKEN_STATIC_NONE))){
	            	      count++;      
	            	 }
	            	 linePreviousOfPrevious = linePrevious;
	            	 linePrevious = line;
