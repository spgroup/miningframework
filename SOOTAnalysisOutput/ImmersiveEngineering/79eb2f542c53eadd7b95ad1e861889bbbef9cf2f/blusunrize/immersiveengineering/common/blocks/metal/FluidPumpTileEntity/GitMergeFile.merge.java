...
public class FluidPumpTileEntity extends IEBaseTileEntity implements ITickableTileEntity, IBlockBounds, IHasDummyBlocks, IConfigurableSides, IFluidPipe, IIEInternalFluxHandler, IBlockOverlayText {
    ...
    {
        for (Direction neighbor : Direction.VALUES) neighborFluids.put(neighbor, CapabilityReference.forNeighbor(this, CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, neighbor));
    }
    ...
    public void tick() {
        ...
        if (isRSPowered()) {
            for (Direction f : Direction.values()) if (sideConfig.get(f) == IOSideConfig.INPUT) {
...
<<<<<<< MINE
                } else if (world.getGameTime() % 20 == ((getPos().getX() ^ getPos().getZ()) & 19) && world.getFluidState(getPos().offset(f)).getFluid().isIn(FluidTags.WATER) && IEServerConfig.MACHINES.pump_infiniteWater.get() && tank.fill(new FluidStack(Fluids.WATER, 1000), FluidAction.SIMULATE) == 1000 && this.energyStorage.extractEnergy(consumption, true) >= consumption) {
=======
                } else if (world.getGameTime() % 20 == ((getPos().getX() ^ getPos().getZ()) & 19) && world.getFluidState(getPos().offset(f)).getFluid().isIn(FluidTags.WATER) && IEConfig.MACHINES.pump_infiniteWater.get() && tank.fill(new FluidStack(Fluids.WATER, FluidAttributes.BUCKET_VOLUME), FluidAction.SIMULATE) == FluidAttributes.BUCKET_VOLUME && this.energyStorage.extractEnergy(consumption, true) >= consumption) {
>>>>>>> YOURS
            ...
            if (world.getGameTime() % 40 == (((getPos().getX() ^ getPos().getZ())) % 40 + 40) % 40) {
                if (closedList.isEmpty())
                    ...
                    else if (tank.fill(fs, FluidAction.SIMULATE) == fs.getAmount() && this.energyStorage.extractEnergy(consumption, true) >= consumption) {
                        this.energyStorage.extractEnergy(consumption, false);
            ...
            if (!checked.contains(next)) {
                ...
                if (fluid != Fluids.EMPTY && (fluid != Fluids.WATER || !infiniteWater) && (searchFluid == null || fluid == searchFluid)) {
                    ...
                    for (Direction f : Direction.values()) {
                        ...
                        if (fluid != Fluids.EMPTY && !checked.contains(pos2) && !closedList.contains(pos2) && !openList.contains(pos2) && (fluid != Fluids.WATER || !infiniteWater) && (searchFluid == null || fluid == searchFluid))
                            openList.add(pos2);
                    }
                }
                checked.add(next);
    ...
    public int outputFluid(FluidStack fs, FluidAction action) {
        ...
        for (Direction f : Direction.values()) if (sideConfig.get(f) == IOSideConfig.OUTPUT) {
    ...
    public void readCustomNBT(CompoundNBT nbt, boolean descPacket) {
        ...
        for (Direction d : Direction.VALUES) sideConfig.put(d, IOSideConfig.VALUES[sideConfigArray[d.ordinal()]]);
    ...
    public void writeCustomNBT(CompoundNBT nbt, boolean descPacket) {
        ...
        for (Direction d : Direction.VALUES) sideConfigArray[d.ordinal()] = sideConfig.get(d).ordinal();
    ...
    public void placeDummies(BlockItemUseContext ctx, BlockState state) {
        ...
        getWorldNonnull().setBlockState(dummyPos, IEBaseBlock.applyLocationalWaterlogging(state, getWorldNonnull(), dummyPos));
    ...
    public void breakDummies(BlockPos pos, BlockState state) {
        for (int i = 0; i <= 1; i++) if (Utils.isBlockAt(world, getPos().add(0, isDummy() ? -1 : 0, 0).add(0, i, 0), MetalDevices.fluidPump))
