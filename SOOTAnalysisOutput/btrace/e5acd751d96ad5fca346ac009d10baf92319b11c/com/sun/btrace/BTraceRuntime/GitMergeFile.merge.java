...
>>>>>>> YOURS
    ...
    public BTraceRuntime(final String className, String[] args, final CommandListener cmdListener, DebugSupport ds, Instrumentation inst) {
        this.args = args;
        ...
        this.cmdThread = new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    BTraceRuntime.enter();
                    queue.drain(new ConsumerWrapper(cmdListener, exitting), waitStrategy, exitCondition);
                } finally {
    ...
    static void writeXML(Object obj, String fileName) {
        try {
            try (BufferedWriter bw = new BufferedWriter(new FileWriter(resolveFileName(fileName)))) {
    ...
    private static void setupCmdQueueParams() {
        ...
        if (maxQLen == null) {
            ...
            try {
                ...
                warning("\"" + maxQLen + "\" is not a valid int number. " + "Using the default cmd queue limit of " + CMD_QUEUE_LIMIT_DEFAULT);
                CMD_QUEUE_LIMIT = CMD_QUEUE_LIMIT_DEFAULT;
            }
        }
    }
