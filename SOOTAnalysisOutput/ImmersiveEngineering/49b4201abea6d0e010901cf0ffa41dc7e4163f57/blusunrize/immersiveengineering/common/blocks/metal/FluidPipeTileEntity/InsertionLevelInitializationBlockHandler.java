...
public class FluidPipeTileEntity extends IEBaseTileEntity implements IFluidPipe, IAdvancedHasObjProperty, IOBJModelCallback<BlockState>, IColouredTile, IPlayerInteraction, IHammerInteraction, IPlacementInteraction, ISelectionBounds, ICollisionBounds, IAdditionalDrops {
    ...
    {
        for (Direction d : Direction.VALUES) sideConfig.put(d, true);
    }
                    ...
                    if (((IFluidPipe) pipeTile).hasOutputConnection(fd)) {
                        ...
                        if (adjacentTile != null)
                            if (adjacentTile instanceof FluidPipeTileEntity)
                                ...
                                LazyOptional<IFluidHandler> handlerOptional = adjacentTile.getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, fd.getOpposite());
    ...
    public void onLoad() {
        ...
        if (world != null && !world.isRemote) {
            ApiUtils.addFutureServerTask(world, () -> {
                ...
                for (Direction f : Direction.VALUES) changed |= updateConnectionByte(f);
                if (changed) {
    ...
    public void onEntityCollision(World world, Entity entity) {
        if (!(entity instanceof LivingEntity) || ((LivingEntity) entity).isOnLadder() || this.cover == Blocks.AIR)
            ...
            boolean climb = false;
            for (Predicate<Block> f : climbablePipeCovers) if (f != null && f.test(cover)) {
                climb = true;
                break;
            }
            if (climb)
    ...
    public void writeCustomNBT(CompoundNBT nbt, boolean descPacket) {
        ...
        for (int i = 0; i < 6; ++i) if (sideConfig.getBoolean(Direction.byIndex(i)))
            config[i] = 1;
    ...
    public List<BakedQuad> modifyQuads(BlockState object, List<BakedQuad> quads) {
        if (hasCover()) {
            ...
            if (model != null)
                for (RenderType layer : new RenderType[] { RenderType.getSolid(), RenderType.getTranslucent(), RenderType.getCutout(), RenderType.getCutoutMipped() }) {
                    ForgeHooksClient.setRenderLayer(layer);
                    for (Direction direction : Direction.values()) quads.addAll(model.getQuads(state, direction, world.rand, EmptyModelData.INSTANCE));
    ...
    static class PipeFluidHandler implements IFluidHandler {
            ...
            if (sum > 0) {
                ...
                for (DirectionalFluidOutput output : sorting.keySet()) {
                    ...
                    if (sum > resource.getAmount()) {
                        ...
                        amount = (int) Math.ceil(MathHelper.clamp(amount, 1, Math.min(resource.getAmount() * prio, tileSpecificAcceptedFluid)));
        ...
        private int getTranferrableAmount(FluidStack resource, DirectionalFluidOutput output) {
            return (resource.hasTag() && resource.getOrCreateTag().contains("pressurized")) || pipe.canOutputPressurized(output.containingTile, false) ? FluidAttributes.BUCKET_VOLUME : FluidAttributes.BUCKET_VOLUME / 20;
        }
    ...
    public int getConnectionStyle(Direction connection) {
        int styleConnections = connections | (1 << connection.getIndex());
        if (styleConnections != 3 && styleConnections != 12 && styleConnections != 48)
            return 1;
        ...
        if (con instanceof FluidPipeTileEntity) {
            ...
            if (styleConnections == tileConnections)
                return 0;
        }
        return 1;
    }
        ...
        for (Direction d : Direction.VALUES) {
            ...
            if ((availableConnections & 1) == 1) {
                if ((activeConnections & 1) == 1 || key.showToolView) {
                    list.add(new AxisAlignedBB(i == 4 ? 0 : i == 5 ? 0.75 : 0.25, i == 0 ? 0 : i == 1 ? 0.75 : 0.25, i == 2 ? 0 : i == 3 ? 0.75 : 0.25, i == 4 ? 0.25 : i == 5 ? 1 : 0.75, i == 0 ? 0.25 : i == 1 ? 1 : 0.75, i == 2 ? 0.25 : i == 3 ? 1 : 0.75));
                    if (key.connectionStyles.get(d) == 1)
                    list.add(new AxisAlignedBB(i == 4 ? 0 : i == 5 ? 0.875 : 0.125, i == 0 ? 0 : i == 1 ? 0.875 : 0.125, i == 2 ? 0 : i == 3 ? 0.875 : 0.125, i == 4 ? 0.125 : i == 5 ? 1 : 0.875, i == 0 ? 0.125 : i == 1 ? 1 : 0.875, i == 2 ? 0.125 : i == 3 ? 1 : 0.875));
            }
            }
    ...
    private static class BoundingBoxKey {
        ...
        private BoundingBoxKey(boolean showToolView, FluidPipeTileEntity te) {
            ...
            for (Direction d : Direction.VALUES) connectionStyles.put(d, te.getConnectionStyle(d));
        }
        ...
        public boolean equals(Object o) {
            ...
            return showToolView == that.showToolView && connections == that.connections && availableConnections == that.availableConnections && hasCover == that.hasCover && connectionStyles.equals(that.connectionStyles);
        }
    ...
    public static IEObjState getStateFromKey(String key) {
        if (!cachedOBJStates.containsKey(key)) {
            ...
            switch(totalConnections) {
                ...
                case 1:
                    ...
                    if ((connections & 2) != 0)
                        ...
                        rotationMatrix.rotate(-Math.PI / 2, 1, 0, 0);
                    else if ((connections & 16) != 0)
                        rotationMatrix.rotate(-Math.PI / 2, 0, 0, 1);
                    else if ((connections & 32) != 0)
                        rotationMatrix.rotate(Math.PI / 2, 0, 0, 1);
                    parts.add("con_yMin");
                    break;
                case 2:
                    if (straightY) {
                        ...
                        if ((connections & 3) != 0) {
                            ...
                            else if (connectTo == 24)
                                rotationMatrix.rotate(-Math.PI / 2, 1, 0, 0);
                            else if (connectTo == 36)
                                rotationMatrix.rotate(Math.PI / 2, 1, 0, 0);
                        }
                    }
                    break;
            ...
            rotationMatrix.translate(-0.5, -0.5, -0.5);
            cachedOBJStates.put(key, new IEObjState(VisibilityList.show(parts), new TransformationMatrix(rotationMatrix.toMatrix4f())));
        }
    ...
    public boolean interact(Direction side, PlayerEntity player, Hand hand, ItemStack heldItem, float hitX, float hitY, float hitZ) {
        if (heldItem.isEmpty() && player.isSneaking() && hasCover()) {
            ...
            if (heldBlock != Blocks.AIR)
                for (Predicate<Block> func : validPipeCovers) if (func.test(heldBlock)) {
    ...
    public boolean hammerUseSide(Direction side, PlayerEntity player, Hand hand, Vector3d hitVec) {
        ...
        for (AxisAlignedBB box : boxes) if (box.grow(.002).contains(hitVec)) {
            for (Direction d : Direction.VALUES) {
    ...
    public void onTilePlaced(World world, BlockPos pos, BlockState state, Direction side, float hitX, float hitY, float hitZ, LivingEntity placer, ItemStack stack) {
        if (!world.isRemote()) {
        TileEntity te;
            for (Direction dir : Direction.values()) if ((te = world.getTileEntity(pos.offset(dir))) instanceof FluidPipeTileEntity) {
