    ...
    public static DateFormatProvider getDateFormatProvider() {
        return new DateFormatProvider() {
            ...
            public DateFormat getDateInstance(int style, Locale locale) {
                ...
                return new SimpleDateFormat(patterns.get(style / 2), getCalendarLocale(locale));
            }

            @Override
            public DateFormat getTimeInstance(int style, Locale locale) {
                AtomicReferenceArray<String> patterns = getDateTimePatterns(locale);
                return new SimpleDateFormat(patterns.get(style / 2 + 2), getCalendarLocale(locale));
            }

            @Override
            public DateFormat getDateTimeInstance(int dateStyle, int timeStyle, Locale locale) {
                AtomicReferenceArray<String> patterns = getDateTimePatterns(locale);
                String pattern = new StringBuilder(patterns.get(dateStyle / 2)).append(" ").append(patterns.get(timeStyle / 2 + 2)).toString();
            ...
            private AtomicReferenceArray<String> getDateTimePatterns(Locale locale) {
                ...
                if (ref == null || (patterns = ref.get()) == null) {
                    ...
                    patterns.compareAndSet(0, null, convertDateTimePattern(getDateTimePattern(DateFormat.LONG, -1, langtag)));
                    patterns.compareAndSet(1, null, convertDateTimePattern(getDateTimePattern(DateFormat.SHORT, -1, langtag)));
                    patterns.compareAndSet(2, null, convertDateTimePattern(getDateTimePattern(-1, DateFormat.LONG, langtag)));
                    patterns.compareAndSet(3, null, convertDateTimePattern(getDateTimePattern(-1, DateFormat.SHORT, langtag)));
    ...
    public static NumberFormatProvider getNumberFormatProvider() {
        return new NumberFormatProvider() {
            ...
            public NumberFormat getCurrencyInstance(Locale locale) {
                ...
                return new DecimalFormat(patterns.get(NF_CURRENCY), DecimalFormatSymbols.getInstance(locale));
            }

            @Override
            public NumberFormat getIntegerInstance(Locale locale) {
                AtomicReferenceArray<String> patterns = getNumberPatterns(locale);
                return new DecimalFormat(patterns.get(NF_INTEGER), DecimalFormatSymbols.getInstance(locale));
            }

            @Override
            public NumberFormat getNumberInstance(Locale locale) {
                AtomicReferenceArray<String> patterns = getNumberPatterns(locale);
                return new DecimalFormat(patterns.get(NF_NUMBER), DecimalFormatSymbols.getInstance(locale));
            }

            @Override
            public NumberFormat getPercentInstance(Locale locale) {
                AtomicReferenceArray<String> patterns = getNumberPatterns(locale);
                return new DecimalFormat(patterns.get(NF_PERCENT), DecimalFormatSymbols.getInstance(locale));
            }
    ...
    public static CalendarDataProvider getCalendarDataProvider() {
        return new CalendarDataProvider() {
            ...
            public int getFirstDayOfWeek(Locale locale) {
                int first = getCalendarDataValue(removeExtensions(locale).toLanguageTag(), CD_FIRSTDAYOFWEEK);
                if (first != -1) {
                    return (first + 1) % 7 + 1;
                } else {
                    return 0;
    ...
    public static CalendarProvider getCalendarProvider() {
        return new CalendarProvider() {
            ...
            public Calendar getInstance(TimeZone zone, Locale locale) {
                return new Calendar.Builder().setLocale(getCalendarLocale(locale)).setTimeZone(zone).setInstant(System.currentTimeMillis()).build();
            }
        };
    }
    ...
    public static CurrencyNameProvider getCurrencyNameProvider() {
        return new CurrencyNameProvider() {
            ...
            public String getSymbol(String currencyCode, Locale locale) {
                try {
                    if (Currency.getInstance(locale).getCurrencyCode().equals(currencyCode)) {
                        return getDisplayString(locale.toLanguageTag(), DN_CURRENCY_SYMBOL, currencyCode);
                    }
            ...
            public String getDisplayName(String currencyCode, Locale locale) {
                try {
                    if (Currency.getInstance(locale).getCurrencyCode().equals(currencyCode)) {
                        return getDisplayString(locale.toLanguageTag(), DN_CURRENCY_NAME, currencyCode);
                    }
                } catch (IllegalArgumentException iae) {
                }
                return null;
    ...
    public static LocaleNameProvider getLocaleNameProvider() {
        return new LocaleNameProvider() {
            ...
            public String getDisplayCountry(String countryCode, Locale locale) {
                return getDisplayString(locale.toLanguageTag(), DN_LOCALE_REGION, nativeDisplayLanguage + "-" + countryCode);
            }
    ...
    private static boolean isSupportedNativeDigitLocale(Locale locale) {
        ...
        if (supportedLocaleSet.contains(base)) {
            if (numtype == null || numtype.equals("latn")) {
                ...
                return "thai".equals(numtype) && isNativeDigit(locale.toLanguageTag());
            }
        }
        return false;
    }
