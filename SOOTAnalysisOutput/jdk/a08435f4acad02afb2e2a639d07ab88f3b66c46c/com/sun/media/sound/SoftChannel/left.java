...
public final class SoftChannel implements MidiChannel, ModelDirectedPlayer {
    ...
    private int findFreeVoice(int x) {
        ...
        for (int i = x; i < voices.length; i++) if (!voices[i].active)
            return i;
    ...
    void initVoice(SoftVoice voice, SoftPerformer p, int voiceID, int noteNumber, int velocity, int delay, ModelConnectionBlock[] connectionBlocks, ModelChannelMixer channelmixer, boolean releaseTriggered) {
        if (voice.active) {
            ...
            for (int i = 0; i < voices.length; i++) if (voices[i].active && voices[i].voiceID == voice.voiceID)
                voices[i].soundOff();
            return;
        }
    ...
    public void controlChangePerNote(int noteNumber, int controller, int value) {
        ...
        if (controller < 120) {
            for (int i = 0; i < voices.length; i++) if (voices[i].active)
                ...
                case 64:
                    ...
                    if (sustain != on) {
                        ...
                        if (!on) {
                            ...
                            for (int i = 0; i < voices.length; i++) if (voices[i].active && voices[i].channel == channel)
                                voices[i].redamp();
                        }
                    }
                    break;
    ...
    public void allSoundOff() {
        ...
        synchronized (control_mutex) {
            for (int i = 0; i < voices.length; i++) if (voices[i].on && voices[i].channel == channel)
                voices[i].soundOff();
        }
    }
