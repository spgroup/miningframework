...
public class MessageView extends K9Activity implements OnClickListener {
    ...
    public void onCreate(Bundle icicle) {
        ...
        mMessageView.setAttachmentCallback(new AttachmentFileDownloadCallback() {
            ...
            public void showFileBrowser(final AttachmentView caller) {
                FileBrowserHelper.getInstance().showFileBrowserActivity(MessageView.this, null, MessageView.ACTIVITY_CHOOSE_DIRECTORY, callback);
    ...
    private void setupButtonViews() {
        ...
        if ((Account.ScrollButtons.ALWAYS == scrollButtons) || (Account.ScrollButtons.KEYBOARD_AVAILABLE == scrollButtons && (this.getResources().getConfiguration().hardKeyboardHidden == Configuration.HARDKEYBOARDHIDDEN_NO))) {
            scrollButtons();
        } else {
            staticButtons();
        }
        Account.ScrollButtons scrollMoveButtons = mAccount.getScrollMessageViewMoveButtons();
        if ((Account.ScrollButtons.ALWAYS == scrollMoveButtons) || (Account.ScrollButtons.KEYBOARD_AVAILABLE == scrollMoveButtons && (this.getResources().getConfiguration().hardKeyboardHidden == Configuration.HARDKEYBOARDHIDDEN_NO))) {
            scrollMoveButtons();
        } else {
            staticMoveButtons();
        }
    ...
    private void setupDisplayMessageButtons() {
        ...
        if (mController.isMoveCapable(mAccount)) {
            mArchive.setEnabled(!mMessageReference.folderName.equals(mAccount.getArchiveFolderName()) && !K9.FOLDER_NONE.equalsIgnoreCase(mAccount.getArchiveFolderName()));
            mSpam.setEnabled(!mMessageReference.folderName.equals(mAccount.getSpamFolderName()) && !K9.FOLDER_NONE.equalsIgnoreCase(mAccount.getSpamFolderName()));
    ...
    private void onFlag() {
        if (mMessage != null) {
            mController.setFlag(mAccount, mMessage.getFolder().getName(), new String[] { mMessage.getUid() }, Flag.FLAGGED, !mMessage.isSet(Flag.FLAGGED));
            try {
    ...
    protected Dialog onCreateDialog(final int id) {
        switch(id) {
            case R.id.dialog_confirm_delete:
                return ConfirmationDialog.create(this, id, R.string.dialog_confirm_delete_title, R.string.dialog_confirm_delete_message, R.string.dialog_confirm_delete_confirm_button, R.string.dialog_confirm_delete_cancel_button, new Runnable() {
            ...
            case R.id.dialog_confirm_spam:
                return ConfirmationDialog.create(this, id, R.string.dialog_confirm_spam_title, getResources().getQuantityString(R.plurals.dialog_confirm_spam_message, 1), R.string.dialog_confirm_spam_confirm_button, R.string.dialog_confirm_spam_cancel_button, new Runnable() {
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        if (menu != null) {
            ...
            if (additionalHeadersItem != null) {
                additionalHeadersItem.setTitle(mMessageView.additionalHeadersVisible() ? R.string.hide_full_header_action : R.string.show_full_header_action);
            }
        }
    ...
    class Listener extends MessagingListener {
        ...
        @Override
        public void loadMessageForViewHeadersAvailable(final Account account, String folder, String uid, final Message message) {
            if (!mMessageReference.uid.equals(uid) || !mMessageReference.folderName.equals(folder) || !mMessageReference.accountUuid.equals(account.getUuid())) {
                return;
            }
            MessageView.this.mMessage = message;
            runOnUiThread(new Runnable() {
