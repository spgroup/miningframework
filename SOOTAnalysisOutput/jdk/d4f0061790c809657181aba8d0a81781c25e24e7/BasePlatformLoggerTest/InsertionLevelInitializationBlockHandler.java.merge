...
public class BasePlatformLoggerTest {
    ...
    static final PlatformLogger.Level[] julLevels = { PlatformLogger.Level.ALL, PlatformLogger.Level.FINEST, PlatformLogger.Level.FINER, PlatformLogger.Level.FINE, PlatformLogger.Level.CONFIG, PlatformLogger.Level.INFO, PlatformLogger.Level.WARNING, PlatformLogger.Level.SEVERE, PlatformLogger.Level.OFF };
    ...
    public static interface TestLoggerFinder {
        ...
        public static final class LogEvent implements Cloneable {
            ...
            Object[] toArray() {
                return new Object[] { sequenceNumber, isLoggable, loggerName, level, bundle, thrown, args, supplier, msg };
            }
    ...
    public static void test(TestLoggerFinder provider, boolean hasRequiredPermissions) {
        ...
        try {
            allowControl.get().set(true);
            appSink = TestLoggerFinder.LoggerImpl.class.cast(provider.getLogger("foo", BasePlatformLoggerTest.class.getModule()));
        } finally {
    ...
    private static void testLogger(TestLoggerFinder provider, Map<PlatformLogger, String> loggerDescMap, String name, ResourceBundle loggerBundle, PlatformLogger logger, TestLoggerFinder.LoggerImpl sink) {
        ...
        for (Level loggerLevel : Level.values()) {
            ...
            for (PlatformLogger.Level messageLevel : julLevels) {
                ...
                String desc2 = "logger." + messageLevel.toString().toLowerCase() + "(fooMsg): loggerLevel=" + loggerLevel + ", messageLevel=" + messageLevel;
        ...
        for (Level loggerLevel : Level.values()) {
            ...
            for (PlatformLogger.Level messageLevel : julLevels) {
                Level expectedMessageLevel = julToSpiMap.get(messageLevel);
                TestLoggerFinder.LogEvent expected = TestLoggerFinder.LogEvent.of(sequencer.get(), loggerLevel != Level.OFF && expectedMessageLevel.compareTo(loggerLevel) >= 0, name, expectedMessageLevel, (ResourceBundle) null, fooMsg, null, (Throwable) thrown, (Object[]) null);
                String desc2 = "logger." + messageLevel.toString().toLowerCase() + "(msg, thrown): loggerLevel=" + loggerLevel + ", messageLevel=" + messageLevel;
                if (messageLevel == PlatformLogger.Level.FINEST) {
        ...
        for (Level loggerLevel : Level.values()) {
            ...
            for (PlatformLogger.Level messageLevel : julLevels) {
                Level expectedMessageLevel = julToSpiMap.get(messageLevel);
                TestLoggerFinder.LogEvent expected = TestLoggerFinder.LogEvent.of(sequencer.get(), loggerLevel != Level.OFF && expectedMessageLevel.compareTo(loggerLevel) >= 0, name, expectedMessageLevel, (ResourceBundle) null, format, null, (Throwable) null, foo, fooMsg);
                String desc2 = "logger." + messageLevel.toString().toLowerCase() + "(format, foo, fooMsg): loggerLevel=" + loggerLevel + ", messageLevel=" + messageLevel;
                if (messageLevel == PlatformLogger.Level.FINEST) {
