...
public class ObjectStore implements RawStore, Configurable {
    ...
    private void initializeHelper(Properties dsProps) {
        ...
        LOG.debug("RawStore: {}, with PersistenceManager: {}" + " created in the thread with id: {}", this, pm, Thread.currentThread().getId());
    }
    ...
    @Override
    public Table getTable(String catName, String dbName, String tableName, long txnId, String writeIdList) throws MetaException {
        boolean commited = false;
        Table tbl = null;
        try {
            openTransaction();
            ...
            if (writeIdList != null) {
                if (tbl != null && TxnUtils.isTransactionalTable(tbl) && tbl.getPartitionKeysSize() == 0) {
    ...
    private MCreationMetadata convertToMCreationMetadata(CreationMetadata m) throws MetaException {
        ...
        return new MCreationMetadata(m.getCatName(), m.getDbName(), m.getTblName(), tablesUsed, m.getValidTxnList());
    }
    ...
    @Override
    public Partition getPartition(String catName, String dbName, String tableName, List<String> part_vals, long txnId, String writeIdList) throws NoSuchObjectException, MetaException {
        openTransaction();
    ...
    private void setPartitionStatsParam(Partition part, Map<String, String> tableParams, long partWriteId, long reqTxnId, String reqWriteIdList) throws MetaException {
    ...
    private PartitionValuesResponse getDistinctValuesForPartitionsNoTxn(String catName, String dbName, String tableName, List<FieldSchema> cols, boolean applyDistinct, long maxParts) throws MetaException {
        try {
            ...
            List<FieldSchema> partitionKeys = getTable(catName, dbName, tableName, -1, null).getPartitionKeys();
    ...
    @Override
    public void alterTable(String catName, String dbname, String name, Table newTable, long queryTxnId, String queryValidWriteIds) throws InvalidObjectException, MetaException {
        boolean success = false;
    ...
    private MColumnDescriptor alterPartitionNoTxn(String catName, String dbname, String name, List<String> part_vals, Partition newPart, long queryTxnId, String queryValidWriteIds) throws InvalidObjectException, MetaException {
        catName = normalizeIdentifier(catName);
        name = normalizeIdentifier(name);
        dbname = normalizeIdentifier(dbname);
        ...
        if (isTxn) {
            String errorMsg = verifyStatsChangeCtx(oldp.getParameters(), newPart.getParameters(), newPart.getWriteId(), queryValidWriteIds, false);
            if (errorMsg != null) {
                throw new MetaException(errorMsg);
            }
        }
    ...
    @Override
    public void alterPartition(String catName, String dbname, String name, List<String> part_vals, Partition newPart, long queryTxnId, String queryValidWriteIds) throws InvalidObjectException, MetaException {
        boolean success = false;
        Throwable e = null;
        try {
            openTransaction();
            ...
            MColumnDescriptor oldCd = alterPartitionNoTxn(catName, dbname, name, part_vals, newPart, queryTxnId, queryValidWriteIds);
            removeUnusedColumnDescriptor(oldCd);
            success = commitTransaction();
    ...
    @Override
    public void alterPartitions(String catName, String dbname, String name, List<List<String>> part_vals, List<Partition> newParts, long writeId, long queryTxnId, String queryWriteIdList) throws InvalidObjectException, MetaException {
        boolean success = false;
        Exception e = null;
        try {
            openTransaction();
            ...
            for (Partition tmpPart : newParts) {
                ...
                MColumnDescriptor oldCd = alterPartitionNoTxn(catName, dbname, name, tmpPartVals, tmpPart, queryTxnId, queryWriteIdList);
                if (oldCd != null) {
                    oldCds.add(oldCd);
                }
            }
            ...
            if (CollectionUtils.isNotEmpty(privilegeList)) {
                ...
                while (privIter.hasNext()) {
                    ...
                    if (hiveObject.getObjectType() == HiveObjectType.GLOBAL) {
                        ...
                        boolean found = false;
                        Table tabObj = this.getTable(catName, hiveObject.getDbName(), hiveObject.getObjectName(), -1, null);
                        String partName = null;
                        if (hiveObject.getPartValues() != null) {
    ...
    public boolean isPartitionMarkedForEvent(String catName, String dbName, String tblName, Map<String, String> partName, PartitionEventType evtType) throws UnknownTableException, MetaException, InvalidPartitionException, UnknownPartitionException {
        ...
        try {
            ...
            Table tbl = getTable(catName, dbName, tblName, -1, null);
            if (null == tbl) {
    ...
    public Table markPartitionForEvent(String catName, String dbName, String tblName, Map<String, String> partName, PartitionEventType evtType) throws MetaException, UnknownTableException, InvalidPartitionException, UnknownPartitionException {
        ...
        try {
            openTransaction();
            tbl = getTable(catName, dbName, tblName, -1, null);
            if (null == tbl) {
                throw new UnknownTableException("Table: " + tblName + " is not found.");
            }
    ...
    @Override
    public boolean updateTableColumnStatistics(ColumnStatistics colStats, long txnId, String validWriteIds, long writeId) throws NoSuchObjectException, MetaException, InvalidObjectException, InvalidInputException {
        boolean committed = false;
        openTransaction();
        try {
    ...
    @Override
    public boolean updatePartitionColumnStatistics(ColumnStatistics colStats, List<String> partVals, long txnId, String validWriteIds, long writeId) throws MetaException, NoSuchObjectException, InvalidObjectException, InvalidInputException {
        boolean committed = false;
        try {
            openTransaction();
            List<ColumnStatisticsObj> statsObjs = colStats.getStatsObj();
    ...
    @Override
    public ColumnStatistics getTableColumnStatistics(String catName, String dbName, String tableName, List<String> colNames, long txnId, String writeIdList) throws MetaException, NoSuchObjectException {
        Boolean isCompliant = null;
        if (writeIdList != null) {
            MTable table = this.getMTable(catName, dbName, tableName);
            isCompliant = !TxnUtils.isTransactionalTable(table.getParameters()) || isCurrentStatsValidForTheQuery(table, txnId, writeIdList, false);
        }
    ...
    @Override
    public List<ColumnStatistics> getPartitionColumnStatistics(String catName, String dbName, String tableName, List<String> partNames, List<String> colNames, long txnId, String writeIdList) throws MetaException, NoSuchObjectException {
        ...
        if (writeIdList != null) {
            for (ColumnStatistics cs : allStats) {
                ...
                if (mpart == null || !isCurrentStatsValidForTheQuery(mpart, txnId, writeIdList, false)) {
                    if (mpart != null) {
                        LOG.debug("The current metastore transactional partition column statistics for {}.{}.{} " + "(write ID {}) are not valid for current query ({} {})", dbName, tableName, mpart.getPartitionName(), mpart.getWriteId(), txnId, writeIdList);
                    }
    ...
    @Override
    public AggrStats get_aggr_stats_for(String catName, String dbName, String tblName, final List<String> partNames, final List<String> colNames, long txnId, String writeIdList) throws MetaException, NoSuchObjectException {
        if (writeIdList != null) {
            if (partNames == null && partNames.isEmpty()) {
                LOG.warn("The given partNames does not have any name.");
                return null;
    ...
    private boolean isCurrentStatsValidForTheQuery(MTable tbl, long queryTxnId, String queryValidWriteIdList, boolean isCompleteStatsWriter) throws MetaException {
        return isCurrentStatsValidForTheQuery(conf, tbl.getDatabase().getName(), tbl.getTableName(), tbl.getParameters(), tbl.getWriteId(), queryTxnId, queryValidWriteIdList, isCompleteStatsWriter);
    }

    private boolean isCurrentStatsValidForTheQuery(MPartition part, long queryTxnId, String queryValidWriteIdList, boolean isCompleteStatsWriter) throws MetaException {
        return isCurrentStatsValidForTheQuery(conf, part.getTable().getDatabase().getName(), part.getTable().getTableName(), part.getParameters(), part.getWriteId(), queryTxnId, queryValidWriteIdList, isCompleteStatsWriter);
    }

    private boolean isCurrentStatsValidForTheQuery(Partition part, long partWriteId, long queryTxnId, String queryValidWriteIdList, boolean isCompleteStatsWriter) throws MetaException {
        return isCurrentStatsValidForTheQuery(conf, part.getDbName(), part.getTableName(), part.getParameters(), partWriteId, queryTxnId, queryValidWriteIdList, isCompleteStatsWriter);
    }

    public static boolean isCurrentStatsValidForTheQuery(Configuration conf, String dbName, String tblName, Map<String, String> statsParams, long statsWriteId, long queryTxnId, String queryValidWriteIdList, boolean isCompleteStatsWriter) throws MetaException {
        LOG.info("isCurrentStatsValidForTheQuery with stats write ID {}; query {}, {}; writer: {} params {}", statsWriteId, queryTxnId, queryValidWriteIdList, isCompleteStatsWriter, statsParams);
        if (statsWriteId < 1) {
            return true;
        }
        ...
        long statsTxnId = HiveMetaStore.HMSHandler.getMsThreadTxnHandler(conf).getTxnIdForWriteId(dbName, tblName, statsWriteId);
