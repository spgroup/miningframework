...
public class PJOGL extends PGL {
    ...
    protected void initSurface(int antialias) {
        if (profile == null) {
            if (PROFILE == 2) {
                ...
                throw new RuntimeException(UNSUPPORTED_GLPROF_ERROR);
            if (2 < PROFILE) {
        ...
        if (WINDOW_TOOLKIT == AWT) {
            ...
            if (RETINA) {
                canvasAWT.setSurfaceScale(new int[] { ScalableSurface.AUTOMAX_PIXELSCALE, ScalableSurface.AUTOMAX_PIXELSCALE });
                retf = 2;
            } else {
                canvasAWT.setSurfaceScale(new int[] { ScalableSurface.IDENTITY_PIXELSCALE, ScalableSurface.IDENTITY_PIXELSCALE });
            }
    ...
    protected void endDraw(boolean clear0) {
        if (isFBOBacked()) {
            if (USE_JOGL_FBOLAYER) {
                if (!clear0 && isFBOBacked() && !isMultisampled() && frontFBO != null && backFBO != null) {
                    frontFBO.bind(gl);
                    gl.glDisable(GL.GL_BLEND);
                    drawTexture(TEXTURE_2D, backTexAttach.getName(), backTexAttach.getWidth(), backTexAttach.getHeight(), pg.width, pg.height, 0, 0, pg.width, pg.height, 0, 0, pg.width, pg.height);
                    backFBO.bind(gl);
                }
            } else {
    ...
    protected void nativeKeyEvent(com.jogamp.newt.event.KeyEvent nativeEvent, int peAction) {
        ...
        KeyEvent ke = new KeyEvent(nativeEvent, nativeEvent.getWhen(), peAction, peModifiers, keyChar, nativeEvent.getKeyCode());
