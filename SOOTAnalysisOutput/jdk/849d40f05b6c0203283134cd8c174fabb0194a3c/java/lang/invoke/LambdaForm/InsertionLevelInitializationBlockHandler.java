...
class LambdaForm {
    ...
    LambdaForm(String debugName, int arity, Name[] names, int result) {
        this(debugName, arity, names, result, true, null, Kind.GENERIC);
    }

    LambdaForm(String debugName, int arity, Name[] names, int result, Kind kind) {
        this(debugName, arity, names, result, true, null, kind);
    }
    ...
    LambdaForm(String debugName, int arity, Name[] names, int result, boolean forceInline, MethodHandle customized, Kind kind) {
    ...
    LambdaForm(String debugName, int arity, Name[] names) {
        this(debugName, arity, names, LAST_RESULT, true, null, Kind.GENERIC);
    }

    LambdaForm(String debugName, int arity, Name[] names, Kind kind) {
        this(debugName, arity, names, LAST_RESULT, true, null, kind);
    }

    LambdaForm(String debugName, int arity, Name[] names, boolean forceInline) {
        this(debugName, arity, names, LAST_RESULT, forceInline, null, Kind.GENERIC);
    }

    LambdaForm(String debugName, int arity, Name[] names, boolean forceInline, Kind kind) {
        this(debugName, arity, names, LAST_RESULT, forceInline, null, kind);
    }
    ...
    private static synchronized void createFormsFor(BasicType type) {
        ...
        if (isVoid) {
            ...
            zeFun = new NamedFunction(zeMem, MethodHandleImpl.makeIntrinsic(zeMem.getInvocationType(), zeForm, MethodHandleImpl.Intrinsic.ZERO));
        }
        LF_zero[ord] = zeForm;
        NF_zero[ord] = zeFun;
