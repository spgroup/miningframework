...
public class XSelection {
    ...
    void getTargetsDelayed() {
        ...
        try {
            ...
            if (isSelectionNotifyProcessed || curTime >= convertSelectionTime + SELECTION_TIMEOUT) {
                ...
                XlibWrapper.XConvertSelection(XToolkit.getDisplay(), getSelectionAtom().getAtom(), XDataTransferer.TARGETS_ATOM.getAtom(), targetsPropertyAtom.getAtom(), XWindow.getXAWTRootWindow().getWindow(), XlibWrapper.CurrentTime);
                isSelectionNotifyProcessed = false;
            }
        } finally {
            XToolkit.awtUnlock();
    ...
    private static void checkChange(XSelectionEvent xse) {
        ...
        if (propertyAtom == XlibWrapper.None) {
            ...
            WindowPropertyGetter targetsGetter = new WindowPropertyGetter(XWindow.getXAWTRootWindow().getWindow(), XAtom.get(propertyAtom), 0, MAX_LENGTH, true, XlibWrapper.AnyPropertyType);
            try {
