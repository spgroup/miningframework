...
public class GarbageCollectWriteAheadLogsTest {
    ...
    public void testMapServersToFiles() {
        ...
        fileToServerMap.put(path1, "server1");
        Path path2 = new Path(DIR_1_PATH, UUID2);
        fileToServerMap.put(path2, "");
        Path path3 = new Path(new Path(DIR_3_PATH, "server3"), UUID3);
        fileToServerMap.put(path3, "server3");
    ...
    public void replicationEntriesAffectGC() throws Exception {
        ...
        assertTrue(replGC.neededByReplication(conn, "/wals/" + file1));
        replData.clear();
        replData.add(Maps.immutableEntry(new Key("/wals/" + file1, StatusSection.NAME.toString(), "1"), ProtobufUtil.toValue(Status.newBuilder().setInfiniteEnd(true).setBegin(Long.MAX_VALUE).setClosed(true).build())));
