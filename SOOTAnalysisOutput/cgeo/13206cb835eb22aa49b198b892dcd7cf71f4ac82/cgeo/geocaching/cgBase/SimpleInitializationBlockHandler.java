...
public class cgBase {
    ...
    private final static Pattern PATTERN_TRACKABLE_DETAILSIMAGE = Pattern.compile("<h3>\\W*About This Item[^<]*</h3>[^<]*<div id=\"TrackableDetails\">([^<]*<p>([^<]*<img id=\"ctl00_ContentBody_BugDetails_BugImage\" class=\"[^\"]+\" src=\"([^\"]+)\"[^>]*>)?[^<]*</p>)?[^<]*<p[^>]*>(.*)</p>[^<]*</div>");<<<<<<< MINE
=======
private final static Pattern PATTERN_TRACKABLE_Goal = Pattern.compile("<h3>\\W*Current GOAL[^<]*</h3>[^<]*<div id=\"TrackableGoal\">[^<]*<p>(.*?)</p>[^<]*</div>[^<]*<h3>");
>>>>>>> YOURS
<<<<<<< MINE
        ...
        if (caches != null) {
            ...
            if (CollectionUtils.isNotEmpty(caches.cacheList)) {
                for (final cgCache cache : caches.cacheList) {
                    final boolean excludeCache = (excludeDisabled && cache.disabled) || (excludeMine && (cache.own || cache.found)) || (cacheType != null && !cacheType.equals(cache.type));
                    if (!excludeCache) {
