...
public final class TransformTranslator {
    ...
    private static <K, V> TransformEvaluator<GroupByKeyOnly<K, V>> gbk() {
        return new TransformEvaluator<GroupByKeyOnly<K, V>>() {
            ...
            public void evaluate(GroupByKeyOnly<K, V> transform, EvaluationContext context) {
                @SuppressWarnings("unchecked")
                JavaRDDLike<WindowedValue<KV<K, V>>, ?> inRDD = (JavaRDDLike<WindowedValue<KV<K, V>>, ?>) context.getInputRDD(transform);
                @SuppressWarnings("unchecked")
                ...
                JavaRDDLike<WindowedValue<KV<K, Iterable<V>>>, ?> outRDD = fromPair(toPair(inRDD.map(WindowingHelpers.<KV<K, V>>unwindowFunction())).mapToPair(CoderHelpers.toByteFunction(keyCoder, valueCoder)).groupByKey().mapToPair(CoderHelpers.fromByteFunctionIterable(keyCoder, valueCoder))).map(WindowingHelpers.<KV<K, Iterable<V>>>windowFunction());
    ...
    private static <K, VI, VO> TransformEvaluator<Combine.GroupedValues<K, VI, VO>> grouped() {
        return new TransformEvaluator<Combine.GroupedValues<K, VI, VO>>() {
            ...
            public void evaluate(Combine.GroupedValues<K, VI, VO> transform, EvaluationContext context) {
                ...
                @SuppressWarnings("unchecked")
                JavaRDDLike<WindowedValue<KV<K, Iterable<VI>>>, ?> inRDD = (JavaRDDLike<WindowedValue<KV<K, Iterable<VI>>>, ?>) context.getInputRDD(transform);
    ...
    private static <I, A, O> TransformEvaluator<Combine.Globally<I, O>> combineGlobally() {
        return new TransformEvaluator<Combine.Globally<I, O>>() {
            ...
            public void evaluate(Combine.Globally<I, O> transform, EvaluationContext context) {
                ...
                JavaRDD<byte[]> outRdd = context.getSparkContext().parallelize(CoderHelpers.toByteArrays(Collections.singleton(output), coder));
    ...
    private static <K, VI, VA, VO> TransformEvaluator<Combine.PerKey<K, VI, VO>> combinePerKey() {
        return new TransformEvaluator<Combine.PerKey<K, VI, VO>>() {
            ...
            public void evaluate(Combine.PerKey<K, VI, VO> transform, EvaluationContext context) {
                ...
                @SuppressWarnings("unchecked")
                JavaRDDLike<WindowedValue<KV<K, VI>>, ?> inRdd = (JavaRDDLike<WindowedValue<KV<K, VI>>, ?>) context.getInputRDD(transform);
                @SuppressWarnings("unchecked")
                ...
                JavaPairRDD<WindowedValue<K>, WindowedValue<KV<K, VI>>> inRddDuplicatedKeyPair = inRdd.flatMapToPair(new PairFlatMapFunction<WindowedValue<KV<K, VI>>, WindowedValue<K>, WindowedValue<KV<K, VI>>>() {

                    @Override
                    public Iterable<Tuple2<WindowedValue<K>, WindowedValue<KV<K, VI>>>> call(WindowedValue<KV<K, VI>> kv) {
                        List<Tuple2<WindowedValue<K>, WindowedValue<KV<K, VI>>>> tuple2s = Lists.newArrayListWithCapacity(kv.getWindows().size());
                        for (BoundedWindow boundedWindow : kv.getWindows()) {
                            WindowedValue<K> wk = WindowedValue.of(kv.getValue().getKey(), boundedWindow.maxTimestamp(), boundedWindow, kv.getPane());
                            tuple2s.add(new Tuple2<>(wk, kv));
                        }
                        return tuple2s;
                    }
                });
                final WindowedValue.FullWindowedValueCoder<K> wkCoder = WindowedValue.FullWindowedValueCoder.of(keyCoder, context.getInput(transform).getWindowingStrategy().getWindowFn().windowCoder());
                final WindowedValue.FullWindowedValueCoder<KV<K, VI>> wkviCoder = WindowedValue.FullWindowedValueCoder.of(kviCoder, context.getInput(transform).getWindowingStrategy().getWindowFn().windowCoder());
                final WindowedValue.FullWindowedValueCoder<KV<K, VA>> wkvaCoder = WindowedValue.FullWindowedValueCoder.of(kvaCoder, context.getInput(transform).getWindowingStrategy().getWindowFn().windowCoder());
                JavaPairRDD<ByteArray, byte[]> inRddDuplicatedKeyPairBytes = inRddDuplicatedKeyPair.mapToPair(CoderHelpers.toByteFunction(wkCoder, wkviCoder));
                JavaPairRDD<ByteArray, byte[]> accumulatedBytes = inRddDuplicatedKeyPairBytes.combineByKey(new Function<byte[], byte[]>() {
                    ...
                    public byte[] call(byte[] input) {
                        ...
                        WindowedValue<KV<K, VA>> wkva = WindowedValue.of(KV.of(wkvi.getValue().getKey(), va), wkvi.getTimestamp(), wkvi.getWindows(), wkvi.getPane());
                    ...
                    public byte[] call(byte[] acc, byte[] input) {
                        ...
                        WindowedValue<KV<K, VI>> wkvi = CoderHelpers.fromByteArray(input, wkviCoder);
                        VA va = keyed.addInput(wkva.getValue().getKey(), wkva.getValue().getValue(), wkvi.getValue().getValue());
                        wkva = WindowedValue.of(KV.of(wkva.getValue().getKey(), va), wkva.getTimestamp(), wkva.getWindows(), wkva.getPane());
                        return CoderHelpers.toByteArray(wkva, wkvaCoder);
                    }
                }, new Function2<byte[], byte[], byte[]>() {
                    ...
                    public byte[] call(byte[] acc1, byte[] acc2) {
                        ...
                        VA va = keyed.mergeAccumulators(wkva1.getValue().getKey(), Collections.unmodifiableList(Arrays.asList(wkva1.getValue().getValue(), wkva2.getValue().getValue())));
                        WindowedValue<KV<K, VA>> wkva = WindowedValue.of(KV.of(wkva1.getValue().getKey(), va), wkva1.getTimestamp(), wkva1.getWindows(), wkva1.getPane());
                        return CoderHelpers.toByteArray(wkva, wkvaCoder);
                    }
                });
                JavaPairRDD<WindowedValue<K>, WindowedValue<VO>> extracted = accumulatedBytes.mapToPair(CoderHelpers.fromByteFunction(wkCoder, wkvaCoder)).mapValues(new Function<WindowedValue<KV<K, VA>>, WindowedValue<VO>>() {

                    @Override
                    public WindowedValue<VO> call(WindowedValue<KV<K, VA>> acc) {
                        return WindowedValue.of(keyed.extractOutput(acc.getValue().getKey(), acc.getValue().getValue()), acc.getTimestamp(), acc.getWindows(), acc.getPane());
                    }
                });
                context.setOutputRDD(transform, fromPair(extracted).map(new Function<KV<WindowedValue<K>, WindowedValue<VO>>, WindowedValue<KV<K, VO>>>() {

                    @Override
                    public WindowedValue<KV<K, VO>> call(KV<WindowedValue<K>, WindowedValue<VO>> kwvo) throws Exception {
    ...
    private static final class KVFunction<K, VI, VO> implements Function<WindowedValue<KV<K, Iterable<VI>>>, WindowedValue<KV<K, VO>>> {
        ...
        @Override
        public WindowedValue<KV<K, VO>> call(WindowedValue<KV<K, Iterable<VI>>> windowedKv) throws Exception {
            KV<K, Iterable<VI>> kv = windowedKv.getValue();
            return WindowedValue.of(KV.of(kv.getKey(), keyed.apply(kv.getKey(), kv.getValue())), windowedKv.getTimestamp(), windowedKv.getWindows(), windowedKv.getPane());
        }
    }
    ...
    private static <I, O> TransformEvaluator<ParDo.Bound<I, O>> parDo() {
        return new TransformEvaluator<ParDo.Bound<I, O>>() {
            ...
            public void evaluate(ParDo.Bound<I, O> transform, EvaluationContext context) {
                DoFnFunction<I, O> dofn = new DoFnFunction<>(transform.getFn(), context.getRuntimeContext(), getSideInputs(transform.getSideInputs(), context));
                @SuppressWarnings("unchecked")
                JavaRDDLike<WindowedValue<I>, ?> inRDD = (JavaRDDLike<WindowedValue<I>, ?>) context.getInputRDD(transform);
        ...
        return new TransformEvaluator<ParDo.BoundMulti<I, O>>() {
            ...
            public void evaluate(ParDo.BoundMulti<I, O> transform, EvaluationContext context) {
                ...
                for (Map.Entry<TupleTag<?>, PCollection<?>> e : pct.getAll().entrySet()) {
                    @SuppressWarnings("unchecked")
                    JavaPairRDD<TupleTag<?>, WindowedValue<?>> filtered = all.filter(new TupleTagFilter(e.getKey()));
                    @SuppressWarnings("unchecked")
    ...
    private static <T> TransformEvaluator<TextIO.Read.Bound<T>> readText() {
        return new TransformEvaluator<TextIO.Read.Bound<T>>() {
            ...
            public void evaluate(TextIO.Read.Bound<T> transform, EvaluationContext context) {
                ...
                JavaRDD<WindowedValue<String>> rdd = context.getSparkContext().textFile(pattern).map(WindowingHelpers.<String>windowFunction());
                context.setOutputRDD(transform, rdd);
            }
        };
    }
    ...
    private static <T> TransformEvaluator<TextIO.Write.Bound<T>> writeText() {
        return new TransformEvaluator<TextIO.Write.Bound<T>>() {
            ...
            public void evaluate(TextIO.Write.Bound<T> transform, EvaluationContext context) {
                @SuppressWarnings("unchecked")
                JavaPairRDD<T, Void> last = ((JavaRDDLike<WindowedValue<T>, ?>) context.getInputRDD(transform)).map(WindowingHelpers.<T>unwindowFunction()).mapToPair(new PairFunction<T, T, Void>() {
    ...
    private static <T> TransformEvaluator<AvroIO.Read.Bound<T>> readAvro() {
        return new TransformEvaluator<AvroIO.Read.Bound<T>>() {
            ...
            public void evaluate(AvroIO.Read.Bound<T> transform, EvaluationContext context) {
                ...
                @SuppressWarnings("unchecked")
                JavaRDD<AvroKey<T>> avroFile = (JavaRDD<AvroKey<T>>) (JavaRDD<?>) jsc.newAPIHadoopFile(pattern, AvroKeyInputFormat.class, AvroKey.class, NullWritable.class, new Configuration()).keys();
    ...
    private static <T> TransformEvaluator<AvroIO.Write.Bound<T>> writeAvro() {
        return new TransformEvaluator<AvroIO.Write.Bound<T>>() {
            ...
            public void evaluate(AvroIO.Write.Bound<T> transform, EvaluationContext context) {
                ...
                @SuppressWarnings("unchecked")
                JavaPairRDD<AvroKey<T>, NullWritable> last = ((JavaRDDLike<WindowedValue<T>, ?>) context.getInputRDD(transform)).map(WindowingHelpers.<T>unwindowFunction()).mapToPair(new PairFunction<T, AvroKey<T>, NullWritable>() {
    ...
    private static <K, V> TransformEvaluator<HadoopIO.Read.Bound<K, V>> readHadoop() {
        return new TransformEvaluator<HadoopIO.Read.Bound<K, V>>() {
            ...
            public void evaluate(HadoopIO.Read.Bound<K, V> transform, EvaluationContext context) {
                ...
                JavaSparkContext jsc = context.getSparkContext();
                @SuppressWarnings("unchecked")
                JavaPairRDD<K, V> file = jsc.newAPIHadoopFile(pattern, transform.getFormatClass(), transform.getKeyClass(), transform.getValueClass(), new Configuration());
    ...
    private static <K, V> TransformEvaluator<HadoopIO.Write.Bound<K, V>> writeHadoop() {
        return new TransformEvaluator<HadoopIO.Write.Bound<K, V>>() {
            ...
            public void evaluate(HadoopIO.Write.Bound<K, V> transform, EvaluationContext context) {
                @SuppressWarnings("unchecked")
                JavaPairRDD<K, V> last = ((JavaRDDLike<WindowedValue<KV<K, V>>, ?>) context.getInputRDD(transform)).map(WindowingHelpers.<KV<K, V>>unwindowFunction()).mapToPair(new PairFunction<KV<K, V>, K, V>() {
    ...
    private static <T, W extends BoundedWindow> TransformEvaluator<Window.Bound<T>> window() {
        return new TransformEvaluator<Window.Bound<T>>() {
            ...
            public void evaluate(Window.Bound<T> transform, EvaluationContext context) {
                @SuppressWarnings("unchecked")
                JavaRDDLike<WindowedValue<T>, ?> inRDD = (JavaRDDLike<WindowedValue<T>, ?>) context.getInputRDD(transform);
                ...
                if (windowFn instanceof GlobalWindows) {
                    ...
                    DoFnFunction<T, T> dofn = new DoFnFunction<>(addWindowsDoFn, context.getRuntimeContext(), null);
                    context.setOutputRDD(transform, inRDD.mapPartitions(dofn));
                }
            }
        };
    ...
    private static <T> TransformEvaluator<View.AsSingleton<T>> viewAsSingleton() {
        return new TransformEvaluator<View.AsSingleton<T>>() {
            ...
            public void evaluate(View.AsSingleton<T> transform, EvaluationContext context) {
                Iterable<? extends WindowedValue<?>> iter = context.getWindowedValues(context.getInput(transform));
    ...
    private static final class TupleTagFilter<V> implements Function<Tuple2<TupleTag<V>, WindowedValue<?>>, Boolean> {
    ...
    private static Map<TupleTag<?>, BroadcastHelper<?>> getSideInputs(List<PCollectionView<?>> views, EvaluationContext context) {
        if (views == null) {
            ...
            for (PCollectionView<?> view : views) {
                ...
                @SuppressWarnings("unchecked")
                BroadcastHelper<?> helper = BroadcastHelper.create((Iterable<WindowedValue<?>>) collectionView, coderInternal);
