...
public final class ServerCommands {
    ...
    @Command(label = "sendmessage", aliases = { "sendmsg", "msg" }, usage = "Usage: sendmessage <player> <message>")
...
>>>>>>> YOURS
        ...
        public void execute(GenshinPlayer player, List<String> args) {
            if (args.size() < 2) {
                CommandHandler.sendMessage(player, "Usage: sendmessage <player> <message>");
                return;
            }
            try {
                int target = Integer.parseInt(args.get(0));
...
>>>>>>> YOURS
    ...
    @Command(label = "account", usage = "Usage: account <create|delete> <username> [uid]", execution = Command.Execution.CONSOLE)
    ...
    @Command(label = "permission", usage = "Usage: permission <add|remove> <username> <permission>", execution = Command.Execution.CONSOLE)
            ...
            switch(action) {
                ...
                case "add":
                    if (account.addPermission(permission)) {
                        ...
                        CommandHandler.sendMessage(null, "They already have this permission!");
                    break;
                case "remove":
                    if (account.removePermission(permission)) {
                        CommandHandler.sendMessage(null, "Permission removed.");
                    } else
                        CommandHandler.sendMessage(null, "They don't have this permission!");
                    break;
            }
            account.save();
        }
    ...
    @Command(label = "help", usage = "Usage: help [command]")
    public static class HelpCommand implements CommandHandler {

        @Override
        public void execute(List<String> args) {
            List<CommandHandler> handlers = CommandMap.getInstance().getHandlers();
            List<Command> annotations = handlers.stream().map(handler -> handler.getClass().getAnnotation(Command.class)).collect(Collectors.toList());
            if (args.size() < 1) {
                ...
                if (handler == null) {
                    CommandHandler.sendMessage(null, "Command not found.");
                    return;
                }
        ...
        public void execute(GenshinPlayer player, List<String> args) {
            ...
            if (args.size() < 1) {
                ...
                CommandHandler handler = CommandMap.getInstance().getHandler(command);
                if (handler == null) {
                    CommandHandler.sendMessage(player, "Command not found.");
                    return;
                }
                Command annotation = handler.getClass().getAnnotation(Command.class);
