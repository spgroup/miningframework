...
public class RestartStressIT extends AccumuloClusterIT {
    ...
    public void test() throws Exception {
        ...
        Future<Integer> retCode = svc.submit(new Callable<Integer>() {
            ...
            public Integer call() {
                try {
                    return control.exec(TestIngest.class, new String[] { "-u", "root", "-p", new String(token.getPassword(), Charsets.UTF_8), "-i", cluster.getInstanceName(), "-z", cluster.getZooKeepers(), "--rows", "" + IOPTS.rows, "--table", tableName });
                } catch (Exception e) {
