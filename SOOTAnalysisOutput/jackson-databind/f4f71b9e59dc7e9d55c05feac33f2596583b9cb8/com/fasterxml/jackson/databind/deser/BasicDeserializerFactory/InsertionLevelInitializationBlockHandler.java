    ...
    protected <T extends JavaType> T modifyTypeByAnnotation(DeserializationContext ctxt, Annotated a, T type) throws JsonMappingException {
        ...
        if (subclass != null) {
            try {
                ...
                throw JsonMappingException.from(ctxt.getParser(), "Failed to narrow type " + type + " with concrete-type annotation (value " + subclass.getName() + "), method '" + a.getName() + "': " + iae.getMessage(), iae);
            }
        }
        if (type.isContainerType()) {
            ...
            if (keyClass != null) {
                ...
                try {
                    ...
                    throw JsonMappingException.from(ctxt.getParser(), "Failed to narrow key type " + type + " with key-type annotation (" + keyClass.getName() + "): " + iae.getMessage(), iae);
                }
            }
            JavaType keyType = type.getKeyType();
            ...
            if (cc != null) {
                try {
                    ...
                    throw JsonMappingException.from(ctxt.getParser(), "Failed to narrow content type " + type + " with content-type annotation (" + cc.getName() + "): " + iae.getMessage(), iae);
                }
            }
            JavaType contentType = type.getContentType();
