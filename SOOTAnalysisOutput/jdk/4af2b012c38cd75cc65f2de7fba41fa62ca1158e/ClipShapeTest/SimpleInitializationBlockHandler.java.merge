...
public final class ClipShapeTest {
    ...
    static final boolean SHOW_DETAILS = false;
    ...
    public static void main(String[] args) {
        ...
        switch(SHAPE_MODE) {
            ...
            default:
                THRESHOLD_DELTA = 2;
                THRESHOLD_NBPIX = (USE_DASHES) ? ((isMarlinFloat.get()) ? 30 : 6) : (isMarlinFloat.get()) ? 10 : 0;
        }
        ...
        try {
            if (TEST_STROKER) {
                final float[][] dashArrays = (USE_DASHES) ? new float[][] { new float[] { 13f, 7f } } : new float[][] { null };
                System.out.println("dashes: " + Arrays.deepToString(dashArrays));
                final float[] strokeWidths = (USE_VAR_STROKE) ? new float[5] : new float[] { 10f };
                int nsw = 0;
                if (USE_VAR_STROKE) {
    ...
    private static void paintShape(final Shape p2d, final Graphics2D g2d, final boolean fill, final boolean clip) {
        reset(g2d);
        setClip(g2d, clip);
        if (fill) {
            g2d.fill(p2d);
    ...
    private static Graphics2D initialize(final BufferedImage img, final Stroke s) {
    ...
    static void subdivide(final Path2D p2d, final int type, final double[] in) {
        ...
        double curveLen = ((type == 8) ? curvelen(in[0], in[1], in[2], in[3], in[4], in[5], in[6], in[7]) : quadlen(in[0], in[1], in[2], in[3], in[4], in[5]));
    ...
    private static void paintShapeDetails(final Graphics2D g2d, final Shape shape) {
        ...
        for (final PathIterator it = shape.getPathIterator(null); !it.isDone(); it.next()) {
            ...
            if (SHOW_POINTS) {
                ELL_POINT.setFrame(px - POINT_RADIUS, py - POINT_RADIUS, POINT_RADIUS * 2f, POINT_RADIUS * 2f);
                g2d.fill(ELL_POINT);
            }
        }
        if (SHOW_INFO) {
    ...
    private final static class TestSetup {
        ...
        TestSetup(ShapeMode shapeMode, final boolean closed, final float strokeWidth, final int strokeCap, final int strokeJoin, final float[] dashes) {
        ...
        public String toString() {
            if (isStroke()) {
                return "TestSetup{id=" + id + ", shapeMode=" + shapeMode + ", closed=" + closed + ", strokeWidth=" + strokeWidth + ", strokeCap=" + getCap(strokeCap) + ", strokeJoin=" + getJoin(strokeJoin) + ((dashes != null) ? ", dashes: " + Arrays.toString(dashes) : "") + '}';
            }
            return "TestSetup{id=" + id + ", shapeMode=" + shapeMode + ", closed=" + closed + ", fill" + ", windingRule=" + getWindingRule(windingRule) + '}';
        }
    ...
    public static BufferedImage computeDiffImage(final DiffContext testCtx, final DiffContext testThCtx, final BufferedImage tstImage, final BufferedImage refImage, final BufferedImage diffImage) {
    ...
    static double linelen(final double x0, final double y0, final double x1, final double y1) {
        final double dx = x1 - x0;
        final double dy = y1 - y0;
        return Math.sqrt(dx * dx + dy * dy);
    }

    static double quadlen(final double x0, final double y0, final double x1, final double y1, final double x2, final double y2) {
        return (linelen(x0, y0, x1, y1) + linelen(x1, y1, x2, y2) + linelen(x0, y0, x2, y2)) / 2.0d;
    }

    static double curvelen(final double x0, final double y0, final double x1, final double y1, final double x2, final double y2, final double x3, final double y3) {
        return (linelen(x0, y0, x1, y1) + linelen(x1, y1, x2, y2) + linelen(x2, y2, x3, y3) + linelen(x0, y0, x3, y3)) / 2.0d;
    }
}