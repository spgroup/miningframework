...
public class ExtensionsWithLDAP {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Path hostsFilePath = Paths.get(System.getProperty("test.src", ".") + File.separator + extension);
        System.setProperty("jdk.net.hosts.file", hostsFilePath.toFile().getAbsolutePath());
        ...
        try (SocksProxy proxy = SocksProxy.startProxy(socketConsumer)) {
            CertPathValidator.getInstance("PKIX").validate(cp, new PKIXParameters(trustedCertsSet));
        ...
        if (!hosts.contains(targetHost)) {
            throw new RuntimeException(String.format("The %s from %s extension is not requested", targetHost, extension));
        }
