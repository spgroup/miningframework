...
public class KDC {
    ...
    private static void writeKtab0(String tab, boolean append, KDC... kdcs) throws IOException, KrbException {
        ...
        for (KDC kdc : kdcs) {
            for (String name : kdc.passwords.keySet()) {
                ...
                ktab.addEntry(new PrincipalName(name, name.indexOf('/') < 0 ? PrincipalName.KRB_NT_UNKNOWN : PrincipalName.KRB_NT_SRV_HST), pass, kvno, true);
            }
        }
        ktab.save();
    }
    ...
    private byte[] processTgsReq(byte[] in) throws Exception {
        ...
        try {
            ...
            EncTicketPart enc = new EncTicketPart(tFlags, key, etp.crealm, etp.cname, new TransitedEncoding(1, new byte[0]), new KerberosTime(new Date()), body.from, till, body.rtime, body.addresses != null ? body.addresses : new HostAddresses(new InetAddress[] { InetAddress.getLocalHost() }), null);
            ...
            EncTGSRepPart enc_part = new EncTGSRepPart(key, new LastReq(new LastReqEntry[] { new LastReqEntry(0, new KerberosTime(new Date().getTime() - 10000)) }), body.getNonce(), new KerberosTime(new Date().getTime() + 1000 * 3600 * 24), tFlags, new KerberosTime(new Date()), body.from, till, body.rtime, body.crealm, body.sname, body.addresses != null ? body.addresses : new HostAddresses(new InetAddress[] { InetAddress.getLocalHost() }));
