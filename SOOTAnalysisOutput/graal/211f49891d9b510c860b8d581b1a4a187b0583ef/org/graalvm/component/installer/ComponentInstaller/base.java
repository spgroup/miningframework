...
public class ComponentInstaller extends Launcher {
    ...
    int processOptions(LinkedList<String> cmdline) {
        ...
        RemoteCatalogDownloader downloader = new RemoteCatalogDownloader(input, feedback, catalogURL);
        ...
        CatalogFactory cFactory = (CommandInput in, ComponentRegistry lreg) -> {
            ...
            if (lreg == in.getLocalRegistry()) {
                ...
                nDownloader = new RemoteCatalogDownloader(in, env, downloader.getOverrideCatalogSpec());
            }
        ...
        downloader.setRemoteSourcesAllowed(builtinsImplied || env.hasOption(Commands.OPTION_CATALOG) || env.hasOption(Commands.OPTION_FOREIGN_CATALOG));
        return -1;
    }
    ...
    Path finddGraalHome() {
        String graalHome = input.getParameter("GRAAL_HOME", input.getParameter("GRAAL_HOME", false), true);
        Path graalPath = null;
        if (graalHome != null) {
