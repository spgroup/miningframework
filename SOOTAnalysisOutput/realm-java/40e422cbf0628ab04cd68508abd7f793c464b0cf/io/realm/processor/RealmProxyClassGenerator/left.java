...
public class RealmProxyClassGenerator {
    ...
    private void emitValidateTableMethod(JavaWriter writer) throws IOException {
        ...
        for (VariableElement field : metadata.getFields()) {
            ...
            if (JAVA_TO_REALM_TYPES.containsKey(fieldTypeCanonicalName)) {
                ...
                writer.endControlFlow();
                writer.beginControlFlow("if (!transaction.hasTable(\"%s%s\"))", Constants.TABLE_PREFIX, fieldTypeSimpleName);
                writer.emitStatement("throw new IllegalStateException(\"Missing table '%s%s' for column '%s'\")", Constants.TABLE_PREFIX, fieldTypeSimpleName, fieldName);
                writer.endControlFlow();
                ...
                writer.endControlFlow();
                writer.beginControlFlow("if (!transaction.hasTable(\"%s%s\"))", Constants.TABLE_PREFIX, genericType);
                writer.emitStatement("throw new IllegalStateException(\"Missing table '%s%s' for column '%s'\")", Constants.TABLE_PREFIX, genericType, fieldName);
                writer.endControlFlow();
    ...
    private void emitCopyOrUpdateMethod(JavaWriter writer) throws IOException {
        ...
        if (!metadata.hasPrimaryKey()) {
            ...
            writer.beginControlFlow("if (rowIndex != TableOrView.NO_MATCH)").emitStatement("realmObject = new %s()", Utils.getProxyClassName(className)).emitStatement("realmObject.realm = realm").emitStatement("realmObject.row = table.getRow(rowIndex)").emitStatement("cache.put(object, realmObject)").nextControlFlow("else").emitStatement("canUpdate = false").endControlFlow();
            writer.endControlFlow();
    ...
    private void emitCopyMethod(JavaWriter writer) throws IOException {
        writer.beginMethod(className, "copy", EnumSet.of(Modifier.PUBLIC, Modifier.STATIC), "Realm", "realm", className, "newObject", "boolean", "update", "Map<RealmObject,RealmObject>", "cache");
        if (metadata.hasPrimaryKey()) {
    ...
    private void emitUpdateMethod(JavaWriter writer) throws IOException {
        writer.beginMethod(className, "update", EnumSet.of(Modifier.STATIC), "Realm", "realm", className, "realmObject", className, "newObject", "Map<RealmObject, RealmObject>", "cache");
        for (VariableElement field : metadata.getFields()) {
            String fieldName = field.getSimpleName().toString();
            if (typeUtils.isAssignable(field.asType(), realmObject)) {
