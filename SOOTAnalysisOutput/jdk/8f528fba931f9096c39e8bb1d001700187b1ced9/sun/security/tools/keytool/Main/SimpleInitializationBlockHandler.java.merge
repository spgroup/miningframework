...
public final class Main {
    ...
    enum Command {
        ...
        CERTREQ("Generates.a.certificate.request", ALIAS, SIGALG, FILEOUT, KEYPASS, KEYSTORE, DNAME, EXT, STOREPASS, STORETYPE, PROVIDERNAME, ADDPROVIDER, PROVIDERCLASS, PROVIDERPATH, V, PROTECTED),
        ...
        SELFCERT("Generates.a.self.signed.certificate", ALIAS, SIGALG, DNAME, STARTDATE, EXT, VALIDITY, KEYPASS, STORETYPE, KEYSTORE, STOREPASS, PROVIDERNAME, ADDPROVIDER, PROVIDERCLASS, PROVIDERPATH, V),
        GENCRL("Generates.CRL", RFC, FILEOUT, ID, ALIAS, SIGALG, KEYPASS, KEYSTORE, STOREPASS, STORETYPE, PROVIDERNAME, ADDPROVIDER, PROVIDERCLASS, PROVIDERPATH, V, PROTECTED),
    ...
    String[] parseArgs(String[] args) throws Exception {
                ...
                if (collator.compare(flags, "-conf") == 0) {
                    ...
                    if (c != null) {
                        if (command == null) {
                            ...
                            throw new Exception(String.format(rb.getString("multiple.commands.1.2"), command.name, c.name));
                        }
                    }
                }
            }
        ...
        for (i = 0; (i < args.length) && args[i].startsWith("-"); i++) {
            ...
            if (c != null) {
                command = c;
            } else if (collator.compare(flags, "--help") == 0 || collator.compare(flags, "-h") == 0 || collator.compare(flags, "-?") == 0 || collator.compare(flags, "-help") == 0) {
                help = true;
    ...
    void doCommands(PrintStream out) throws Exception {
        ...
        if (ksfile != null && ksStream != null && providerName == null && storetype == null && !inplaceImport) {
        ...
        if (isKeyStoreRelated(command) && !token && !nullStream && ksfname != null) {
            ...
            if (f.exists()) {
                ...
                if (realType.equalsIgnoreCase("JKS") || realType.equalsIgnoreCase("JCEKS")) {
                    ...
                    if (!allCerts) {
                        weakWarnings.add(String.format(rb.getString("jks.storetype.warning"), realType, ksfname));
                    }
                }
                if (inplaceImport) {
                    String realSourceStoreType = KeyStore.getInstance(new File(inplaceBackupName), srcstorePass).getType();
                    String format = realType.equalsIgnoreCase(realSourceStoreType) ? rb.getString("backup.keystore.warning") : rb.getString("migrate.keystore.warning");
                    weakWarnings.add(String.format(format, srcksfname, realSourceStoreType, inplaceBackupName, realType));
                }
            }
        }
    }
    ...
    boolean inplaceImportCheck() throws Exception {
        ...
        if (srcksfname != null) {
            ...
            if (srcksfile.getCanonicalFile().equals(new File(ksfname).getCanonicalFile())) {
                ...
                System.err.println(String.format(rb.getString("importing.keystore.status"), srcksfname, ksfname));
                return false;
            }
        } else {
    ...
    KeyStore loadSourceKeyStore() throws Exception {
        ...
        KeyStore store;
        try {
            if (srcksfile != null && is != null && srcProviderName == null && srcstoretype == null) {
    ...
    private int doImportKeyStoreSingle(KeyStore srckeystore, String alias) throws Exception {
        ...
        if (destKeyPass != null) {
            ...
            newPass = P12KEYSTORE.equalsIgnoreCase(storetype) ? storePass : objs.snd;
    ...
    private String withWeak(PublicKey key) {
        if (DISABLED_CHECK.permits(SIG_PRIMITIVE_SET, key)) {
            ...
            if (kLen >= 0) {
                return String.format(rb.getString("key.bit"), kLen, key.getAlgorithm());
            } else {
                return String.format(rb.getString("unknown.size.1"), key.getAlgorithm());
            }
        } else {
