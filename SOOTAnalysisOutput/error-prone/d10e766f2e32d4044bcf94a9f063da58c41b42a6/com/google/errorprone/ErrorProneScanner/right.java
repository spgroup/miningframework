...
public class ErrorProneScanner extends Scanner {
    ...
    private Set<Class<? extends Annotation>> customSuppressionAnnotations = new HashSet<>();
        ...
        for (String checkName : disabledChecks) {
            for (BugChecker checker : nameToChecker.get(checkName)) {
                if (!checker.isDisableable()) {
                    throw new InvalidCommandLineOptionException("error-prone check " + checker.getCanonicalName() + " may not be disabled");
                }
            }
        }
    }
    ...
    private final List<ConditionalExpressionTreeMatcher> conditionalExpressionMatchers = new ArrayList<>();
    ...
    private final List<ExpressionStatementTreeMatcher> expressionStatementMatchers = new ArrayList<>();
    ...
    private final List<UnionTypeTreeMatcher> unionTypeTreeMatchers = new ArrayList<>();
