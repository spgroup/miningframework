...
public class IEFontRender extends FontRenderer {
    ...
    static {
        unicodeReplacements.put((char) Integer.parseInt("260E", 16), new TexturedGlyph(new ResourceLocation(ImmersiveEngineering.MODID, "textures/gui/hud_elements.png"), .5f, .75f, .5625f, .8125f, 0, 7.99F, 0, 7.99F));
    }
    ...
    private float renderStringAtPosImpl(String text, float x, float y, int color, boolean isShadow) {
        ...
        for (int i = 0; i < text.length(); ++i) {
            ...
            if (currentChar == '\u00a7' && i + 1 < text.length()) {
                ...
                if (glyphLocation != null) {
                    ...
                    this.renderGlyph(texturedglyph, bold, italic, boldOffset, x + shadowOffset, y + shadowOffset, bufferbuilder, currentRed, currentGreen, currentBlue, alpha, currentChar);
                }
                float advance = getCharWidthIE(currentChar, bold);
                float shadowOffset = isShadow ? 1.0F : 0.0F;
                if (strikethrough)
                    lineSegments.add(new Entry(x + shadowOffset - 1.0F, y + shadowOffset + 4.5F, x + shadowOffset + advance, y + shadowOffset + 4.5F - 1.0F, currentRed, currentGreen, currentBlue, alpha));
                if (underline)
                    lineSegments.add(new Entry(x + shadowOffset - 1.0F, y + shadowOffset + 9.0F, x + shadowOffset + advance, y + shadowOffset + 9.0F - 1.0F, currentRed, currentGreen, currentBlue, alpha));
                charPositions.add(x);
                x += advance;
            }
        }
        ...
        if (!lineSegments.isEmpty()) {
            ...
            for (Entry entry : lineSegments) entry.pipe(bufferbuilder);
            tessellator.draw();
    ...
    protected void renderGlyph(TexturedGlyph glyph, boolean bold, boolean italic, float boldOffset, float x, float y, BufferBuilder bufferBuilder, float red, float green, float blue, float alpha, char orig) {
