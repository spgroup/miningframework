...
public class SmartLightingQuad extends BakedQuad {
    ...
    public SmartLightingQuad(int[] vertexDataIn, int tintIndexIn, Direction faceIn, TextureAtlasSprite spriteIn, VertexFormat format, BlockPos p) {
        ...
        ignoreLight = false;
        for (int i = 0; i < 4; i++) relativePos[i] = new int[] { (int) Math.floor(Float.intBitsToFloat(vertexDataIn[7 * i])), (int) Math.floor(Float.intBitsToFloat(vertexDataIn[7 * i + 1])), (int) Math.floor(Float.intBitsToFloat(vertexDataIn[7 * i + 2])) };
    }
    ...
    public void pipe(IVertexConsumer consumer) {
        ...
        for (int v = 0; v < 4; v++) for (int e = 0; e < count; e++) if (eMap[e] != itemCount) {
            if (format.getElement(e).getUsage() == Usage.UV && format.getElement(e).getType() == Type.SHORT) {
                int brightness;
