...
public class Launcher extends Activity implements View.OnClickListener, OnLongClickListener, LauncherModel.Callbacks, View.OnTouchListener, PageSwitchListener, LauncherProviderChangeListener, LauncherStateTransitionAnimation.Callbacks {
    ...
    @Thunk
    boolean mRotationEnabled = false;

    private boolean mScreenOrientationSettingReceiverRegistered = false;

    final private BroadcastReceiver mScreenOrientationSettingReceiver = new BroadcastReceiver() {

        @Thunk
        Runnable mUpdateOrientationRunnable = new Runnable() {
        ...
        public void onReceive(Context context, Intent intent) {
            mRotationEnabled = intent.getBooleanExtra(Utilities.SCREEN_ROTATION_SETTING_EXTRA, false);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!mRotationEnabled) {
            String updateOrientationBroadcastPermission = getResources().getString(R.string.receive_update_orientation_broadcasts_permission);
            registerReceiver(mScreenOrientationSettingReceiver, new IntentFilter(Utilities.SCREEN_ROTATION_SETTING_INTENT), updateOrientationBroadcastPermission, null);
    ...
    protected void onResume() {
        ...
        if (mOnResumeState == State.WORKSPACE) {
            ...
            showAppsView(false, false, !launchedFromApp);
    ...
    public void updateInteraction(Workspace.State fromState, Workspace.State toState) {
        ...
        if (Launcher.DISABLE_ALL_APPS_SEARCH_INTEGRATION) {
            ...
            fromStateWithOverlay = fromState != Workspace.State.NORMAL && fromState != Workspace.State.NORMAL_HIDDEN;
            toStateWithOverlay = toState != Workspace.State.NORMAL && toState != Workspace.State.NORMAL_HIDDEN;
        }
    ...
    public void showWorkspace(boolean animated) {
        showWorkspace(WorkspaceStateTransitionAnimation.SCROLL_TO_CURRENT_PAGE, animated, null, true);
    }

    public void showWorkspace(boolean animated, Runnable onCompleteRunnable) {
        showWorkspace(WorkspaceStateTransitionAnimation.SCROLL_TO_CURRENT_PAGE, animated, onCompleteRunnable, true);
    }
    ...
    void showWorkspace(int snapToPage, boolean animated, Runnable onCompleteRunnable, boolean notifyLauncherCallbacks) {
    ...
    @Thunk
    boolean isAllAppsSearchOverridden() {
        if (DISABLE_ALL_APPS_SEARCH_INTEGRATION) {
            return false;
        }
        if (mLauncherCallbacks != null) {
