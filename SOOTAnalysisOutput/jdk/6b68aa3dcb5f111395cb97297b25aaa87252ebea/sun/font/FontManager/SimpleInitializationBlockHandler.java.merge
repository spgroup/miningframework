...
public final class FontManager {
    ...
    public static Font2D createFont2D(File fontFile, int fontFormat, boolean isCopy) throws FontFormatException {
    ...
    private static synchronized void initFontConfigFonts(boolean includeFallbacks) {
    ...
    private static PhysicalFont registerFromFcInfo(FcCompFont fcInfo) {
        ...
        if (physFont != null) {
            if (isTTC) {
                Font2D f2d = findFont2D(fcInfo.firstFont.familyName, fcInfo.style, NO_FALLBACK);
        ...
        physFont = findJREDeferredFont(fcInfo.firstFont.familyName, fcInfo.style);
        if (physFont == null && deferredFontFiles.get(fontFile) != null) {
        ...
        if (physFont == null) {
            ...
            physFont = registerFontFile(fcInfo.firstFont.fontFile, null, fontFormat, true, fontRank);
        }
        return physFont;
    }
    ...
    private static CompositeFont getFontConfigFont(String name, int style) {
        ...
        if (logging) {
            logger.info("FC name=" + name + " style=" + style + " uses " + fcInfo.firstFont.familyName + " in file: " + fcInfo.firstFont.fontFile);
        }
