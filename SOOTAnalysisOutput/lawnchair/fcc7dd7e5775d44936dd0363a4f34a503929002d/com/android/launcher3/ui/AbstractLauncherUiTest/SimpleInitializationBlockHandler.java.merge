...
public abstract class AbstractLauncherUiTest {
    ...
    static {
        if (TestHelpers.isInLauncherProcess()) {
            StrictMode.VmPolicy.Builder builder = new StrictMode.VmPolicy.Builder().detectActivityLeaks().penaltyLog().penaltyListener(Runnable::run, violation -> {
                if (sStrictmodeDetectedActivityLeak == null) {
                    sStrictmodeDetectedActivityLeak = violation.toString() + ", " + dumpHprofData() + ".";
                }
            });
    ...
    public static void checkDetectedLeaks(LauncherInstrumentation launcher) {
        ...
        Wait.atMost(AbstractLauncherUiTest::getActivityLeakErrorMessage, () -> {
            launcher.getTotalPssKb();
            return MAIN_EXECUTOR.submit(() -> ACTIVITY_LEAK_TRACKER.noLeakedActivities()).get();
    ...
    private static String dumpHprofData() {
        try {
            final String fileName = getInstrumentation().getTargetContext().getFilesDir().getPath() + "/ActivityLeakHeapDump.hprof";
    ...
    protected AbstractLauncherUiTest() {
        ...
        if (TestHelpers.isInLauncherProcess()) {
            ...
            mLauncher.setSystemHealthSupplier(startTime -> TestCommandReceiver.callCommand(TestCommandReceiver.GET_SYSTEM_HEALTH_MESSAGE, startTime.toString()).getString("result"));
            mLauncher.setOnSettledStateAction(containerType -> executeOnLauncher(launcher -> checkLauncherIntegrity(launcher, containerType)));
        }
    ...
    @Rule
    public ShellCommandRule mDisableHeadsUpNotification = ShellCommandRule.disableHeadsUpNotification();
    ...
    protected void clearPackageData(String pkg) throws IOException, InterruptedException {
        ...
        mTargetContext.registerReceiver(broadcastReceiver, PackageManagerHelper.getPackageFilter(pkg, Intent.ACTION_PACKAGE_RESTARTED, Intent.ACTION_PACKAGE_DATA_CLEARED));
    ...
    protected TestRule getRulesInsideActivityMonitor() {
        final RuleChain inner = RuleChain.outerRule(new PortraitLandscapeRunner(this)).around(new FailureWatcher(mDevice));
        return TestHelpers.isInLauncherProcess() ? RuleChain.outerRule(ShellCommandRule.setDefaultLauncher()).around(inner) : inner;
    }

    @Rule
    public TestRule mOrderSensitiveRules = RuleChain.outerRule(new FailureRewriterRule()).around(new TestStabilityRule()).around(mActivityMonitor).around(getRulesInsideActivityMonitor());
    ...
    public void verifyLauncherState() {
        ...
        if (mLauncherPid != 0) {
            assertEquals("Launcher crashed, pid mismatch:", mLauncherPid, mLauncher.getPid().intValue());
        }
    ...
    public void clearHomescreen() throws Throwable {
        ...
        LauncherSettings.Settings.call(mTargetContext.getContentResolver(), LauncherSettings.Settings.METHOD_CLEAR_EMPTY_DB_FLAG);
        resetLoaderState();
    }

    protected void resetLoaderState() {
        try {
            mMainThreadExecutor.execute(() -> LauncherAppState.getInstance(mTargetContext).getModel().forceReload());
        } catch (Throwable t) {
    ...
    protected void waitForState(String message, Supplier<LauncherState> state) {
        waitForLauncherCondition(message, launcher -> launcher.getStateManager().getCurrentStableState() == state.get());
    }
    ...
    protected void waitForLauncherCondition(String message, Function<Launcher, Boolean> condition, long timeout) {
        if (!TestHelpers.isInLauncherProcess())
            return;
    ...
    protected void waitForLauncherCondition(String message, Runnable testThreadAction, Function<Launcher, Boolean> condition, long timeout) {
        if (!TestHelpers.isInLauncherProcess())
            return;
        Wait.atMost(message, () -> {
    ...
    protected LauncherActivityInfo getSettingsApp() {
        return mTargetContext.getSystemService(LauncherApps.class).getActivityList("com.android.settings", Process.myUserHandle()).get(0);
    }
    ...
    public static void startAppFast(String packageName) {
        startIntent(getInstrumentation().getContext().getPackageManager().getLaunchIntentForPackage(packageName), By.pkg(packageName).depth(0), true);
    }

    public static void startTestActivity(int activityNumber) {
        final String packageName = getAppPackageName();
        final Intent intent = getInstrumentation().getContext().getPackageManager().getLaunchIntentForPackage(packageName);
        intent.setComponent(new ComponentName(packageName, "com.android.launcher3.tests.Activity" + activityNumber));
        startIntent(intent, By.pkg(packageName).text("TestActivity" + activityNumber), false);
    }
    ...
    private static void startIntent(Intent intent, BySelector selector, boolean newTask) {
        ...
        assertTrue("App didn't start: " + selector, UiDevice.getInstance(getInstrumentation()).wait(Until.hasObject(selector), DEFAULT_UI_TIMEOUT));
    }

    public static ActivityInfo resolveSystemAppInfo(String category) {
        return getInstrumentation().getContext().getPackageManager().resolveActivity(new Intent(Intent.ACTION_MAIN).addCategory(category), PackageManager.MATCH_SYSTEM_ONLY).activityInfo;
    }
    ...
    protected void closeLauncherActivity() {
        ...
        waitForLauncherCondition("Launcher still active", launcher -> launcher == null, DEFAULT_UI_TIMEOUT);
    }
