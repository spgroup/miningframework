...
public class FlinkStreamingTransformTranslators {
    ...
    private static class CreateStreamingTranslator<OUT> implements FlinkStreamingPipelineTranslator.StreamTransformTranslator<Create.Values<OUT>> {
        ...
        public void translateNode(Create.Values<OUT> transform, FlinkStreamingTranslationContext context) {
            ...
            FlinkStreamingCreateFunction<Integer, OUT> createFunction = new FlinkStreamingCreateFunction<>(serializedElements, elementCoder);
            ...
            DataStream<WindowedValue<OUT>> outputDataStream = initDataSet.flatMap(createFunction).returns(outputType);
    ...
    private static class BoundedReadSourceTranslator<T> implements FlinkStreamingPipelineTranslator.StreamTransformTranslator<Read.Bounded<T>> {
        ...
        public void translateNode(Read.Bounded<T> transform, FlinkStreamingTranslationContext context) {
            ...
            DataStream<WindowedValue<T>> source = context.getExecutionEnvironment().createInput(new SourceInputFormat<>(boundedSource, context.getPipelineOptions()), typeInfo);
    ...
    private static class ParDoBoundStreamingTranslator<IN, OUT> implements FlinkStreamingPipelineTranslator.StreamTransformTranslator<ParDo.Bound<IN, OUT>> {
        ...
        public void translateNode(ParDo.Bound<IN, OUT> transform, FlinkStreamingTranslationContext context) {
            PCollection<OUT> output = context.getOutput(transform);
            final WindowingStrategy<OUT, ? extends BoundedWindow> windowingStrategy = (WindowingStrategy<OUT, ? extends BoundedWindow>) context.getOutput(transform).getWindowingStrategy();
            ...
            SingleOutputStreamOperator<WindowedValue<OUT>> outDataStream = inputDataStream.flatMap(doFnWrapper).name(transform.getName()).returns(outputWindowedValueCoder);
    ...
    public static class WindowBoundTranslator<T> implements FlinkStreamingPipelineTranslator.StreamTransformTranslator<Window.Bound<T>> {
        ...
        public void translateNode(Window.Bound<T> transform, FlinkStreamingTranslationContext context) {
            PValue input = context.getInput(transform);
            DataStream<WindowedValue<T>> inputDataStream = context.getInputDataStream(input);
            final WindowingStrategy<T, ? extends BoundedWindow> windowingStrategy = (WindowingStrategy<T, ? extends BoundedWindow>) context.getOutput(transform).getWindowingStrategy();
            final WindowFn<T, ? extends BoundedWindow> windowFn = windowingStrategy.getWindowFn();
            WindowedValue.WindowedValueCoder<T> outputStreamCoder = WindowedValue.getFullCoder(context.getInput(transform).getCoder(), windowingStrategy.getWindowFn().windowCoder());
            CoderTypeInformation<WindowedValue<T>> outputWindowedValueCoder = new CoderTypeInformation<>(outputStreamCoder);
            final FlinkParDoBoundWrapper<T, T> windowDoFnAssigner = new FlinkParDoBoundWrapper<>(context.getPipelineOptions(), windowingStrategy, createWindowAssigner(windowFn));
            SingleOutputStreamOperator<WindowedValue<T>> windowedStream = inputDataStream.flatMap(windowDoFnAssigner).returns(outputWindowedValueCoder);
    ...
    public static class GroupByKeyTranslator<K, V> implements FlinkStreamingPipelineTranslator.StreamTransformTranslator<GroupByKey<K, V>> {
        ...
        public void translateNode(GroupByKey<K, V> transform, FlinkStreamingTranslationContext context) {
            ...
            KeyedStream<WindowedValue<KV<K, V>>, K> groupByKStream = FlinkGroupByKeyWrapper.groupStreamByKey(inputDataStream, inputKvCoder);
            DataStream<WindowedValue<KV<K, Iterable<V>>>> groupedByKNWstream = FlinkGroupAlsoByWindowWrapper.createForIterable(context.getPipelineOptions(), context.getInput(transform), groupByKStream);
    ...
    public static class CombinePerKeyTranslator<K, VIN, VACC, VOUT> implements FlinkStreamingPipelineTranslator.StreamTransformTranslator<Combine.PerKey<K, VIN, VOUT>> {
        ...
        public void translateNode(Combine.PerKey<K, VIN, VOUT> transform, FlinkStreamingTranslationContext context) {
            ...
            KeyedStream<WindowedValue<KV<K, VIN>>, K> groupByKStream = FlinkGroupByKeyWrapper.groupStreamByKey(inputDataStream, inputKvCoder);
            Combine.KeyedCombineFn<K, VIN, VACC, VOUT> combineFn = (Combine.KeyedCombineFn<K, VIN, VACC, VOUT>) transform.getFn();
            DataStream<WindowedValue<KV<K, VOUT>>> groupedByKNWstream = FlinkGroupAlsoByWindowWrapper.create(context.getPipelineOptions(), context.getInput(transform), groupByKStream, combineFn, outputKvCoder);
            context.setOutputDataStream(context.getOutput(transform), groupedByKNWstream);
        }
    }
    ...
    public static class ParDoBoundMultiStreamingTranslator<IN, OUT> implements FlinkStreamingPipelineTranslator.StreamTransformTranslator<ParDo.BoundMulti<IN, OUT>> {
        ...
        public void translateNode(ParDo.BoundMulti<IN, OUT> transform, FlinkStreamingTranslationContext context) {
            ...
            WindowedValue.WindowedValueCoder<RawUnionValue> outputStreamCoder = WindowedValue.getFullCoder(intermUnionCoder, windowingStrategy.getWindowFn().windowCoder());
            CoderTypeInformation<WindowedValue<RawUnionValue>> intermWindowedValueCoder = new CoderTypeInformation<>(outputStreamCoder);
            FlinkParDoBoundMultiWrapper<IN, OUT> doFnWrapper = new FlinkParDoBoundMultiWrapper<>(context.getPipelineOptions(), windowingStrategy, transform.getFn(), transform.getMainOutputTag(), tagsToLabels);
            DataStream<WindowedValue<IN>> inputDataStream = context.getInputDataStream(context.getInput(transform));
            SingleOutputStreamOperator<WindowedValue<RawUnionValue>> intermDataStream = inputDataStream.flatMap(doFnWrapper).returns(intermWindowedValueCoder);
            for (Map.Entry<TupleTag<?>, PCollection<?>> output : outputs.entrySet()) {
                final int outputTag = tagsToLabels.get(output.getKey());
                WindowedValue.WindowedValueCoder<?> coderForTag = WindowedValue.getFullCoder(output.getValue().getCoder(), windowingStrategy.getWindowFn().windowCoder());
                CoderTypeInformation<WindowedValue<?>> windowedValueCoder = new CoderTypeInformation(coderForTag);
                context.setOutputDataStream(output.getValue(), intermDataStream.filter(new FilterFunction<WindowedValue<RawUnionValue>>() {
                    ...
                    public void flatMap(WindowedValue<RawUnionValue> value, Collector<WindowedValue<?>> collector) throws Exception {
                        collector.collect(WindowedValue.of(value.getValue().getValue(), value.getTimestamp(), value.getWindows(), value.getPane()));
                    }
