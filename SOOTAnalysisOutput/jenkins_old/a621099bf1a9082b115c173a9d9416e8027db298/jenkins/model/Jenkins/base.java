...
@ExportedBean
public class Jenkins extends AbstractCIBase implements ModifiableTopLevelItemGroup, StaplerProxy, StaplerFallback, ViewGroup, AccessControlled, DescriptorByNameOwner, ModelObjectWithContextMenu, ModelObjectWithChildren {
    ...
    @edu.umd.cs.findbugs.annotations.SuppressWarnings({ "SC_START_IN_CTOR", "ST_WRITE_TO_STATIC_FROM_INSTANCE_METHOD" })
        ...
        try {
            ...
            if (slaveAgentPort != -1) {
                try {
                    ...
                    new AdministrativeError(getClass().getName() + ".tcpBind", "Failed to listen to incoming slave connection", "Failed to listen to incoming slave connection. <a href='configure'>Change the port number</a> to solve the problem.", e);
                }
            } else
            ...
            updateComputerList();
            {
                ...
                if (c != null)
                    for (ComputerListener cl : ComputerListener.all()) cl.onOnline(c, StreamTaskListener.fromStdout());
            }
    ...
    private File expandVariablesForDirectory(String base, Item item) {
        return new File(Util.replaceMacro(base, ImmutableMap.of("JENKINS_HOME", getRootDir().getPath(), "ITEM_ROOTDIR", item.getRootDir().getPath(), "ITEM_FULLNAME", item.getFullName(), "ITEM_FULL_NAME", item.getFullName().replace(':', '$'))));
    }
    ...
    public void onDeleted(TopLevelItem item) throws IOException {
        for (ItemListener l : ItemListener.all()) l.onDeleted(item);
    ...
    private synchronized TaskBuilder loadTasks() throws IOException {
        ...
        g.requires(JOB_LOADED).add("Finalizing set up", new Executable() {
            ...
            public void run(Reactor session) throws Exception {
                ...
                {
                    for (Node slave : slaves) slave.getAssignedLabels();
                    getAssignedLabels();
                }
    ...
    public void cleanUp() {
        for (ItemListener l : ItemListener.all()) l.onBeforeShutdown();
    ...
    public Object getDynamic(String token) {
        ...
        for (Action a : getManagementLinks()) if (a.getUrlName().equals(token))
            return a;
        return null;
    }
    ...
    public HttpRedirect doQuietDown(@Option(name = "-block", usage = "Block until the system really quiets down and no builds are running") @QueryParameter boolean block, @Option(name = "-timeout", usage = "If non-zero, only block up to the specified number of milliseconds") @QueryParameter int timeout) throws InterruptedException, IOException {
        synchronized (this) {
            checkPermission(ADMINISTER);
            isQuietingDown = true;
        }
        if (block) {
            ...
            while (isQuietingDown && (timeout <= 0 || System.currentTimeMillis() < timeout) && !RestartListener.isAllReady()) {
                Thread.sleep(1000);
            }
        }
        ...
        new Thread("safe-exit thread") {
            ...
            public void run() {
                try {
                    ...
                    while (isQuietingDown && (overallLoad.computeTotalExecutors() > overallLoad.computeIdleExecutors())) {
                        Thread.sleep(5000);
                    }
                    if (isQuietingDown) {
                        cleanUp();
    ...
    public Object getTarget() {
        try {
            ...
            if (rest.startsWith("/login") || rest.startsWith("/logout") || rest.startsWith("/accessDenied") || rest.startsWith("/adjuncts/") || rest.startsWith("/oops") || rest.startsWith("/signup") || rest.startsWith("/tcpSlaveAgentListener") || rest.matches("/computer/[^/]+/slave-agent[.]jnlp") && "true".equals(Stapler.getCurrentRequest().getParameter("encrypt")) || rest.startsWith("/cli") || rest.startsWith("/federatedLoginService/") || rest.startsWith("/securityRealm"))
                return this;
    ...
    transient final ExecutorService threadPoolForLoad = new ThreadPoolExecutor(TWICE_CPU_NUM, TWICE_CPU_NUM, 5L, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>(), new DaemonThreadFactory());
