...
class TypeUtils {
    ...
    private void process(TypeDescriptor descriptor, TypeMirror type) {
        try {
            ...
            this.env.getMessager().printMessage(Kind.WARNING, "Failed to generated type descriptor for " + type + ": " + ex.getMessage(), this.types.asElement(type));
        }
    }
    ...
    private static class TypeExtractor extends SimpleTypeVisitor8<String, TypeDescriptor> {
        ...
        public String visitDeclared(DeclaredType type, TypeDescriptor descriptor) {
            ...
            name.append("<").append(type.getTypeArguments().stream().map((t) -> visit(t, descriptor)).collect(Collectors.joining(","))).append(">");
    ...
    static class TypeDescriptor {
        ...
        TypeMirror resolveGeneric(String parameterName) {
            return this.generics.entrySet().stream().filter((e) -> getParameterName(e.getKey()).equals(parameterName)).findFirst().map(Entry::getValue).orElse(null);
        }
        ...
        private void registerIfNecessary(TypeMirror variable, TypeMirror resolution) {
            if (variable instanceof TypeVariable) {
                ...
                if (this.generics.keySet().stream().noneMatch((candidate) -> getParameterName(candidate).equals(getParameterName(typeVariable)))) {
