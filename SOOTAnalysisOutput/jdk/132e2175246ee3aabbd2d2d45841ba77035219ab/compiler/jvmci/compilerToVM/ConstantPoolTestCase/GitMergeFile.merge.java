...
public class ConstantPoolTestCase {
    ...
    public static enum ConstantTypes {
        ...
        protected void checkIndex(ConstantPool constantPoolSS, int index) {
            ...
            if (!this.equals(type)) {
                String msg = String.format("TESTBUG: CP tag should be a %s, but is %s", this.name(), type.name());
                throw new Error(msg);
            }
        }
    }
    ...
    public static interface Validator {
        ...
        void validate(jdk.vm.ci.meta.ConstantPool constantPoolCTVM, ConstantTypes cpType, DummyClasses dummyClass, int index);
    }
    ...
    public void test() {
        for (DummyClasses dummyClass : DummyClasses.values()) {
            ...
            System.out.printf("Testing dummy %s with constant pool cached = %b%n", dummyClass.klass, isCPCached);
