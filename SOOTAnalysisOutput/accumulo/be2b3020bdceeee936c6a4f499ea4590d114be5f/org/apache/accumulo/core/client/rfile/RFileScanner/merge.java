...
class RFileScanner extends ScannerOptions implements Scanner {
    ...
    RFileScanner(Opts opts) {
        ...
        if (opts.indexCacheSize > 0 || opts.dataCacheSize > 0) {
            ConfigurationCopy cc = tableConf instanceof ConfigurationCopy ? (ConfigurationCopy) tableConf : new ConfigurationCopy(tableConf);
            try {
    ...
    public Iterator<Entry<Key, Value>> iterator() {
        try {
            ...
            for (int i = 0; i < sources.length; i++) {
                ...
                CachableBuilder cb = new CachableBuilder().cacheId("source-" + i).input(inputStream).length(sources[i].getLength()).conf(opts.in.getConf()).cacheProvider(cacheProvider).cryptoService(cryptoService);
