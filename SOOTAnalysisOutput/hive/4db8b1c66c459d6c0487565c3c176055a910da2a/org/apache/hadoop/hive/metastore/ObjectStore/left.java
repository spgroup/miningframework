...
public class ObjectStore implements RawStore, Configurable {
    ...
    public void alterTable(String catName, String dbname, String name, Table newTable) throws InvalidObjectException, MetaException {
        ...
        try {
            ...
            if (newTable.getValidWriteIdList() != null && TxnUtils.isTransactionalTable(newTable)) {
                if (!isCurrentStatsValidForTheQuery(oldt, newt.getTxnId(), newt.getWriteIdList(), newt.getWriteId(), true)) {
    ...
    private MColumnDescriptor alterPartitionNoTxn(String catName, String dbname, String name, List<String> part_vals, Partition newPart) throws InvalidObjectException, MetaException {
        ...
        if (newPart.getValidWriteIdList() != null && TxnUtils.isTransactionalTable(table.getParameters())) {
            if (!isCurrentStatsValidForTheQuery(oldp, newp.getTxnId(), newp.getWriteIdList(), newp.getWriteId(), true)) {
    ...
    @Override
    public void alterPartitions(String catName, String dbname, String name, List<List<String>> part_vals, List<Partition> newParts, long txnId, String writeIdList, long writeId) throws InvalidObjectException, MetaException {
        boolean success = false;
        Exception e = null;
        try {
            openTransaction();
    ...
    public ColumnStatistics getTableColumnStatistics(String catName, String dbName, String tableName, List<String> colNames, long txnId, String writeIdList) throws MetaException, NoSuchObjectException {
        ...
        ColumnStatistics cS = getTableColumnStatisticsInternal(catName, dbName, tableName, colNames, true, true);
        if (cS != null) {
    ...
    public List<ColumnStatistics> getPartitionColumnStatistics(String catName, String dbName, String tableName, List<String> partNames, List<String> colNames, long txnId, String writeIdList) throws MetaException, NoSuchObjectException {
        if (writeIdList != null) {
            ...
            for (String partName : partNames) {
                ...
                if (!isCurrentStatsValidForTheQuery(mpart, txnId, writeIdList, -1, false)) {
                    LOG.debug("The current metastore transactional partition column statistics " + "for " + dbName + "." + tableName + "." + mpart.getPartitionName() + " is not valid " + "for the current query.");
                    return null;
                }
            }
        }
    ...
    private boolean isCurrentStatsValidForTheQuery(MTable tbl, long queryTxnId, String queryValidWriteIdList, long queryWriteId, boolean checkConcurrentWrites) throws MetaException {
        return isCurrentStatsValidForTheQuery(tbl.getParameters(), tbl.getTxnId(), tbl.getWriteIdList(), tbl.getWriteId(), queryTxnId, queryValidWriteIdList, queryWriteId, checkConcurrentWrites);
    }

    private boolean isCurrentStatsValidForTheQuery(MPartition part, long queryTxnId, String queryValidWriteIdList, long queryWriteId, boolean checkConcurrentWrites) throws MetaException {
        return isCurrentStatsValidForTheQuery(part.getParameters(), part.getTxnId(), part.getWriteIdList(), part.getWriteId(), queryTxnId, queryValidWriteIdList, queryWriteId, checkConcurrentWrites);
    }

    private boolean isCurrentStatsValidForTheQuery(Map<String, String> statsParams, long statsTxnId, String statsWriteIdList, long statsWriteId, long queryTxnId, String queryValidWriteIdList, long queryWriteId, boolean checkConcurrentWrites) throws MetaException {
        ...
        LOG.trace("Called with stats {}, {}; query {}, {}; checkConcurrentWrites {}", statsTxnId, statsWriteIdList, queryTxnId, queryValidWriteIdList, checkConcurrentWrites);
        ...
        return !checkConcurrentWrites ? TxnIdUtils.checkEquivalentWriteIds(list4Stats, list4TheQuery) : !TxnIdUtils.areTheseConcurrentWrites(list4Stats, statsWriteId, list4TheQuery, queryWriteId);
    }
}
