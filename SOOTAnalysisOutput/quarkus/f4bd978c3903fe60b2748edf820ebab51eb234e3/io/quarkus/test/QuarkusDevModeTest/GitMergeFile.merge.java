...
public class QuarkusDevModeTest implements BeforeAllCallback, AfterAllCallback, BeforeEachCallback, AfterEachCallback, TestInstanceFactory {
    ...
    private DevModeContext exportArchive(Path deploymentDir, Path testSourceDir, Path testSourcesParentDir) {
        try {
            ...
            DevModeContext.ModuleInfo.Builder moduleBuilder = new DevModeContext.ModuleInfo.Builder().setAppArtifactKey(AppArtifactKey.fromString("io.quarkus.test:app-under-test")).setName("default").setProjectDirectory(deploymentDir.toAbsolutePath().toString()).setSourcePaths(Collections.singleton(deploymentSourcePath.toAbsolutePath().toString())).setClassesPath(classes.toAbsolutePath().toString()).setResourcePath(deploymentResourcePath.toAbsolutePath().toString()).setResourcesOutputPath(classes.toAbsolutePath().toString()).setSourceParents(Collections.singleton(deploymentSourceParentPath.toAbsolutePath().toString())).setPreBuildOutputDir(targetDir.resolve("generated-sources").toAbsolutePath().toString()).setTargetDir(targetDir.toAbsolutePath().toString());
            if (testArchiveProducer != null) {
                ...
                moduleBuilder.setTestSourcePaths(Collections.singleton(deploymentTestSourcePath.toAbsolutePath().toString())).setTestClassesPath(testClasses.toAbsolutePath().toString()).setTestResourcePath(deploymentTestResourcePath.toAbsolutePath().toString());
            }
