...
public class LocalsAndOperands {
    ...
    private static final long LOWER_LONG_VAL = 4L;

    private static final long UPPER_LONG_VAL = 0x123400000000L;
    ...
    public static StackFrame[][] knownLocalsProvider() {
        ...
        list.add(new KnownLocalsTester().testLocalsKeepAliveArgs(0xA, 'z', "himom", 0x3FF00000000L + 0xFFFF, Math.PI));
        if (testUnused) {
    ...
    public static void checkLocalValues(StackFrame... frames) {
        ...
        try {
            Stream.of(frames).filter(f -> KnownLocalsTester.TEST_METHODS.contains(f.getMethodName())).forEach(LocalsAndOperands::checkFrameLocals);
        } catch (Exception e) {
    ...
    private static void checkFrameLocals(StackFrame f) {
        ...
        for (int i = 0; i < locals.length; i++) {
            ...
            if (expected == null) {
                ...
                assertEquals(expectedArray[i + 1], null, "Malformed array of expected values - slot after TwoSlotValue should be null");
                assertLongIsInSlots(locals[i], locals[i + 1], ((KnownLocalsTester.TwoSlotValue) expected).value);
                i++;
            } else if (primitiveSlotClass.isInstance(observed)) {
                assertTrue(primitiveValueEquals(observed, expected), "Local value mismatch: local " + i + " value is " + observed + ", expected " + expected);
            } else if (expected instanceof Class) {
                assertTrue(((Class) expected).isInstance(observed), "Local value mismatch: local " + i + " expected instancof " + expected + " but got " + observed);
            } else if (expected instanceof String) {
                assertEquals(expected, observed, "Local value mismatch: local " + i + " value is " + observed + ", expected " + expected);
            } else {
    ...
    public synchronized void fullStackSanityCheck() throws Throwable {
        ...
        StackWalker sw = (StackWalker) getExtendedWalker.invoke(null, EnumSet.of(SHOW_HIDDEN_FRAMES, SHOW_REFLECT_FRAMES, RETAIN_CLASS_REFERENCE));
        sw.forEach(f -> {
            if (debug) {
                printLocals(f);
            } else {
                try {
                    System.out.println("    " + f + ": " + ((Object[]) getLocals.invoke(f)).length + " locals, " + ((Object[]) getOperands.invoke(f)).length + " operands, " + ((Object[]) getMonitors.invoke(f)).length + " monitors");
    ...
    static class KnownLocalsTester {
        ...
        private synchronized StackFrame[] testLocalsUnused() {
            ...
            double d = Math.PI;
            return walker.walk(s -> s.filter(f -> TEST_METHODS.contains(f.getMethodName())).toArray(StackFrame[]::new));
        }
        ...
        private synchronized StackFrame[] testLocalsKeepAlive() {
            ...
            long l = 0x3FF00000000L + 0xFFFFL;
            double d = Math.PI;
            StackFrame[] frames = walker.walk(s -> s.filter(f -> TEST_METHODS.contains(f.getMethodName())).toArray(StackFrame[]::new));
            System.out.println("Stayin' alive: " + this + " " + x + " " + c + " " + hi + " " + l + " " + d);
            return frames;
        }

        private synchronized StackFrame[] testLocalsKeepAliveArgs(int x, char c, String hi, long l, double d) {
            StackFrame[] frames = walker.walk(s -> s.filter(f -> TEST_METHODS.contains(f.getMethodName())).toArray(StackFrame[]::new));
            System.out.println("Stayin' alive: " + this + " " + x + " " + c + " " + hi + " " + l + " " + d);
            return frames;
        }
        ...
        private final static Object[] LOCAL_VALUES = new Object[] { LocalsAndOperands.KnownLocalsTester.class, Integer.valueOf(0xA), Integer.valueOf(0x7A), "himom", new TwoSlotValue(0x3FF00000000L + 0xFFFFL), null, new TwoSlotValue(Double.doubleToRawLongBits(Math.PI)), null, Integer.valueOf(0) };

        private final static List<String> TEST_METHODS = List.of("testLocalsUnused", "testLocalsKeepAlive", "testLocalsKeepAliveArgs");
    }
    ...
    private static void usedLong(long longArg) throws Exception {
        StackFrame[] frames = extendedWalker.walk(s -> s.filter(f -> "usedLong".equals(f.getMethodName())).toArray(StackFrame[]::new));
        try {
            dumpFramesIfDebug(frames);
    ...
    private static void unusedLong(long longArg) throws Exception {
        StackFrame[] frames = extendedWalker.walk(s -> s.filter(f -> "unusedLong".equals(f.getMethodName())).toArray(StackFrame[]::new));
        try {
            dumpFramesIfDebug(frames);
    ...
    private static void usedDouble(double doubleArg) throws Exception {
        StackFrame[] frames = extendedWalker.walk(s -> s.filter(f -> "usedDouble".equals(f.getMethodName())).toArray(StackFrame[]::new));
        try {
            dumpFramesIfDebug(frames);
            Object[] locals = (Object[]) getLocals.invoke(frames[0]);
    ...
    static boolean primitiveValueEquals(Object primVal, Object expectedVal) {
        try {
            if (expectedVal instanceof Long) {
                ...
                if (is32bit) {
                    assertTrue(primitiveSlot32Class.isInstance(primVal), "expected a PrimitiveSlot32 on 32-bit VM");
                    assertTrue(4 == (int) primitiveSize.invoke(primVal));
                    return expectedInt == (int) primitiveIntValue.invoke(primVal);
                } else {
                    assertTrue(primitiveSlot64Class.isInstance(primVal), "expected a PrimitiveSlot64 on 64-bit VM");
                    assertTrue(8 == (int) primitiveSize.invoke(primVal));
                    long primValLong = (long) primitiveLongValue.invoke(primVal);
                    return (int) (primValLong & 0x00000000FFFFFFFFL) == expectedInt || (int) (primValLong >>> 32) == expectedInt;
                }
            } else {
    ...
    static void assertLongIsInSlots(Object primVal0, Object primVal1, long expected) {
        try {
            if (is32bit) {
                ...
                if (!((primitiveValueEquals(primVal0, upper) && primitiveValueEquals(primVal1, lower)) || (primitiveValueEquals(primVal0, lower) && primitiveValueEquals(primVal1, upper)))) {
                    throw new RuntimeException(String.format("0x%X and 0x%X of 0x%016X not found in 0x%X and 0x%X", upper, lower, expected, (int) primitiveIntValue.invoke(primVal0), (int) primitiveIntValue.invoke(primVal1)));
                }
            } else {
                if (!(primitiveValueEquals(primVal0, expected) || primitiveValueEquals(primVal1, expected))) {
                    throw new RuntimeException(String.format("0x%016X not found in 0x%016X or 0x%016X", expected, (long) primitiveLongValue.invoke(primVal0), (long) primitiveLongValue.invoke(primVal1)));
                }
            }
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new RuntimeException(e);
