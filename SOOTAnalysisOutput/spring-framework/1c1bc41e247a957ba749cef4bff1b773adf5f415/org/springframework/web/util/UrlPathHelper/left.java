...
public class UrlPathHelper {
    ...
    private static boolean isServlet4Present = ClassUtils.isPresent("javax.servlet.http.HttpServletMapping", UrlPathHelper.class.getClassLoader());
    ...
    private boolean skipServletPathDetermination(HttpServletRequest request) {
        if (isServlet4Present) {
            if (request.getHttpServletMapping().getMappingMatch() != null) {
                return !request.getHttpServletMapping().getMappingMatch().equals(MappingMatch.PATH) || request.getHttpServletMapping().getPattern().equals("/*");
            }
        }
        return false;
    }
