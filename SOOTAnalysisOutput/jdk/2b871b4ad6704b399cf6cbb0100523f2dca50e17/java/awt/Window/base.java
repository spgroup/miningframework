...
public class Window extends Container implements Accessible {
    ...
    void setOpacity(float opacity) {
        synchronized (getTreeLock()) {
            ...
            if ((gc.getDevice().getFullScreenWindow() == this) && (opacity < 1.0f)) {
                throw new IllegalArgumentException("Setting opacity for full-screen window is not supported.");
            }
    ...
    void setShape(Shape shape) {
        synchronized (getTreeLock()) {
            ...
            if ((gc.getDevice().getFullScreenWindow() == this) && (shape != null)) {
                throw new IllegalArgumentException("Setting shape for full-screen window is not supported.");
            }
            this.shape = shape;
            WindowPeer peer = (WindowPeer) getPeer();
            if (peer != null) {
    ...
    void setOpaque(boolean opaque) {
        synchronized (getTreeLock()) {
            ...
            if ((gc.getDevice().getFullScreenWindow() == this) && !opaque) {
                throw new IllegalArgumentException("Making full-screen window non opaque is not supported.");
            }
    ...
    private static void setLayersOpaque(Component component, boolean isOpaque) {
        if (Component.doesImplement(component, "javax.swing.RootPaneContainer")) {
            ...
            javax.swing.JComponent gp = (rpc.getGlassPane() instanceof javax.swing.JComponent) ? (javax.swing.JComponent) rpc.getGlassPane() : null;
            if (gp != null) {
    ...
    private Point2D calculateSecurityWarningPosition(double x, double y, double w, double h) {
        return new Point2D.Double(x + w * securityWarningAlignmentX + securityWarningPointX, y + h * securityWarningAlignmentY + securityWarningPointY);
    }
