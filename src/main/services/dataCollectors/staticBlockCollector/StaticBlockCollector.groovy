 ...
 */
class StaticBlockCollector extends ModifiedLinesCollectorAbstract {

    public StaticBlockCollector() {
        modifiedStaticBlocksHelper = new StaticBlocksHelper("diffj.jar");
    }
    ...
    void collectData(Project project, MergeCommit mergeCommit) {
        ...
        for (String filePath : mutuallyModifiedFiles) {
            ...
            Set<StaticBlock> allModifiedStaticBlocks = modifiedStaticBlocksHelper.getModifiedStaticBlocks(project, filePath, mergeCommit.getAncestorSHA(), mergeCommit.getSHA(),mergeCommit)
            // get staticBlock modified by both left and right revisions
            Map<String, Tuple2<StaticBlock, StaticBlock>> mutuallyModifiedStaticBlocks = getMutuallyModifiedStaticBlocks(project, mergeCommit, filePath);
            ...
            if (fileHasMutuallyModifiedStaticBlocks) {
                ...
                String pathFilesMergeCommint =   revisionsCollector.collectDataFromFile(project, mergeCommit, filePath);

                StaticBlockRunner.collectStaticBlockResults(pathFilesMergeCommint)
                for (def staticBlock : allModifiedStaticBlocks) {
                    // get left and right staticBlock for the specific merge staticBlock
                    Tuple2<StaticBlock, StaticBlock> leftAndRightStaticBlocks = mutuallyModifiedStaticBlocks[staticBlock.getIdentifier()];
        ...
        println "${project.getName()} - StaticBlockCollector collection finished"
    }
