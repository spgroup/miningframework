...
public class ObjectStore implements RawStore, Configurable {
    ...
    public void alterTable(String catName, String dbname, String name, Table newTable) throws InvalidObjectException, MetaException {
        ...
        try {
            ...
            if (newTable.getValidWriteIdList() != null && TxnUtils.isTransactionalTable(newTable)) {
                if (!isCurrentStatsValidForTheQuery(oldt, newt.getTxnId(), newt.getWriteIdList(), -1, true)) {
    ...
    private MColumnDescriptor alterPartitionNoTxn(String catName, String dbname, String name, List<String> part_vals, Partition newPart) throws InvalidObjectException, MetaException {
        ...
        if (newPart.getValidWriteIdList() != null && TxnUtils.isTransactionalTable(table.getParameters())) {
            if (!isCurrentStatsValidForTheQuery(oldp, newp.getTxnId(), newp.getWriteIdList(), -1, true)) {
    ...
    @Override
    public void alterPartitions(String catName, String dbname, String name, List<List<String>> part_vals, List<Partition> newParts, long txnId, String writeIdList) throws InvalidObjectException, MetaException {
        boolean success = false;
        Exception e = null;
        try {
            openTransaction();
    ...
    private boolean isCurrentStatsValidForTheQuery(MTable tbl, long txnId, String queryValidWriteIdList, long statsWriteId, boolean checkConcurrentWrites) throws MetaException {
        return isCurrentStatsValidForTheQuery(tbl.getTxnId(), tbl.getParameters(), tbl.getWriteIdList(), txnId, queryValidWriteIdList, statsWriteId, checkConcurrentWrites);
    }

    private boolean isCurrentStatsValidForTheQuery(MPartition part, long txnId, String queryValidWriteIdList, long statsWriteId, boolean checkConcurrentWrites) throws MetaException {
        return isCurrentStatsValidForTheQuery(part.getTxnId(), part.getParameters(), part.getWriteIdList(), txnId, queryValidWriteIdList, statsWriteId, checkConcurrentWrites);
    }

    private boolean isCurrentStatsValidForTheQuery(long statsTxnId, Map<String, String> statsParams, String statsWriteIdList, long queryTxnId, String queryValidWriteIdList, long statsWriteId, boolean checkConcurrentWrites) throws MetaException {
        ...
        return !checkConcurrentWrites ? TxnIdUtils.checkEquivalentWriteIds(list4Stats, list4TheQuery) : !TxnIdUtils.areTheseConcurrentWrites(list4Stats, list4TheQuery, statsWriteId);
    }
}