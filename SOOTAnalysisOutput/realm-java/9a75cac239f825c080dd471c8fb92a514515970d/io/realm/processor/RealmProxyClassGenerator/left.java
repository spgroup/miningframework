...
public class RealmProxyClassGenerator {
    ...
    public void generate() throws IOException, UnsupportedOperationException {
        String qualifiedGeneratedClassName = String.format("%s.%s", REALM_PACKAGE_NAME, getProxyClassSimpleName(className));
    ...
    private void emitCopyOrUpdateMethod(JavaWriter writer) throws IOException {
        ...
        if (primaryKey == null) {
            ...
            writer.emitStatement("%s realmObject = null", className).beginControlFlow("if (update)").emitStatement("Table table = realm.getTable(%s.class)", className).emitStatement("long pkColumnIndex = table.getPrimaryKey()");
            if (primaryKey == null) {
                writer.emitStatement("long rowIndex = TableOrView.NO_MATCH");
            } else if (isString(primaryKey)) {
            ...
            writer.beginControlFlow("if (rowIndex != TableOrView.NO_MATCH)").emitStatement("realmObject = new %s()", getProxyClassSimpleName(className)).emitStatement("realmObject.realm = realm").emitStatement("realmObject.row = table.getRow(rowIndex)").nextControlFlow("else").emitStatement("update = false").endControlFlow();
            writer.endControlFlow();
            writer.emitEmptyLine().beginControlFlow("if (update)").emitStatement("return update(realm, realmObject, object)").nextControlFlow("else").emitStatement("return copy(realm, object)").endControlFlow();
        }
        writer.endMethod();
        writer.emitEmptyLine();
    }
    ...
    private void emitCopyMethod(JavaWriter writer) throws IOException {
        writer.beginMethod(className, "copy", EnumSet.of(Modifier.PUBLIC, Modifier.STATIC), "Realm", "realm", className, "newObject");
        writer.emitStatement("%s realmObject = realm.createObject(%s.class)", className, className);
        for (VariableElement field : fields) {
            String fieldName = field.getSimpleName().toString();
            if (typeUtils.isAssignable(field.asType(), realmObject)) {
                writer.emitEmptyLine().emitStatement("%s %s = newObject.%s()", getFieldTypeSimpleName(field), fieldName, getters.get(fieldName)).beginControlFlow("if (%s != null)", fieldName).emitStatement("realmObject.%s(%s.copy(realm, %s))", setters.get(fieldName), getProxyClassSimpleName(field), fieldName).endControlFlow();
            } else if (typeUtils.isAssignable(field.asType(), realmList)) {
                writer.emitEmptyLine().emitStatement("RealmList<%s> %sList = newObject.%s()", getGenericType(field), fieldName, getters.get(fieldName)).beginControlFlow("if (%sList != null)", fieldName).emitStatement("RealmList<%s> %sRealmList = realmObject.%s()", getGenericType(field), fieldName, getters.get(fieldName)).beginControlFlow("for (int i = 0; i < %sList.size(); i++)", fieldName).emitStatement("%sRealmList.add(%s.copy(realm, %sList.get(i)))", fieldName, getProxyClassSimpleName(field), fieldName).endControlFlow().endControlFlow().emitEmptyLine();
            } else {
                boolean wrapInGuard = NULLABLE_JAVA_TYPES.contains(field.asType().toString());
                if (wrapInGuard)
                    writer.beginControlFlow("if (newObject.%s() != null)", getters.get(fieldName));
    ...
    private void emitUpdateMethod(JavaWriter writer) throws IOException {
        ...
        for (VariableElement field : fields) {
            String fieldName = field.getSimpleName().toString();
            if (typeUtils.isAssignable(field.asType(), realmObject)) {
                writer.emitStatement("%s %s = newObject.%s()", getFieldTypeSimpleName(field), fieldName, getters.get(fieldName)).beginControlFlow("if (%s != null)", fieldName).emitStatement("realmObject.%s(%s.copyOrUpdate(realm, %s, realm.getTable(%s.class).hasPrimaryKey()))", setters.get(fieldName), getProxyClassSimpleName(field), fieldName, getFieldTypeSimpleName(field)).nextControlFlow("else").emitStatement("realmObject.%s(null)", setters.get(fieldName)).endControlFlow();
            } else if (typeUtils.isAssignable(field.asType(), realmList)) {
                writer.emitStatement("RealmList<%s> %sList = newObject.%s()", getGenericType(field), fieldName, getters.get(fieldName)).beginControlFlow("if (%sList != null)", fieldName).emitStatement("RealmList<%s> %sRealmList = realmObject.%s()", getGenericType(field), fieldName, getters.get(fieldName)).emitStatement("%sRealmList.clear()", fieldName).beginControlFlow("for (int i = 0; i < %sList.size(); i++)", fieldName).emitStatement("%sRealmList.add(%s.copy(realm, %sList.get(i)))", fieldName, getProxyClassSimpleName(field), fieldName).endControlFlow().endControlFlow();
            } else {
                if (field == primaryKey) {
                    continue;
                }
    ...
    private boolean isString(VariableElement field) {
        if (field == null) {
            return false;
        }
    ...
    private String getProxyClassSimpleName(VariableElement field) {
        if (typeUtils.isAssignable(field.asType(), realmList)) {
            return getProxyClassSimpleName(getGenericType(field));
        } else {
            return getProxyClassSimpleName(getFieldTypeSimpleName(field));
        }
    }

    private String getProxyClassSimpleName(String className) {
