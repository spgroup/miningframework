...
public final class StreamingTransformTranslator {
    ...
    private static <T> TransformEvaluator<ConsoleIO.Write.Unbound<T>> print() {
        return new TransformEvaluator<ConsoleIO.Write.Unbound<T>>() {
            ...
            public void evaluate(ConsoleIO.Write.Unbound<T> transform, EvaluationContext context) {
                ...
                JavaDStreamLike<WindowedValue<T>, ?, JavaRDD<WindowedValue<T>>> dstream = (JavaDStreamLike<WindowedValue<T>, ?, JavaRDD<WindowedValue<T>>>) ((StreamingEvaluationContext) context).getStream(transform);
    ...
    private static <T> TransformEvaluator<Flatten.FlattenPCollectionList<T>> flattenPColl() {
        return new TransformEvaluator<Flatten.FlattenPCollectionList<T>>() {
            ...
            public void evaluate(Flatten.FlattenPCollectionList<T> transform, EvaluationContext context) {
                ...
                JavaDStream<WindowedValue<T>> unifiedStreams = sec.getStreamingContext().union(dStreams.remove(0), dStreams);
                if (rdds.size() > 0) {
                    JavaDStream<WindowedValue<T>> joined = unifiedStreams.transform(new Function<JavaRDD<WindowedValue<T>>, JavaRDD<WindowedValue<T>>>() {
    ...
    private static <T, W extends BoundedWindow> TransformEvaluator<Window.Bound<T>> window() {
        return new TransformEvaluator<Window.Bound<T>>() {
            ...
            public void evaluate(Window.Bound<T> transform, EvaluationContext context) {
                ...
                @SuppressWarnings("unchecked")
                JavaDStream<WindowedValue<T>> dStream = (JavaDStream<WindowedValue<T>>) sec.getStream(transform);
                ...
                JavaDStream<WindowedValue<T>> windowedDStream = dStream.window(windowDuration, slideDuration);
                if (TranslationUtils.skipAssignWindows(transform, context)) {
                    ...
                    JavaDStream<WindowedValue<T>> outStream = windowedDStream.transform(new Function<JavaRDD<WindowedValue<T>>, JavaRDD<WindowedValue<T>>>() {

                        @Override
                        public JavaRDD<WindowedValue<T>> call(JavaRDD<WindowedValue<T>> rdd) throws Exception {
                            final Accumulator<NamedAggregators> accum = AccumulatorSingleton.getInstance(new JavaSparkContext(rdd.context()));
                            return rdd.mapPartitions(new DoFnFunction<>(accum, addWindowsDoFn, runtimeContext, null, null));
                        }
                    });
    ...
    private static <K, V> TransformEvaluator<GroupByKeyOnly<K, V>> gbko() {
        return new TransformEvaluator<GroupByKeyOnly<K, V>>() {
            ...
            public void evaluate(GroupByKeyOnly<K, V> transform, EvaluationContext context) {
                StreamingEvaluationContext sec = (StreamingEvaluationContext) context;
                @SuppressWarnings("unchecked")
                JavaDStream<WindowedValue<KV<K, V>>> dStream = (JavaDStream<WindowedValue<KV<K, V>>>) sec.getStream(transform);
                @SuppressWarnings("unchecked")
                final KvCoder<K, V> coder = (KvCoder<K, V>) sec.getInput(transform).getCoder();
                JavaDStream<WindowedValue<KV<K, Iterable<V>>>> outStream = dStream.transform(new Function<JavaRDD<WindowedValue<KV<K, V>>>, JavaRDD<WindowedValue<KV<K, Iterable<V>>>>>() {
    ...
    private static <K, V, W extends BoundedWindow> TransformEvaluator<GroupAlsoByWindow<K, V>> gabw() {
        return new TransformEvaluator<GroupAlsoByWindow<K, V>>() {
            ...
            public void evaluate(final GroupAlsoByWindow<K, V> transform, EvaluationContext context) {
                ...
                final SparkRuntimeContext runtimeContext = sec.getRuntimeContext();
                @SuppressWarnings("unchecked")
                JavaDStream<WindowedValue<KV<K, Iterable<WindowedValue<V>>>>> dStream = (JavaDStream<WindowedValue<KV<K, Iterable<WindowedValue<V>>>>>) sec.getStream(transform);
                @SuppressWarnings("unchecked")
                final KvCoder<K, Iterable<WindowedValue<V>>> inputKvCoder = (KvCoder<K, Iterable<WindowedValue<V>>>) sec.getInput(transform).getCoder();
                JavaDStream<WindowedValue<KV<K, Iterable<V>>>> outStream = dStream.transform(new Function<JavaRDD<WindowedValue<KV<K, Iterable<WindowedValue<V>>>>>, JavaRDD<WindowedValue<KV<K, Iterable<V>>>>>() {

                    @Override
                    public JavaRDD<WindowedValue<KV<K, Iterable<V>>>> call(JavaRDD<WindowedValue<KV<K, Iterable<WindowedValue<V>>>>> rdd) throws Exception {
                        final Accumulator<NamedAggregators> accum = AccumulatorSingleton.getInstance(new JavaSparkContext(rdd.context()));
                        return GroupCombineFunctions.groupAlsoByWindow(rdd, transform, runtimeContext, accum, inputKvCoder);
                    }
                });
                sec.setStream(transform, outStream);
            }
    ...
    private static <K, InputT, OutputT> TransformEvaluator<Combine.GroupedValues<K, InputT, OutputT>> combineGrouped() {
        return new TransformEvaluator<Combine.GroupedValues<K, InputT, OutputT>>() {
            ...
            public void evaluate(Combine.GroupedValues<K, InputT, OutputT> transform, EvaluationContext context) {
                StreamingEvaluationContext sec = (StreamingEvaluationContext) context;
                PCollection<? extends KV<K, ? extends Iterable<InputT>>> input = sec.getInput(transform);
                WindowingStrategy<?, ?> windowingStrategy = input.getWindowingStrategy();
                AppliedCombineFn<? super K, ? super InputT, ?, OutputT> appliedFn = transform.getAppliedFn(context.getPipeline().getCoderRegistry(), input.getCoder(), windowingStrategy);
                @SuppressWarnings("unchecked")
                CombineWithContext.KeyedCombineFnWithContext<K, InputT, ?, OutputT> fn = (CombineWithContext.KeyedCombineFnWithContext<K, InputT, ?, OutputT>) appliedFn.getFn();
                @SuppressWarnings("unchecked")
                JavaDStream<WindowedValue<KV<K, Iterable<InputT>>>> dStream = (JavaDStream<WindowedValue<KV<K, Iterable<InputT>>>>) sec.getStream(transform);
                SparkKeyedCombineFn<K, InputT, ?, OutputT> combineFnWithContext = new SparkKeyedCombineFn<>(fn, sec.getRuntimeContext(), TranslationUtils.getSideInputs(transform.getSideInputs(), context), windowingStrategy);
                sec.setStream(transform, dStream.map(new TranslationUtils.CombineGroupedValues<>(combineFnWithContext)));
            }
        };
    }
    ...
    private static <InputT, AccumT, OutputT> TransformEvaluator<Combine.Globally<InputT, OutputT>> combineGlobally() {
        return new TransformEvaluator<Combine.Globally<InputT, OutputT>>() {
            ...
            public void evaluate(Combine.Globally<InputT, OutputT> transform, EvaluationContext context) {
                ...
                @SuppressWarnings("unchecked")
                final CombineWithContext.CombineFnWithContext<InputT, AccumT, OutputT> combineFn = (CombineWithContext.CombineFnWithContext<InputT, AccumT, OutputT>) CombineFnUtil.toFnWithContext(transform.getFn());
                ...
                @SuppressWarnings("unchecked")
                JavaDStream<WindowedValue<InputT>> dStream = (JavaDStream<WindowedValue<InputT>>) sec.getStream(transform);
                JavaDStream<WindowedValue<OutputT>> outStream = dStream.transform(new Function<JavaRDD<WindowedValue<InputT>>, JavaRDD<WindowedValue<OutputT>>>() {
    ...
    private static <K, InputT, AccumT, OutputT> TransformEvaluator<Combine.PerKey<K, InputT, OutputT>> combinePerKey() {
        return new TransformEvaluator<Combine.PerKey<K, InputT, OutputT>>() {
            ...
            @Override
            public void evaluate(final Combine.PerKey<K, InputT, OutputT> transform, final EvaluationContext context) {
                StreamingEvaluationContext sec = (StreamingEvaluationContext) context;
                final PCollection<KV<K, InputT>> input = sec.getInput(transform);
                @SuppressWarnings("unchecked")
                final KvCoder<K, InputT> inputCoder = (KvCoder<K, InputT>) sec.getInput(transform).getCoder();
                @SuppressWarnings("unchecked")
                final CombineWithContext.KeyedCombineFnWithContext<K, InputT, AccumT, OutputT> combineFn = (CombineWithContext.KeyedCombineFnWithContext<K, InputT, AccumT, OutputT>) CombineFnUtil.toFnWithContext(transform.getFn());
                final WindowingStrategy<?, ?> windowingStrategy = input.getWindowingStrategy();
                final SparkRuntimeContext runtimeContext = sec.getRuntimeContext();
                final Map<TupleTag<?>, KV<WindowingStrategy<?, ?>, BroadcastHelper<?>>> sideInputs = TranslationUtils.getSideInputs(transform.getSideInputs(), context);
                @SuppressWarnings("unchecked")
                JavaDStream<WindowedValue<KV<K, InputT>>> dStream = (JavaDStream<WindowedValue<KV<K, InputT>>>) sec.getStream(transform);
                JavaDStream<WindowedValue<KV<K, OutputT>>> outStream = dStream.transform(new Function<JavaRDD<WindowedValue<KV<K, InputT>>>, JavaRDD<WindowedValue<KV<K, OutputT>>>>() {
    ...
    private static <InputT, OutputT> TransformEvaluator<ParDo.Bound<InputT, OutputT>> parDo() {
        return new TransformEvaluator<ParDo.Bound<InputT, OutputT>>() {
            ...
            @Override
            public void evaluate(final ParDo.Bound<InputT, OutputT> transform, final EvaluationContext context) {
                final StreamingEvaluationContext sec = (StreamingEvaluationContext) context;
                final SparkRuntimeContext runtimeContext = sec.getRuntimeContext();
                final Map<TupleTag<?>, KV<WindowingStrategy<?, ?>, BroadcastHelper<?>>> sideInputs = TranslationUtils.getSideInputs(transform.getSideInputs(), context);
                @SuppressWarnings("unchecked")
                final WindowFn<Object, ?> windowFn = (WindowFn<Object, ?>) sec.getInput(transform).getWindowingStrategy().getWindowFn();
                @SuppressWarnings("unchecked")
                JavaDStream<WindowedValue<InputT>> dStream = (JavaDStream<WindowedValue<InputT>>) sec.getStream(transform);
                JavaDStream<WindowedValue<OutputT>> outStream = dStream.transform(new Function<JavaRDD<WindowedValue<InputT>>, JavaRDD<WindowedValue<OutputT>>>() {
        ...
        return new TransformEvaluator<ParDo.BoundMulti<InputT, OutputT>>() {
            ...
            public void evaluate(final ParDo.BoundMulti<InputT, OutputT> transform, final EvaluationContext context) {
                ...
                for (Map.Entry<TupleTag<?>, PCollection<?>> e : pct.getAll().entrySet()) {
                    @SuppressWarnings("unchecked")
                    JavaPairDStream<TupleTag<?>, WindowedValue<?>> filtered = all.filter(new TranslationUtils.TupleTagFilter(e.getKey()));
                    @SuppressWarnings("unchecked")
                    JavaDStream<WindowedValue<Object>> values = (JavaDStream<WindowedValue<Object>>) (JavaDStream<?>) TranslationUtils.dStreamValues(filtered);
    ...
    private static final Map<Class<? extends PTransform>, TransformEvaluator<?>> EVALUATORS = Maps.newHashMap();<<<<<<< MINE
@SuppressWarnings("unchecked")
    private static <TransformT extends PTransform<?, ?>> TransformEvaluator<TransformT> getTransformEvaluator(Class<TransformT> clazz, SparkPipelineTranslator rddTranslator) {
        TransformEvaluator<TransformT> transform = (TransformEvaluator<TransformT>) EVALUATORS.get(clazz);
        if (transform == null) {
            if (UNSUPPORTED_EVALUATORS.contains(clazz)) {
                throw new UnsupportedOperationException("Beam transformation " + clazz.getCanonicalName() + " is currently unsupported by the Spark streaming pipeline");
            }
...
>>>>>>> YOURS
    ...
    public static class Translator implements SparkPipelineTranslator {
        ...
        public <TransformT extends PTransform<?, ?>> TransformEvaluator<TransformT> translateBounded(Class<TransformT> clazz) {
            ...
            checkState(transformEvaluator != null, "No TransformEvaluator registered for BOUNDED transform %s", clazz);
        ...
        public <TransformT extends PTransform<?, ?>> TransformEvaluator<TransformT> translateUnbounded(Class<TransformT> clazz) {
            @SuppressWarnings("unchecked")
            TransformEvaluator<TransformT> transformEvaluator = (TransformEvaluator<TransformT>) EVALUATORS.get(clazz);
            checkState(transformEvaluator != null, "No TransformEvaluator registered for for UNBOUNDED transform %s", clazz);
            return transformEvaluator;
        }
    }
}