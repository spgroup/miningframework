...
public class cgBase {
    ...
    public static StatusCode login() {
        ...
        clearCookies();
        final Parameters params = new Parameters("__EVENTTARGET", "", "__EVENTARGUMENT", "", "ctl00$SiteContent$tbUsername", login.left, "ctl00$SiteContent$tbPassword", login.right, "ctl00$SiteContent$cbRememberMe", "on", "ctl00$SiteContent$btnSignIn", "Login");
...
>>>>>>> YOURS
    ...
    private static void loadLogsFromDetails(final String page, final cgCache cache) {
        ...
        final Parameters params = new Parameters("tkn", userToken, "idx", "1", "num", "35", "decrypt", "true");
    ...
    public static List<cgUser> getGeocachersInViewport(final String username, final Double latMin, final Double latMax, final Double lonMin, final Double lonMax) {
        ...
        final Parameters params = new Parameters("u", username, "ltm", String.format((Locale) null, "%.6f", latMin), "ltx", String.format((Locale) null, "%.6f", latMax), "lnm", String.format((Locale) null, "%.6f", lonMin), "lnx", String.format((Locale) null, "%.6f", lonMax));
    ...
    public static List<cgCache> processSearchResults(final cgSearch search, final cgCacheWrap caches, final boolean excludeDisabled, final boolean excludeMine, final String cacheType) {
        ...
        if (caches != null) {
            ...
            if (CollectionUtils.isNotEmpty(caches.cacheList)) {
                for (cgCache cache : caches.cacheList) {
                    if ((!excludeDisabled || (excludeDisabled && cache.disabled == false)) && (!excludeMine || (excludeMine && cache.own == false)) && (!excludeMine || (excludeMine && cache.found == false)) && (cacheType == null || (cacheType.equals(cache.type)))) {
    ...
    public static StatusCode postLog(final cgeoapplication app, final String geocode, final String cacheid, final String[] viewstates, final int logType, final int year, final int month, final int day, final String log, final List<cgTrackableLog> trackables) {
        if (isEmpty(viewstates)) {
    ...
    public static StatusCode postLogTrackable(final String tbid, final String trackingCode, final String[] viewstates, final int logType, final int year, final int month, final int day, final String log) {
        if (isEmpty(viewstates)) {
    ...
    public static Double getElevation(final Geopoint coords) {
        try {
            ...
            final Parameters params = new Parameters("sensor", "false", "locations", String.format((Locale) null, "%.6f", coords.getLatitude()) + "," + String.format((Locale) null, "%.6f", coords.getLongitude()));
