...
public class AccessControlTest {
    ...
    private class LookupCase implements Comparable<LookupCase> {
        ...
        public boolean willAccess(Method m) {
            ...
            if (isLooseModule(c1.getModule()) && (lookupModes & PUBLIC) != 0 && (!c2.getModule().isNamed()) && Modifier.isPublic(c2.getModifiers()) && Modifier.isPublic(m.getModifiers()))
                return true;
        ...
        public boolean willAccessClass(Class<?> c2, boolean load) {
            ...
            if (isLooseModule(c1.getModule()) && (lookupModes & PUBLIC) != 0 && (!c2.getModule().isNamed()) && Modifier.isPublic(c2.getModifiers()))
                return true;
            LookupCase lc = this.in(c2);
            int m1 = lc.lookupModes();
            boolean r = false;
