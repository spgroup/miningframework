...
public class LockStep {
    ...
    static boolean maybe(int n) {
        return rnd.nextInt(n) == 0;
    }

    static void realMain(String[] args) {
        size = intArg(args, 0, DEFAULT_SIZE);
        lockSteps(new TreeMap(), new ConcurrentSkipListMap());
        lockSteps(new TreeMap(), Collections.checkedNavigableMap(new TreeMap(), Integer.class, Integer.class));
        lockSteps(new TreeMap(), Collections.synchronizedNavigableMap(new TreeMap()));
        lockSteps(new TreeMap(reverseOrder()), new ConcurrentSkipListMap(reverseOrder()));
        lockSteps(new TreeSet(), new ConcurrentSkipListSet());
        lockSteps(new TreeSet(), Collections.checkedNavigableSet(new TreeSet(), Integer.class));
        lockSteps(new TreeSet(), Collections.synchronizedNavigableSet(new TreeSet()));
        lockSteps(new TreeSet(reverseOrder()), new ConcurrentSkipListSet(reverseOrder()));
    }
    ...
    static void lockSteps(NavigableMap m1, NavigableMap m2) {
        ...
        if (maybe(4))
            m2 = serialClone(m2);
        lockStep(m1, m2);
        lockStep(m1.descendingMap(), m2.descendingMap());
        lockStep(fullSubMap(m1), fullSubMap(m2));
        lockStep(fullSubMap(m1.descendingMap()), fullSubMap(m2.descendingMap()));
        lockStep(fullHeadMap(m1), fullHeadMap(m2));
        lockStep(fullHeadMap(m1.descendingMap()), fullHeadMap(m2.descendingMap()));
        lockStep(fullTailMap(m1), fullTailMap(m2));
        lockStep(fullTailMap(m1.descendingMap()), fullTailMap(m2.descendingMap()));
    }
    ...
    static void lockSteps(NavigableSet s1, NavigableSet s2) {
        ...
        if (maybe(4))
            s2 = serialClone(s2);
        lockStep(s1, s2);
        lockStep(s1.descendingSet(), s2.descendingSet());
        lockStep(fullSubSet(s1), fullSubSet(s2));
        lockStep(fullSubSet(s1.descendingSet()), fullSubSet(s2.descendingSet()));
        lockStep(fullHeadSet(s1), fullHeadSet(s2));
        lockStep(fullHeadSet(s1.descendingSet()), fullHeadSet(s2.descendingSet()));
        lockStep(fullTailSet(s1), fullTailSet(s2));
        lockStep(fullTailSet(s1.descendingSet()), fullTailSet(s2.descendingSet()));
    }
    ...
    static NavigableMap fullSubMap(NavigableMap m) {
        return isAscending(m) ? m.subMap(Integer.MIN_VALUE, true, Integer.MAX_VALUE, true) : m.subMap(Integer.MAX_VALUE, true, Integer.MIN_VALUE, true);
    }

    static NavigableMap fullHeadMap(NavigableMap m) {
        return isAscending(m) ? m.headMap(Integer.MAX_VALUE, true) : m.headMap(Integer.MIN_VALUE, true);
    }

    static NavigableMap fullTailMap(NavigableMap m) {
        return isAscending(m) ? m.tailMap(Integer.MIN_VALUE, true) : m.tailMap(Integer.MAX_VALUE, true);
    }
    ...
    static NavigableSet fullSubSet(NavigableSet s) {
        return isAscending(s) ? s.subSet(Integer.MIN_VALUE, true, Integer.MAX_VALUE, true) : s.subSet(Integer.MAX_VALUE, true, Integer.MIN_VALUE, true);
    }

    static NavigableSet fullHeadSet(NavigableSet s) {
        return isAscending(s) ? s.headSet(Integer.MAX_VALUE, true) : s.headSet(Integer.MIN_VALUE, true);
    }

    static NavigableSet fullTailSet(NavigableSet s) {
        return isAscending(s) ? s.tailSet(Integer.MIN_VALUE, true) : s.tailSet(Integer.MAX_VALUE, true);
    }
    ...
    static void checkNavigableSet(final NavigableSet s) {
        ...
        Iterator[] its = new Iterator[] { s.iterator(), s.descendingSet().descendingSet().iterator() };
        for (final Iterator it : its) if (maybe(4))
            THROWS(IllegalStateException.class, new Fun() {
    ...
    static void equalIterators(final Iterator<?> it1, final Iterator<?> it2) {
        while (it1.hasNext()) {
            if (maybe(2))
                check(it2.hasNext());
    ...
    static void equalNavigableSetsLeaf(final NavigableSet s1, final NavigableSet s2) {
        equal2(s1, s2);
        equal(s1.size(), s2.size());
        equal(s1.isEmpty(), s2.isEmpty());
        equal(s1.hashCode(), s2.hashCode());
    ...
    static void equalNavigableSets(final NavigableSet s1, final NavigableSet s2) {
        ...
        if (s1.comparator() != null && s1.comparator().compare(min, max) > 0) {
            Object tmp = min;
            min = max;
            max = tmp;
        }
        equalNavigableSetsLeaf(s1.subSet(min, true, max, true), s2.subSet(min, true, max, true));
        equalNavigableSetsLeaf(s1.tailSet(min, true), s2.tailSet(min, true));
        equalNavigableSetsLeaf(s1.headSet(max, true), s2.headSet(max, true));
        equalNavigableSetsLeaf((NavigableSet) s1.subSet(min, max), (NavigableSet) s2.subSet(min, max));
        equalNavigableSetsLeaf((NavigableSet) s1.tailSet(min), (NavigableSet) s2.tailSet(min));
        equalNavigableSetsLeaf((NavigableSet) s1.headSet(max), (NavigableSet) s2.headSet(max));
    }

    static <T> T[] concat(T[]... arrays) {
        int len = 0;
        for (int i = 0; i < arrays.length; i++) len += arrays[i].length;
        T[] a = (T[]) java.lang.reflect.Array.newInstance(arrays[0].getClass().getComponentType(), len);
        int k = 0;
    ...
    static void checkNavigableMap(final NavigableMap m) {
        ...
        Iterator[] kits = new Iterator[] { m.keySet().iterator(), m.descendingMap().descendingKeySet().iterator(), m.descendingKeySet().descendingSet().iterator() };
        Iterator[] vits = new Iterator[] { m.values().iterator(), m.descendingMap().descendingMap().values().iterator() };
        Iterator[] eits = new Iterator[] { m.entrySet().iterator(), m.descendingMap().descendingMap().entrySet().iterator() };
    ...
    static void equalNavigableMapsLeaf(final NavigableMap m1, final NavigableMap m2) {
        equal2(m1, m2);
    ...
    static void equalNavigableMaps(NavigableMap m1, NavigableMap m2) {
        equalNavigableMapsLeaf(m1, m2);
        equalSetsLeaf(m1.keySet(), m2.keySet());
        equalNavigableSets(m1.navigableKeySet(), m2.navigableKeySet());
        equalNavigableSets(m1.descendingKeySet(), m2.descendingKeySet());
        equal2(m1.entrySet(), m2.entrySet());
        equalNavigableMapsLeaf(m1.descendingMap(), m2.descendingMap());
        equalNavigableMapsLeaf(m1.descendingMap().descendingMap(), m2);
        equalNavigableSetsLeaf((NavigableSet) m1.descendingMap().keySet(), (NavigableSet) m2.descendingMap().keySet());
        equalNavigableSetsLeaf(m1.descendingMap().descendingKeySet(), m2.descendingMap().descendingKeySet());
        equal2(m1.descendingMap().entrySet(), m2.descendingMap().entrySet());
        Object min = Integer.MIN_VALUE;
        Object max = Integer.MAX_VALUE;
        if (m1.comparator() != null && m1.comparator().compare(min, max) > 0) {
            Object tmp = min;
            min = max;
            max = tmp;
        }
        switch(rnd.nextInt(6)) {
            case 0:
                equalNavigableMapsLeaf(m1.subMap(min, true, max, true), m2.subMap(min, true, max, true));
                break;
            case 1:
                equalNavigableMapsLeaf(m1.tailMap(min, true), m2.tailMap(min, true));
                break;
            case 2:
                equalNavigableMapsLeaf(m1.headMap(max, true), m2.headMap(max, true));
                break;
            case 3:
                equalNavigableMapsLeaf((NavigableMap) m1.subMap(min, max), (NavigableMap) m2.subMap(min, max));
                break;
            case 4:
                equalNavigableMapsLeaf((NavigableMap) m1.tailMap(min), (NavigableMap) m2.tailMap(min));
                break;
            case 5:
                equalNavigableMapsLeaf((NavigableMap) m1.headMap(max), (NavigableMap) m2.headMap(max));
                break;
        }
    }

    static abstract class MapFrobber {

        abstract void frob(NavigableMap m);
    }

    static abstract class SetFrobber {

        abstract void frob(NavigableSet m);
    }

    static MapFrobber randomAdder(NavigableMap m) {
        final Integer k = unusedKey(m);
        final MapFrobber[] randomAdders = { new MapFrobber() {

            void frob(NavigableMap m) {
    ...
    static SetFrobber randomAdder(NavigableSet s) {
        ...
        final SetFrobber[] randomAdders = { new SetFrobber() {

            void frob(NavigableSet s) {
                check(s.add(e));
            }
        }, new SetFrobber() {
    ...
    static MapFrobber randomRemover(NavigableMap m) {
        ...
        final MapFrobber[] randomRemovers = { new MapFrobber() {
            ...
            void frob(NavigableMap m) {
                ...
                while (it.hasNext()) if (it.next().equals(k)) {
                    it.remove();
                    if (maybe(2))
                        THROWS(IllegalStateException.class, new Fun() {
            ...
            void frob(NavigableMap m) {
                ...
                while (it.hasNext()) if (it.next().getKey().equals(k)) {
                    it.remove();
                    if (maybe(2))
    ...
    static SetFrobber randomRemover(NavigableSet s) {
        ...
        final SetFrobber[] randomRemovers = { new SetFrobber() {
            ...
            void frob(NavigableSet s) {
                ...
                while (it.hasNext()) if (it.next().equals(e)) {
                    it.remove();
                    if (maybe(2))
                        THROWS(IllegalStateException.class, new Fun() {
    ...
    static void lockStep(NavigableMap m1, NavigableMap m2) {
        if (!(thorough || maybe(3)))
            return;
        if (maybe(4))
            m1 = serialClone(m1);
        ...
        while (ints.size() < size) ints.add(rnd.nextInt(1024));
    ...
    static void lockStep(NavigableSet s1, NavigableSet s2) {
        if (!(thorough || maybe(3)))
            return;
        if (maybe(4))
            s1 = serialClone(s1);
