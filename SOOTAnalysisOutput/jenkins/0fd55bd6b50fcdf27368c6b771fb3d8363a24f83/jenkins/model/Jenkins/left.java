...
public class Jenkins extends AbstractCIBase implements DirectlyModifiableTopLevelItemGroup, StaplerProxy, StaplerFallback, ModifiableViewGroup, AccessControlled, DescriptorByNameOwner, ModelObjectWithContextMenu, ModelObjectWithChildren, OnMaster {
                ...
                if (c != null) {
                    for (ComputerListener cl : ComputerListener.all()) {
                        try {
                            ...
                            LOGGER.log(WARNING, String.format("Exception in onOnline() for the computer listener %s on the Jenkins master node", cl.getClass()), e);
                        }
                    }
                }
            }
    ...
    public static String expandVariablesForDirectory(String base, String itemFullName, String itemRootDir) {
        return Util.replaceMacro(base, ImmutableMap.of("JENKINS_HOME", Jenkins.get().getRootDir().getPath(), "ITEM_ROOTDIR", itemRootDir, "ITEM_FULLNAME", itemFullName, "ITEM_FULL_NAME", itemFullName.replace(':', '$')));
    }
    ...
    private static final Set<String> ALWAYS_READABLE_PATHS = new HashSet<>(ImmutableSet.of("login", "loginError", "logout", "accessDenied", "adjuncts", "error", "oops", "signup", "tcpSlaveAgentListener", "federatedLoginService", "securityRealm", "instance-identity"));
