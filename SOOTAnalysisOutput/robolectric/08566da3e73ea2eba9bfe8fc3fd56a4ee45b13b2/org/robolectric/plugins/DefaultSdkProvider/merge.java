...
public class DefaultSdkProvider implements SdkProvider {
    ...
    private static void addSdk(int apiLevel, String androidVersion, String frameworkSdkBuildVersion, String codeName, int requiredJavaVersion) {
        SdkVersion sdkVersion = new SdkVersion(apiLevel, androidVersion, frameworkSdkBuildVersion, codeName, requiredJavaVersion);
        Setup.knownApis.put(apiLevel, sdkVersion);
        if (sdkVersion.isSupportedByRuntime()) {
            Setup.supportedApis.put(apiLevel, sdkVersion);
        } else {
            System.err.printf("[Robolectric] WARN: %s. Tests won't be run on this SDK unless explicitly requested\n", sdkVersion.getUnsupportedMessage());
        }
    }
    ...
    private static final class SdkVersion implements Comparable<SdkVersion> {
        ...
        SdkVersion(int apiLevel, String androidVersion, String robolectricVersion, String codeName, int requiredJavaVersion) {
        ...
        private String getUnsupportedMessage() {
            return String.format(Locale.getDefault(), "Android SDK %d requires Java %d (have Java %d)", apiLevel, requiredJavaVersion, RUNNING_JAVA_VERSION);
        }
        ...
        public boolean isEqualTo(SdkVersion that) {
            return that == this || (Objects.equals(that.androidVersion, androidVersion) && Objects.equals(that.robolectricVersion, robolectricVersion));
        }
    ...
    private static class UnknownSdkConfig extends SdkConfig {
        ...
        public DependencyJar getAndroidSdkDependency() {
            throw new IllegalArgumentException(String.format("Robolectric does not support API level %d.", getApiLevel()));
        }
