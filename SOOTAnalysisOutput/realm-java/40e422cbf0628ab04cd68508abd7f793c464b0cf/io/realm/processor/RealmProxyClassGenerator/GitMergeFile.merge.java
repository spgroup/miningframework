...
>>>>>>> YOURS
        ...
        for (VariableElement field : metadata.getFields()) {
            ...
            if (JAVA_TO_REALM_TYPES.containsKey(fieldTypeCanonicalName)) {
                ...
                writer.emitStatement("throw new IllegalStateException(\"Mismatching link tables for column '%s'\")", fieldName);
                writer.endControlFlow();
            } else if (typeUtils.isAssignable(field.asType(), realmList)) {
                String genericType = Utils.getGenericType(field);
                writer.beginControlFlow("if (!columnTypes.containsKey(\"%s\"))", fieldName);
...
>>>>>>> YOURS
                ...
                writer.beginControlFlow("if (!table.getLinkTarget(%s).equals(table_%d))", staticFieldIndexVarName(field), fieldIndex);
                writer.emitStatement("throw new IllegalStateException(\"Mismatching link list tables for column '%s'\")", fieldName);
                writer.endControlFlow();
            }
            fieldIndex++;
        }
