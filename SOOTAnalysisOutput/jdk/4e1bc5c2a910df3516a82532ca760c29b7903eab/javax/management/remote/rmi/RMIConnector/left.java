...
public class RMIConnector implements JMXConnector, Serializable, JMXAddressable {
    ...
    private class RemoteMBeanServerConnection implements MBeanServerConnection {
        ...
        public ObjectInstance createMBean(String className, ObjectName name, Object[] params, String[] signature) throws ReflectionException, InstanceAlreadyExistsException, MBeanRegistrationException, MBeanException, NotCompliantMBeanException, IOException {
            if (logger.debugOn())
                logger.debug("createMBean(String,ObjectName,Object[],String[])", "className=" + className + ", name=" + name + ", signature=" + strings(signature));
        ...
        public ObjectInstance createMBean(String className, ObjectName name, ObjectName loaderName, Object[] params, String[] signature) throws ReflectionException, InstanceAlreadyExistsException, MBeanRegistrationException, MBeanException, NotCompliantMBeanException, InstanceNotFoundException, IOException {
            if (logger.debugOn())
                logger.debug("createMBean(String,ObjectName,ObjectName,Object[],String[])", "className=" + className + ", name=" + name + ", loaderName=" + loaderName + ", signature=" + strings(signature));
            final MarshalledObject<Object[]> sParams = new MarshalledObject<Object[]>(params);
            final ClassLoader old = pushDefaultClassLoader();
            try {
        ...
        public AttributeList setAttributes(ObjectName name, AttributeList attributes) throws InstanceNotFoundException, ReflectionException, IOException {
            if (logger.debugOn()) {
                logger.debug("setAttributes", "name=" + name + ", attribute names=" + getAttributesNames(attributes));
            }
        ...
        public Object invoke(ObjectName name, String operationName, Object[] params, String[] signature) throws InstanceNotFoundException, MBeanException, ReflectionException, IOException {
            if (logger.debugOn())
                logger.debug("invoke", "name=" + name + ", operationName=" + operationName + ", signature=" + strings(signature));
            final MarshalledObject<Object[]> sParams = new MarshalledObject<Object[]>(params);
            final ClassLoader old = pushDefaultClassLoader();
            try {
    ...
    private RMIServer findRMIServerJNDI(String jndiURL, Map<String, ?> env) throws NamingException {
    ...
    private RMIServer findRMIServerJRMP(String base64, Map<String, ?> env) throws IOException {
    ...
    private static final String pRefClassName = "jdk.jmx.remote.internal.rmi.PRef";
    ...
    static {
        ...
        PrivilegedExceptionAction<Constructor<?>> action = new PrivilegedExceptionAction<Constructor<?>>() {
            ...
            public Constructor<?> run() throws Exception {
                ...
                ClassLoader cl = new NoCallStackClassLoader(pRefClassName, pRefByteCode, new String[] { proxyRefCName }, thisLoader, thisProtectionDomain);
                ...
                ModuleDescriptor descriptor = ModuleDescriptor.newModule("jdk.remoteref", Set.of(SYNTHETIC)).packages(Set.of(pkg)).build();
    ...
    static String getAttributesNames(AttributeList attributes) {
        return attributes != null ? attributes.asList().stream().map(Attribute::getName).collect(Collectors.joining(", ", "[", "]")) : "[]";
    }
}
