...
public class ObjectStore implements RawStore, Configurable {
    ...
    public AggrStats get_aggr_stats_for(String dbName, String tblName, final List<String> partNames, final List<String> colNames) throws MetaException, NoSuchObjectException {
        ...
        return new GetHelper<AggrStats>(dbName, tblName, true, false) {
            ...
            protected AggrStats getSqlResult(GetHelper<AggrStats> ctx) throws MetaException {
                return directSql.aggrColStatsForPartitions(dbName, tblName, partNames, colNames, useDensityFunctionForNDVEstimation, ndvTuner);
            }
    ...
    @Override
    public Map<String, ColumnStatisticsObj> getAggrColStatsForTablePartitions(String dbName, String tableName) throws MetaException, NoSuchObjectException {
        final boolean useDensityFunctionForNDVEstimation = HiveConf.getBoolVar(getConf(), HiveConf.ConfVars.HIVE_METASTORE_STATS_NDV_DENSITY_FUNCTION);
        final double ndvTuner = HiveConf.getFloatVar(getConf(), HiveConf.ConfVars.HIVE_METASTORE_STATS_NDV_TUNER);
        return new GetHelper<Map<String, ColumnStatisticsObj>>(dbName, tableName, true, false) {

            @Override
            protected Map<String, ColumnStatisticsObj> getSqlResult(GetHelper<Map<String, ColumnStatisticsObj>> ctx) throws MetaException {
                return directSql.getAggrColStatsForTablePartitions(dbName, tblName, useDensityFunctionForNDVEstimation, ndvTuner);
            }

            @Override
            protected Map<String, ColumnStatisticsObj> getJdoResult(GetHelper<Map<String, ColumnStatisticsObj>> ctx) throws MetaException, NoSuchObjectException {
                throw new MetaException("Jdo path is not implemented for stats aggr.");
            }
...
<<<<<<< MINE
    ...
    public MTableWrite getTableWrite(String dbName, String tblName, long writeId) throws MetaException {
        ...
        openTransaction();
        try {
            query = pm.newQuery(MTableWrite.class, "table.tableName == t1 && table.database.name == t2 && writeId == t3");
            ...
            if (writes == null || writes.isEmpty())
                return null;
            if (writes.size() > 1) {
                throw new MetaException("More than one TableWrite for " + dbName + "." + tblName + " and " + writeId);
            }
            return writes.get(0);
        } finally {
            closeTransaction(success, query);
    ...
    public List<Long> getTableWriteIds(String dbName, String tblName, long watermarkId, long nextWriteId, char state) throws MetaException {
        ...
        try {
            ...
            query = pm.newQuery("select writeId from org.apache.hadoop.hive.metastore.model.MTableWrite" + " where table.tableName == t1 && table.database.name == t2 && writeId > t3" + " && writeId < t4" + (hasState ? " && state == t5" : ""));
            query.declareParameters("java.lang.String t1, java.lang.String t2, java.lang.Long t3, " + "java.lang.Long t4" + (hasState ? ", java.lang.String t5" : ""));
            query.setResult("writeId");
            query.setOrdering("writeId asc");
            @SuppressWarnings("unchecked")
            List<Long> writes = (List<Long>) (hasState ? query.executeWithArray(tblName, dbName, watermarkId, nextWriteId, String.valueOf(state)) : query.executeWithArray(tblName, dbName, watermarkId, nextWriteId));
            success = true;
    ...
    public List<MTableWrite> getTableWrites(String dbName, String tblName, long from, long to) throws MetaException {
        ...
        openTransaction();
        try {
            String queryStr = "table.tableName == t1 && table.database.name == t2 && writeId > t3", argStr = "java.lang.String t1, java.lang.String t2, java.lang.Long t3";
            if (to >= 0) {
                queryStr += " && writeId < t4";
            ...
            query.setOrdering("writeId asc");
            @SuppressWarnings("unchecked")
            List<MTableWrite> writes = (List<MTableWrite>) (to >= 0 ? query.executeWithArray(tblName, dbName, from, to) : query.executeWithArray(tblName, dbName, from));
            pm.retrieveAll(writes);
            success = true;
    ...
    public void deleteTableWrites(String dbName, String tblName, long from, long to) throws MetaException {
        ...
        openTransaction();
        try {
            query = pm.newQuery(MTableWrite.class, "table.tableName == t1 && table.database.name == t2 && writeId > t3 && writeId < t4");
            query.declareParameters("java.lang.String t1, java.lang.String t2, java.lang.Long t3, java.lang.Long t4");
    ...
    public Collection<String> getAllPartitionLocations(String dbName, String tblName) {
        ...
        openTransaction();
        try {
            String q = "select sd.location from org.apache.hadoop.hive.metastore.model.MPartition" + " where table.tableName == t1 && table.database.name == t2";
            query = pm.newQuery();
            query.declareParameters("java.lang.String t1, java.lang.String t2");
            @SuppressWarnings("unchecked")
