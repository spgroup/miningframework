...
public class OSecurityManager {
    ...
    public static String createHash(final String iInput, String iAlgorithm) throws NoSuchAlgorithmException, UnsupportedEncodingException {
        if (iAlgorithm == null)
    ...
    private static boolean compareHash(final byte[] iFirst, final byte[] iSecond) {
        ...
        for (int i = 0; i < iFirst.length && i < iSecond.length; i++) diff |= iFirst[i] ^ iSecond[i];
        return diff == 0;
    }
    ...
    private static byte[] hexToByteArray(final String data) {
        ...
        for (int i = 0; i < hex.length; i++) hex[i] = (byte) Integer.parseInt(data.substring(2 * i, 2 * i + 2), 16);
        return hex;
    }
}
