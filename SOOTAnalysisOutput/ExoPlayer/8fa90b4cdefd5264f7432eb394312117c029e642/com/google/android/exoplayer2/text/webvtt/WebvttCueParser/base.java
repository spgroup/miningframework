...
public final class WebvttCueParser {
    ...
    private static void applyRubySpans(SpannableStringBuilder text, @Nullable String cueId, StartTag startTag, List<Element> nestedElements, List<WebvttCssStyle> styles) {
        ...
        for (int i = 0; i < sortedNestedElements.size(); i++) {
            ...
            @RubySpan.Position
            int rubyPosition = firstKnownRubyPosition(getRubyPosition(styles, cueId, rubyTextElement.startTag), rubyTagPosition, RubySpan.POSITION_OVER);
    ...
    @RubySpan.Position
    private static int firstKnownRubyPosition(@RubySpan.Position int position1, @RubySpan.Position int position2, @RubySpan.Position int position3) {
    ...
    private static void applyStyleToText(SpannableStringBuilder spannedText, WebvttCssStyle style, int start, int end) {
        ...
        if (style.getStyle() != WebvttCssStyle.UNSPECIFIED) {
            addOrReplaceSpan(spannedText, new StyleSpan(style.getStyle()), start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
        ...
        if (style.hasFontColor()) {
            addOrReplaceSpan(spannedText, new ForegroundColorSpan(style.getFontColor()), start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
        if (style.hasBackgroundColor()) {
            addOrReplaceSpan(spannedText, new BackgroundColorSpan(style.getBackgroundColor()), start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
        if (style.getFontFamily() != null) {
            addOrReplaceSpan(spannedText, new TypefaceSpan(style.getFontFamily()), start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
        switch(style.getFontSizeUnit()) {
            case WebvttCssStyle.FONT_SIZE_UNIT_PIXEL:
                addOrReplaceSpan(spannedText, new AbsoluteSizeSpan((int) style.getFontSize(), true), start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
                break;
            case WebvttCssStyle.FONT_SIZE_UNIT_EM:
                addOrReplaceSpan(spannedText, new RelativeSizeSpan(style.getFontSize()), start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
                break;
            case WebvttCssStyle.FONT_SIZE_UNIT_PERCENT:
                addOrReplaceSpan(spannedText, new RelativeSizeSpan(style.getFontSize() / 100), start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
                break;
