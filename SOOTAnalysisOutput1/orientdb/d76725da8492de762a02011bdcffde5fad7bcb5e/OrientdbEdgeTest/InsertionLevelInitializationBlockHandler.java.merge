...
public class OrientdbEdgeTest {
    ...
    protected static OrientGraphFactory getGraphFactory() throws Exception {
        ...
        return new OrientGraphFactory(conf.getString("storage.url"), conf.getString("storage.user"), conf.getString("storage.password")).setupPool(conf.getInt("storage.pool-min"), conf.getInt("storage.pool-max"));
    }
