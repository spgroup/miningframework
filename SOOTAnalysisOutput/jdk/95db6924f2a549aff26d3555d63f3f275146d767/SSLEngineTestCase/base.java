...
abstract public class SSLEngineTestCase {
    ...
    public enum Ciphers {
        ...
        SUPPORTED_NON_KRB_CIPHERS(SSLEngineTestCase.SUPPORTED_NON_KRB_CIPHERS, "Supported non kerberos"),
        SUPPORTED_NON_KRB_NON_SHA_CIPHERS(SSLEngineTestCase.SUPPORTED_NON_KRB_NON_SHA_CIPHERS, "Supported non kerberos non SHA256 and SHA384"),
        SUPPORTED_KRB_CIPHERS(SSLEngineTestCase.SUPPORTED_KRB_CIPHERS, "Supported kerberos"),
        ENABLED_NON_KRB_NOT_ANON_CIPHERS(SSLEngineTestCase.ENABLED_NON_KRB_NOT_ANON_CIPHERS, "Enabled by default non kerberos not anonymous"),
        TLS13_CIPHERS(SSLEngineTestCase.TLS13_CIPHERS, "Supported by TLS 1.3 only"),
        UNSUPPORTED_CIPHERS(SSLEngineTestCase.UNSUPPORTED_CIPHERS, "Unsupported");
    ...
    public enum HandshakeMode {
        ...
        INITIAL_HANDSHAKE, REHANDSHAKE_BEGIN_CLIENT, REHANDSHAKE_BEGIN_SERVER
    }

    public static final String TESTED_SECURITY_PROTOCOL = System.getProperty("test.security.protocol", "TLS");

    public static final String TEST_MODE = System.getProperty("test.mode", "norm");
    ...
    private static final String KEY_FILE_NAME = System.getProperty("test.src", ".") + FS + PATH_TO_STORES + FS + KEY_STORE_FILE;

    private static final String TRUST_FILE_NAME = System.getProperty("test.src", ".") + FS + PATH_TO_STORES + FS + TRUST_STORE_FILE;
    ...
    private static final String JAAS_CONF_FILE = PATH_TO_COMMON + FS + "jaas.conf";
    ...
    private static final String[] TLS13_CIPHERS = { "TLS_AES_256_GCM_SHA384", "TLS_AES_128_GCM_SHA256" };

    private static final String[] SUPPORTED_NON_KRB_CIPHERS;

    static {
        try {
            String[] allSupportedCiphers = getContext().createSSLEngine().getSupportedCipherSuites();
            List<String> supportedCiphersList = new LinkedList<>();
            for (String cipher : allSupportedCiphers) {
                if (!cipher.contains("KRB5") && !isTLS13Cipher(cipher) && !cipher.contains("TLS_EMPTY_RENEGOTIATION_INFO_SCSV")) {
    ...
    static {
        try {
            ...
            List<String> supportedCiphersList = new LinkedList<>();
            for (String cipher : allSupportedCiphers) {
                if (!cipher.contains("KRB5") && !isTLS13Cipher(cipher) && !cipher.contains("TLS_EMPTY_RENEGOTIATION_INFO_SCSV") && !cipher.endsWith("_SHA256") && !cipher.endsWith("_SHA384")) {
                    supportedCiphersList.add(cipher);
                }
            }
            SUPPORTED_NON_KRB_NON_SHA_CIPHERS = supportedCiphersList.toArray(new String[0]);
        } catch (Exception ex) {
            throw new Error("Unexpected issue", ex);
        }
    }
    ...
    static {
        try {
            ...
            for (String cipher : enabledCiphers) {
                if (!cipher.contains("anon") && !cipher.contains("KRB5") && !isTLS13Cipher(cipher) && !cipher.contains("TLS_EMPTY_RENEGOTIATION_INFO_SCSV")) {
    ...
    public static ByteBuffer doWrap(SSLEngine engine, String wrapper, int maxPacketSize, ByteBuffer app) throws SSLException {
        return doWrap(engine, wrapper, maxPacketSize, app, SSLEngineResult.Status.OK, null);
    }

    public static ByteBuffer doWrap(SSLEngine engine, String wrapper, int maxPacketSize, ByteBuffer app, SSLEngineResult[] result) throws SSLException {
        return doWrap(engine, wrapper, maxPacketSize, app, SSLEngineResult.Status.OK, result);
    }

    public static ByteBuffer doWrap(SSLEngine engine, String wrapper, int maxPacketSize, ByteBuffer app, SSLEngineResult.Status wantedStatus) throws SSLException {
        return doWrap(engine, wrapper, maxPacketSize, app, wantedStatus, null);
    }

    public static ByteBuffer doWrap(SSLEngine engine, String wrapper, int maxPacketSize, ByteBuffer app, SSLEngineResult.Status wantedStatus, SSLEngineResult[] result) throws SSLException {
        ...
        System.out.println(wrapper + " handshake status is " + engine.getHandshakeStatus() + " Result is " + r.getStatus());
        if (maxPacketSize < length && maxPacketSize != 0) {
            throw new AssertionError("Handshake wrapped net buffer length " + length + " exceeds maximum packet size " + maxPacketSize);
        }
    ...
    public static ByteBuffer doUnWrap(SSLEngine engine, String unwrapper, ByteBuffer net) throws SSLException {
        return doUnWrap(engine, unwrapper, net, SSLEngineResult.Status.OK, null);
    }

    public static ByteBuffer doUnWrap(SSLEngine engine, String unwrapper, ByteBuffer net, SSLEngineResult[] result) throws SSLException {
        return doUnWrap(engine, unwrapper, net, SSLEngineResult.Status.OK, result);
    }

    public static ByteBuffer doUnWrap(SSLEngine engine, String unwrapper, ByteBuffer net, SSLEngineResult.Status wantedStatus) throws SSLException {
        return doUnWrap(engine, unwrapper, net, wantedStatus, null);
    }

    public static ByteBuffer doUnWrap(SSLEngine engine, String unwrapper, ByteBuffer net, SSLEngineResult.Status wantedStatus, SSLEngineResult[] result) throws SSLException {
        ...
        app.flip();
        System.out.println(unwrapper + " handshake status is " + engine.getHandshakeStatus() + " Result is " + r.getStatus());
        checkResult(r, wantedStatus);
        if (result != null && result.length > 0) {
            result[0] = r;
        }
    ...
    public static void doHandshake(SSLEngine clientEngine, SSLEngine serverEngine, int maxPacketSize, HandshakeMode mode) throws SSLException {
        doHandshake(clientEngine, serverEngine, maxPacketSize, mode, false);
    }

    public static void doHandshake(SSLEngine clientEngine, SSLEngine serverEngine, int maxPacketSize, HandshakeMode mode, boolean enableReplicatedPacks) throws SSLException {
        System.out.println("=============================================");
    ...
    public static SSLEngineResult sendApplicationData(SSLEngine fromEngine, SSLEngine toEngine) throws SSLException {
        String sender = null;
        ...
        System.out.println("=============================================");
        System.out.println("Trying to send application data from " + sender + " to " + reciever);
        ByteBuffer clientAppSent = ByteBuffer.wrap(excMsgSent.getBytes());
        ...
        byte[] serverAppRecvTrunc = Arrays.copyOf(serverAppRecv.array(), serverAppRecv.limit());
        String msgRecv = new String(serverAppRecvTrunc);
        if (!msgRecv.equals(excMsgSent)) {
            throw new AssertionError(sender + " to " + reciever + ": application data" + " has been altered while sending." + " Message sent: " + "\"" + excMsgSent + "\"." + " Message recieved: " + "\"" + msgRecv + "\".");
        }
        System.out.println("Successful sending application data from " + sender + " to " + reciever);
        return r[0];
    }
    ...
    public static void closeEngines(SSLEngine fromEngine, SSLEngine toEngine) throws SSLException {
        ...
        if (!toEngine.isInboundDone()) {
            throw new AssertionError(from + " sent close request to " + to + ", but " + to + "did not close inbound.");
        }
        ...
        if (!toEngine.isOutboundDone()) {
            throw new AssertionError(from + "sent close request to " + to + ", but " + to + "did not close outbound.");
        }
    ...
    public void runTests(Ciphers ciphers) {
        ...
        if (failed > 0) {
            throw new AssertionError("" + failed + " of " + total + " tests failed!");
        }
    ...
    public static void checkResult(SSLEngineResult r, SSLEngineResult.Status wantedStatus) {
        SSLEngineResult.Status rs = r.getStatus();
        if (!rs.equals(wantedStatus)) {
            throw new AssertionError("Unexpected status " + rs.name() + ", should be " + wantedStatus.name());
        }
    }
    ...
    public static void setUpAndStartKDC() {
        ...
        System.setProperty("java.security.auth.login.config", TEST_SRC + FS + JAAS_CONF_FILE);
    ...
    abstract protected void testOneCipher(String cipher) throws SSLException;
        ...
        for (String cs : ciphers.ciphers) {
            ...
            switch(ciphers) {
                ...
                case UNSUPPORTED_CIPHERS:
                    if (error == null) {
                        System.out.println("Test Failed: " + cs);
                        System.err.println("Test for " + cs + " should have thrown " + "IllegalArgumentException, but it has not!");
                        failedNum++;
    ...
    private static void handshakeProcess(SSLEngine wrapingEngine, SSLEngine unwrapingEngine, int maxPacketSize, boolean enableReplicatedPacks) throws SSLException {
        HandshakeStatus wrapingHSStatus = wrapingEngine.getHandshakeStatus();
        HandshakeStatus unwrapingHSStatus = unwrapingEngine.getHandshakeStatus();
        SSLEngineResult r;
        String wrapper, unwrapper;
        if (wrapingEngine.getUseClientMode() && !unwrapingEngine.getUseClientMode()) {
            wrapper = "Client";
            unwrapper = "Server";
        } else if (unwrapingEngine.getUseClientMode() && !wrapingEngine.getUseClientMode()) {
            wrapper = "Server";
            unwrapper = "Client";
        } else {
            throw new Error("Both engines are in the same mode");
        }
        System.out.println(wrapper + " handshake (wrap) status " + wrapingHSStatus);
        System.out.println(unwrapper + " handshake (unwrap) status " + unwrapingHSStatus);
        ...
        switch(wrapingHSStatus) {
            case NEED_WRAP:
                ...
                ByteBuffer app = ByteBuffer.allocate(wrapingEngine.getSession().getApplicationBufferSize());
            ...
            case NOT_HANDSHAKING:
                switch(unwrapingHSStatus) {
                    ...
                    case NEED_UNWRAP:
                        ...
                        if (enableReplicatedPacks) {
                            System.out.println(unwrapper + " unwrapping replicated packet...");
                            if (unwrapingEngine.getHandshakeStatus().equals(HandshakeStatus.NEED_TASK)) {
                                runDelegatedTasks(unwrapingEngine);
                            }
                    ...
                    case NOT_HANDSHAKING:
                        if (doUnwrapForNotHandshakingStatus) {
                            ...
                            if (wrapingHSStatus == HandshakeStatus.NOT_HANDSHAKING) {
                    ...
                    case NEED_WRAP:
                        ...
                        net = doWrap(unwrapingEngine, unwrapper, maxPacketSize, b);
                        unwrapingHSStatus = unwrapingEngine.getHandshakeStatus();
                        if ((wrapingHSStatus == HandshakeStatus.NOT_HANDSHAKING) && (unwrapingHSStatus == HandshakeStatus.NOT_HANDSHAKING)) {
                            System.out.println("Handshake is completed");
                            endHandshakeLoop = true;
                        }
                        break;
                    default:
                        throw new Error("Unexpected unwraping engine handshake status " + unwrapingHSStatus.name());
                }
                break;
            case NEED_UNWRAP:
                break;
            ...
            case NEED_TASK:
                ...
                break;
            default:
                throw new Error("Unexpected wraping engine handshake status " + wrapingHSStatus.name());
        }
    }
    ...
    private static void startKDC(String realm, Map<String, String> principals, String ktab) {
        try {
            ...
            if (principals != null) {
                principals.entrySet().stream().forEach((entry) -> {
                    ...
                    if (password == null || password.isEmpty()) {
                        System.out.println("KDC: add a principal '" + name + "' with a random password");
                        kdc.addPrincipalRandKey(name);
                    } else {
                        System.out.println("KDC: add a principal '" + name + "' with '" + password + "' password");
            ...
            System.out.println("KDC: started on " + HOST + ":" + kdc.getPort() + " with '" + realm + "' realm");
        } catch (Exception e) {
