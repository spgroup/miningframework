...
public class CompileCodeTestCase {
    ...
    public NMethod compile(int level) {
        String directive = "[{ match: \"" + executable.getDeclaringClass().getName().replace('.', '/') + "." + (executable instanceof Constructor ? "<init>" : executable.getName()) + "\", " + "BackgroundCompilation: false }]";
    ...
    public static List<CompileCodeTestCase> generate(int bci) {
        ...
        for (Class<?> aClass : CLASSES) {
            ...
            Arrays.stream(aClass.getDeclaredMethods()).filter(m -> !Modifier.isAbstract(m.getModifiers())).filter(m -> !Modifier.isNative(m.getModifiers())).map(m -> new CompileCodeTestCase(receiver, m, bci)).forEach(result::add);
        }
        return result;
    }
    ...
    public InstalledCode toInstalledCode() {
        ...
        return CTVMUtilities.getInstalledCode(executable.getName(), address, entryPoint);
    }
