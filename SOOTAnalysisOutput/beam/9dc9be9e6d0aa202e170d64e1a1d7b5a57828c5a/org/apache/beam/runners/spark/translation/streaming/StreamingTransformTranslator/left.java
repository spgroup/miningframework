    ...
    private static <T> TransformEvaluator<Flatten.FlattenPCollectionList<T>> flattenPColl() {
        return new TransformEvaluator<Flatten.FlattenPCollectionList<T>>() {
            ...
            public void evaluate(Flatten.FlattenPCollectionList<T> transform, EvaluationContext context) {
                ...
                JavaDStream<WindowedValue<T>> first = (JavaDStream<WindowedValue<T>>) sec.getStream(pcs.get(0));
    ...
    private static <TransformT extends PTransform<?, ?>> TransformEvaluator<TransformT> rddTransform(final SparkPipelineTranslator rddTranslator) {
        return new TransformEvaluator<TransformT>() {
            ...
            public void evaluate(TransformT transform, EvaluationContext context) {
                TransformEvaluator<TransformT> rddEvaluator = rddTranslator.translate((Class<TransformT>) transform.getClass());
                StreamingEvaluationContext sec = (StreamingEvaluationContext) context;
                if (sec.hasStream(transform)) {
                    JavaDStreamLike<WindowedValue<Object>, ?, JavaRDD<WindowedValue<Object>>> dStream = (JavaDStreamLike<WindowedValue<Object>, ?, JavaRDD<WindowedValue<Object>>>) sec.getStream(transform);
                    sec.setStream(transform, dStream.transform(new RDDTransform<>(sec, rddEvaluator, transform)));
                } else {
    ...
    private static final class RDDTransform<TransformT extends PTransform<?, ?>> implements Function<JavaRDD<WindowedValue<Object>>, JavaRDD<WindowedValue<Object>>> {
        ...
        public JavaRDD<WindowedValue<Object>> call(JavaRDD<WindowedValue<Object>> rdd) throws Exception {
            ...
            if (!context.hasOutputRDD(transform)) {
                context.setOutputRDD(transform, context.getSparkContext().<WindowedValue<Object>>emptyRDD());
            }
            JavaRDD<WindowedValue<Object>> outRDD = (JavaRDD<WindowedValue<Object>>) context.getOutputRDD(transform);
    ...
    private static <T> TransformEvaluator<Window.Bound<T>> window() {
        return new TransformEvaluator<Window.Bound<T>>() {
            ...
            public void evaluate(Window.Bound<T> transform, EvaluationContext context) {
                ...
                @SuppressWarnings("unchecked")
                JavaDStreamLike<WindowedValue<T>, ?, JavaRDD<WindowedValue<T>>> dstream = (JavaDStreamLike<WindowedValue<T>, ?, JavaRDD<WindowedValue<T>>>) sec.getStream(transform);
