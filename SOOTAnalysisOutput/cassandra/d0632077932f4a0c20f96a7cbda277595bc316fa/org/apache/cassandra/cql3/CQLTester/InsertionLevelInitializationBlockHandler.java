...
=======
        for (Session sess : session) if (sess != null)
            sess.close();
        for (Cluster cl : cluster) if (cl != null)
            cl.close();
>>>>>>> YOURS
        if (server != null)
            server.stop();
...
>>>>>>> YOURS
    ...
    public static FunctionName parseFunctionName(String qualifiedName) {
        ...
        return i == -1 ? FunctionName.nativeFunction(qualifiedName) : new FunctionName(qualifiedName.substring(0, i).trim(), qualifiedName.substring(i + 1).trim());
    }
    ...
    protected void assertLastSchemaChange(Event.SchemaChange.Change change, Event.SchemaChange.Target target, String keyspace, String name, String... argTypes) {
...
>>>>>>> YOURS
    ...
    protected void assertValidSyntax(String query) throws Throwable {
        try {
            ...
            Assert.fail(String.format("Expected query syntax to be valid but was invalid. Query is: %s; Error is %s", query, e.getMessage()));
        }
    }
