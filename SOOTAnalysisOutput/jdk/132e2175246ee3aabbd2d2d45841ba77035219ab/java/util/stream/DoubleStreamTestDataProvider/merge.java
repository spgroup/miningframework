    ...
    static {
        {
            ...
            for (Object[] data : arrays) {
                ...
                list.add(new Object[] { "array:" + name, TestData.Factory.ofArray("array:" + name, doubles) });
        ...
        }
        {
            ...
            for (Object[] data : arrays) {
                ...
                spliterators.add(splitDescr("Arrays.s(array):" + name, () -> Arrays.spliterator(doubles)));
                spliterators.add(splitDescr("Arrays.s(array,o,l):" + name, () -> Arrays.spliterator(doubles, 0, doubles.length / 2)));
                spliterators.add(splitDescr("SpinedBuffer.s():" + name, () -> isl.spliterator()));
                spliterators.add(splitDescr("Primitives.s(SpinedBuffer.iterator(), size):" + name, () -> Spliterators.spliterator(isl.iterator(), doubles.length, 0)));
                spliterators.add(splitDescr("Primitives.s(SpinedBuffer.iterator()):" + name, () -> Spliterators.spliteratorUnknownSize(isl.iterator(), 0)));
                spliterators.add(splitDescr("DoubleStream.iterate(0,x->x<l;x->x+1):" + name, () -> DoubleStream.iterate(0.0, x -> x < doubles.length, x -> x + 1.0).spliterator()));
            }
