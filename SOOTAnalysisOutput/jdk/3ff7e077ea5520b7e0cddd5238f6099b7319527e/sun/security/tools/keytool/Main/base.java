...
public final class Main {
    ...
    private void doPrintEntry(String alias, PrintStream out) throws Exception {
        if (keyStore.containsAlias(alias) == false) {
            MessageFormat form = new MessageFormat(rb.getString("Alias.alias.does.not.exist"));
            Object[] source = { alias };
            throw new Exception(form.format(source));
    ...
    private void printX509Cert(X509Certificate cert, PrintStream out) throws Exception {
        ...
        Object[] source = { cert.getSubjectDN().toString(), cert.getIssuerDN().toString(), cert.getSerialNumber().toString(16), cert.getNotBefore().toString(), cert.getNotAfter().toString(), getCertFingerPrint("SHA-1", cert), getCertFingerPrint("SHA-256", cert), cert.getSigAlgName(), pkey.getAlgorithm(), KeyUtil.getKeySize(pkey), cert.getVersion() };
        out.println(form.format(source));
