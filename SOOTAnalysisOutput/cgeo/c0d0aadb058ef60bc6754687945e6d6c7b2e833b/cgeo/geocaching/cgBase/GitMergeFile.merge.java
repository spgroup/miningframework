...
public class cgBase {
    ...
    public static SimpleDateFormat dateEvIn = new SimpleDateFormat("dd MMMMM yyyy", Locale.ENGLISH);

    public static SimpleDateFormat dateTbIn1 = new SimpleDateFormat("EEEEE, dd MMMMM yyyy", Locale.ENGLISH);

    public static SimpleDateFormat dateTbIn2 = new SimpleDateFormat("EEEEE, MMMMM dd, yyyy", Locale.ENGLISH);

    public static SimpleDateFormat dateSqlIn = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");

    public static SimpleDateFormat dateGPXIn = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss'Z'");
    ...
    public cgBase(cgeoapplication appIn, cgSettings settingsIn, SharedPreferences prefsIn) {
        ...
        cacheIDs.put("all", "9a79e6ce-3344-409c-bbe9-496530baf758");
        ...
        try {
            ...
            while (matcherLatLon.find()) {
                if (matcherLatLon.groupCount() > 0) {
                    cache.latlon = matcherLatLon.group(2);
    ...
    public Long searchByViewport(HashMap<String, String> parameters, int reason) {
        ...
        if (caches != null) {
            ...
            if (caches.cacheList != null && caches.cacheList.size() > 0) {
                for (cgCache cache : caches.cacheList) {
                    if ((settings.excludeDisabled == 0 || (settings.excludeDisabled == 1 && cache.disabled == false)) && (settings.excludeMine == 0 || (settings.excludeMine == 1 && cache.own == false)) && (settings.excludeMine == 0 || (settings.excludeMine == 1 && cache.found == false)) && (settings.cacheType == null || (settings.cacheType.equals(cache.type) == true))) {
                        search.addGeocode(cache.geocode);
                        cacheList.add(cache);
                    }
                }
    ...
    public int readWatchlistState(cgCache cache) {
        String page = requestLogged(false, "www.geocaching.com", "/my/watchlist.aspx", "POST", null, false, false, false);
        if (page == null || page.length() == 0) {
    ...
    public int addToWatchlist(cgCache cache) {
        String page = requestLogged(false, "www.geocaching.com", "/my/watchlist.aspx?w=" + cache.cacheid, "POST", null, false, false, false);
        if (page == null || page.length() == 0) {
