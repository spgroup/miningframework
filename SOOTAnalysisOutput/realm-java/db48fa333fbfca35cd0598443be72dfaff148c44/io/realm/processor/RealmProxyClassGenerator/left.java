        ...
        for (VariableElement field : metadata.getFields()) {
            ...
            if (typeUtils.isAssignable(field.asType(), realmObject)) {
                ...
                if (NULLABLE_JAVA_TYPES.containsKey(fieldType)) {
                    writer.emitStatement("realmObject.%s(newObject.%s() != null ? newObject.%s() : %s)", metadata.getSetter(fieldName), metadata.getGetter(fieldName), metadata.getGetter(fieldName), NULLABLE_JAVA_TYPES.get(fieldType));
                } else {
