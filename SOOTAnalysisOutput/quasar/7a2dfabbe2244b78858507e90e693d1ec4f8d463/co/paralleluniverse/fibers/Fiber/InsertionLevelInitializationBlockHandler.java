...
public class Fiber<V> extends Strand implements Joinable<V>, Serializable, Future<V> {
    ...
    private static String callSitesString(Instrumented i) {
        if (i == null)
            return "N/A";
        return "lines " + Arrays.toString(i.suspendableCallSites()) + ", " + " calls " + Arrays.toString(getReadableCallsites(i.suspendableCallSiteNames()));
    }

    private static String[] getReadableCallsites(String[] callsites) {
        String[] readable = new String[callsites.length];
        for (int i = 0; i < callsites.length; i++) readable[i] = SuspendableHelper.getCallsiteOwner(callsites[i]) + "." + SuspendableHelper.getCallsiteName(callsites[i]) + ASMUtil.getReadableDescriptor(SuspendableHelper.getCallsiteDesc(callsites[i]));
        return readable;
    }
