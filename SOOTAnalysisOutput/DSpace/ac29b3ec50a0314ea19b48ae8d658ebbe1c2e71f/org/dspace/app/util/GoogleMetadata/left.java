...
public class GoogleMetadata {
    ...
    private void loadGoogleScholarSettings() throws MalformedURLException, IOException {
        String googleConfigFile = configurationService.getProperty("google-metadata.config");
        ...
        try {
            ...
            log.error("Can't find Google Metadata configuration file: {}", googleConfigFile, mux);
            throw mux;
        }
    ...
    protected ArrayList<MetadataValue> resolveMetadata(String configFilter, int returnType) {
        ...
        for (ArrayList<String> optionFields : parsedOptions) {
            ...
            for (String field : optionFields) {
                ...
                values = itemService.getMetadata(item, components[0], components[1], components[2], Item.ANY);
                if (values.size() > 0) {
