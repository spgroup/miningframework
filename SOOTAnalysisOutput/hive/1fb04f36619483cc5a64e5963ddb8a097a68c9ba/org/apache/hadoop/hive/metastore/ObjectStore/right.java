...
public class ObjectStore implements RawStore, Configurable {
    ...
    private static final Set<Class<? extends Throwable>> retriableExceptionClasses = new HashSet<Class<? extends Throwable>>(Arrays.asList(JDOCanRetryException.class));
    ...
    private void initializeHelper(Properties dsProps) {
        ...
        try {
            ...
            sqlGenerator = new SQLGenerator(DatabaseProduct.determineDatabaseProduct(productName), new HiveConf(hiveConf, ObjectStore.class));
        ...
        LOG.debug("RawStore: " + this + ", with PersistenceManager: " + pm + " created in the thread with id: " + Thread.currentThread().getId());
    }
    ...
    private static PartitionExpressionProxy createExpressionProxy(Configuration conf) {
        ...
        try {
            ...
            Class<? extends PartitionExpressionProxy> clazz = (Class<? extends PartitionExpressionProxy>) MetaStoreUtils.getClass(className);
    ...
    public void shutdown() {
        if (pm != null) {
            LOG.debug("RawStore: " + this + ", with PersistenceManager: " + pm + " will be shutdown");
            pm.close();
            pm = null;
        }
    }
    ...
    @Override
    public List<String> createTableWithConstraints(Table tbl, List<SQLPrimaryKey> primaryKeys, List<SQLForeignKey> foreignKeys, List<SQLUniqueConstraint> uniqueConstraints, List<SQLNotNullConstraint> notNullConstraints) throws InvalidObjectException, MetaException {
        boolean success = false;
        try {
            openTransaction();
            createTable(tbl);
    ...
    private Map<String, String> convertMap(Map<String, String> dnMap) {
        return MetaStoreUtils.trimMapNulls(dnMap, HiveConf.getBoolVar(getConf(), ConfVars.METASTORE_ORM_RETRIEVE_MAPNULLS_AS_EMPTY_STRINGS));
    }
    ...
    private boolean isValidPartition(Partition part, boolean ifNotExists) throws MetaException {
        ...
        boolean doesExist = doesPartitionExist(part.getDbName(), part.getTableName(), part.getValues());
    ...
    private Partition convertToPart(String dbName, String tblName, MPartition mpart) throws MetaException {
        ...
        return new Partition(convertList(mpart.getValues()), dbName, tblName, mpart.getCreateTime(), mpart.getLastAccessTime(), convertToStorageDescriptor(mpart.getSd(), false), convertMap(mpart.getParameters()));
    }
    ...
    private List<String> getPartitionNamesByFilter(String dbName, String tableName, String filter, boolean ascending, long maxParts) throws MetaException {
        boolean success = false;
    ...
    public boolean getPartitionsByExpr(String dbName, String tblName, byte[] expr, String defaultPartitionName, short maxParts, List<Partition> result) throws TException {
        return getPartitionsByExprInternal(dbName, tblName, expr, defaultPartitionName, maxParts, result, true, true);
    }
    ...
    public abstract class GetHelper<T> {
        ...
        public GetHelper(String dbName, String tblName, boolean allowSql, boolean allowJdo) throws MetaException {
            ...
            boolean isConfigEnabled = HiveConf.getBoolVar(getConf(), ConfVars.METASTORE_TRY_DIRECT_SQL) && (HiveConf.getBoolVar(getConf(), ConfVars.METASTORE_TRY_DIRECT_SQL_DDL) || !isInTxn);
    ...
    public void alterPartition(String dbname, String name, List<String> part_vals, Partition newPart) throws InvalidObjectException, MetaException {
        ...
        try {
            ...
            if (!success) {
                rollbackTransaction();
                throw MetaStoreUtils.newMetaException("The transaction for alter partition did not commit successfully.", e);
            }
        }
    }
    ...
    private List<String> addForeignKeys(List<SQLForeignKey> fks, boolean retrieveCD) throws InvalidObjectException, MetaException {
    ...
    public List<MDBPrivilege> listPrincipalMDBGrants(String principalName, PrincipalType principalType, String dbName) {
        ...
        try {
            ...
            openTransaction();
            query = pm.newQuery(MDBPrivilege.class, "principalName == t1 && principalType == t2 && database.name == t3");
            query.declareParameters("java.lang.String t1, java.lang.String t2, java.lang.String t3");
            List<MDBPrivilege> mPrivs = (List<MDBPrivilege>) query.executeWithArray(principalName, principalType.toString(), dbName);
            pm.retrieveAll(mPrivs);
            success = commitTransaction();
    ...
    public List<MTableColumnPrivilege> listTableAllColumnGrants(String dbName, String tableName) {
        ...
        try {
            ...
            query.declareParameters("java.lang.String t1, java.lang.String t2");
            List<MTableColumnPrivilege> mPrivs = (List<MTableColumnPrivilege>) query.executeWithArray(tableName, dbName);
            pm.retrieveAll(mPrivs);
            success = commitTransaction();
    ...
    public List<MPartitionColumnPrivilege> listTableAllPartitionColumnGrants(String dbName, String tableName) {
        ...
        try {
            ...
            query.declareParameters("java.lang.String t1, java.lang.String t2");
            List<MPartitionColumnPrivilege> mPrivs = (List<MPartitionColumnPrivilege>) query.executeWithArray(tableName, dbName);
            pm.retrieveAll(mPrivs);
            success = commitTransaction();
    ...
    public void dropPartitionAllColumnGrantsNoTxn(String dbName, String tableName, List<String> partNames) {
        ObjectPair<Query, Object[]> queryWithParams = makeQueryByPartitionNames(dbName, tableName, partNames, MPartitionColumnPrivilege.class, "partition.table.tableName", "partition.table.database.name", "partition.partitionName");
    ...
    private void dropPartitionGrantsNoTxn(String dbName, String tableName, List<String> partNames) {
        ObjectPair<Query, Object[]> queryWithParams = makeQueryByPartitionNames(dbName, tableName, partNames, MPartitionPrivilege.class, "partition.table.tableName", "partition.table.database.name", "partition.partitionName");
        queryWithParams.getFirst().deletePersistentAll(queryWithParams.getSecond());
    }
    ...
    private <T> List<T> queryByPartitionNames(String dbName, String tableName, List<String> partNames, Class<T> clazz, String tbCol, String dbCol, String partCol) {
        ObjectPair<Query, Object[]> queryAndParams = makeQueryByPartitionNames(dbName, tableName, partNames, clazz, tbCol, dbCol, partCol);
    ...
    public List<MTableColumnPrivilege> listPrincipalMTableColumnGrants(String principalName, PrincipalType principalType, String dbName, String tableName, String columnName) {
        ...
        try {
            ...
            query = pm.newQuery(MTableColumnPrivilege.class, queryStr);
            query.declareParameters("java.lang.String t1, java.lang.String t2, java.lang.String t3, " + "java.lang.String t4, java.lang.String t5");
            List<MTableColumnPrivilege> mPrivs = (List<MTableColumnPrivilege>) query.executeWithArray(principalName, principalType.toString(), tableName, dbName, columnName);
            pm.retrieveAll(mPrivs);
            success = commitTransaction();
            mSecurityColList.addAll(mPrivs);
    ...
    public List<MPartitionColumnPrivilege> listPrincipalMPartitionColumnGrants(String principalName, PrincipalType principalType, String dbName, String tableName, String partitionName, String columnName) {
        ...
        try {
            ...
            openTransaction();
            query = pm.newQuery(MPartitionColumnPrivilege.class, "principalName == t1 && principalType == t2 && partition.table.tableName == t3 " + "&& partition.table.database.name == t4 && partition.partitionName == t5 && columnName == t6");
            ...
            List<MPartitionColumnPrivilege> mPrivs = (List<MPartitionColumnPrivilege>) query.executeWithArray(principalName, principalType.toString(), tableName, dbName, partitionName, columnName);
            pm.retrieveAll(mPrivs);
            success = commitTransaction();
            mSecurityColList.addAll(mPrivs);
    ...
    public List<HiveObjectPrivilege> listPartitionColumnGrantsAll(String dbName, String tableName, String partitionName, String columnName) {
        ...
        try {
            ...
            query = pm.newQuery(MPartitionColumnPrivilege.class, "partition.table.tableName == t3 && partition.table.database.name == t4 && " + "partition.partitionName == t5 && columnName == t6");
            query.declareParameters("java.lang.String t3, java.lang.String t4, java.lang.String t5, java.lang.String t6");
            List<MPartitionColumnPrivilege> mSecurityTabPartList = (List<MPartitionColumnPrivilege>) query.executeWithArray(tableName, dbName, partitionName, columnName);
    ...
    protected ColumnStatistics getTableColumnStatisticsInternal(String dbName, String tableName, final List<String> colNames, boolean allowSql, boolean allowJdo) throws MetaException, NoSuchObjectException {
        final boolean enableBitVector = HiveConf.getBoolVar(getConf(), HiveConf.ConfVars.HIVE_STATS_FETCH_BITVECTOR);
    ...
    public List<ColumnStatistics> getPartitionColumnStatistics(String dbName, String tableName, List<String> partNames, List<String> colNames) throws MetaException, NoSuchObjectException {
        return getPartitionColumnStatisticsInternal(dbName, tableName, partNames, colNames, true, true);
    }
    ...
    public AggrStats get_aggr_stats_for(String dbName, String tblName, final List<String> partNames, final List<String> colNames) throws MetaException, NoSuchObjectException {
        ...
        final double ndvTuner = HiveConf.getFloatVar(getConf(), HiveConf.ConfVars.HIVE_METASTORE_STATS_NDV_TUNER);
        final boolean enableBitVector = HiveConf.getBoolVar(getConf(), HiveConf.ConfVars.HIVE_STATS_FETCH_BITVECTOR);
        return new GetHelper<AggrStats>(dbName, tblName, true, false) {
            ...
            protected AggrStats getSqlResult(GetHelper<AggrStats> ctx) throws MetaException {
                return directSql.aggrColStatsForPartitions(dbName, tblName, partNames, colNames, useDensityFunctionForNDVEstimation, ndvTuner, enableBitVector);
            }
    ...
    @Override
    public Map<String, List<ColumnStatisticsObj>> getColStatsForTablePartitions(String dbName, String tableName) throws MetaException, NoSuchObjectException {
        final boolean enableBitVector = HiveConf.getBoolVar(getConf(), HiveConf.ConfVars.HIVE_STATS_FETCH_BITVECTOR);
        return new GetHelper<Map<String, List<ColumnStatisticsObj>>>(dbName, tableName, true, false) {

            @Override
            protected Map<String, List<ColumnStatisticsObj>> getSqlResult(GetHelper<Map<String, List<ColumnStatisticsObj>>> ctx) throws MetaException {
                return directSql.getColStatsForTablePartitions(dbName, tblName, enableBitVector);
            }

            @Override
            protected Map<String, List<ColumnStatisticsObj>> getJdoResult(GetHelper<Map<String, List<ColumnStatisticsObj>>> ctx) throws MetaException, NoSuchObjectException {
                throw new MetaException("Jdo path is not implemented for stats aggr.");
            }
    ...
    private void dropPartitionColumnStatisticsNoTxn(String dbName, String tableName, List<String> partNames) throws MetaException {
        ObjectPair<Query, Object[]> queryWithParams = makeQueryByPartitionNames(dbName, tableName, partNames, MPartitionColumnStatistics.class, "tableName", "dbName", "partition.partitionName");
        queryWithParams.getFirst().deletePersistentAll(queryWithParams.getSecond());
    }
    ...
    static class RetryingExecutor {
        ...
        RetryingExecutor(Configuration config, Command command) {
            this.maxRetries = config.getInt(ConfVars.NOTIFICATION_SEQUENCE_LOCK_MAX_RETRIES.name(), ConfVars.NOTIFICATION_SEQUENCE_LOCK_MAX_RETRIES.defaultIntVal);
            this.sleepInterval = config.getTimeDuration(ConfVars.NOTIFICATION_SEQUENCE_LOCK_RETRY_SLEEP_INTERVAL.name(), ConfVars.NOTIFICATION_SEQUENCE_LOCK_RETRY_SLEEP_INTERVAL.defaultLongVal, TimeUnit.MILLISECONDS);
        ...
        public void run() throws MetaException {
            while (true) {
                try {
                    ...
                    LOG.info("Attempting to acquire the DB log notification lock: " + currentRetries + " out of " + maxRetries + " retries", e);
                    if (currentRetries >= maxRetries) {
                        String message = "Couldn't acquire the DB log notification lock because we reached the maximum" + " # of retries: {} retries. If this happens too often, then is recommended to " + "increase the maximum number of retries on the" + " hive.notification.sequence.lock.max.retries configuration";
                        LOG.error(message, e);
    ...
    public NotificationEventsCountResponse getNotificationEventsCount(NotificationEventsCountRequest rqst) {
        ...
        try {
            ...
            String queryStr = "select count(eventId) from " + MNotificationLog.class.getName() + " where eventId > fromEventId && dbName == inputDbName";
            query = pm.newQuery(queryStr);
    ...
    protected List<SQLPrimaryKey> getPrimaryKeysInternal(final String db_name_input, final String tbl_name_input, boolean allowSql, boolean allowJdo) throws MetaException, NoSuchObjectException {
    ...
    public List<SQLForeignKey> getForeignKeys(String parent_db_name, String parent_tbl_name, String foreign_db_name, String foreign_tbl_name) throws MetaException {
        try {
            return getForeignKeysInternal(parent_db_name, parent_tbl_name, foreign_db_name, foreign_tbl_name, true, true);
        } catch (NoSuchObjectException e) {
            throw new MetaException(ExceptionUtils.getStackTrace(e));
        }
    }

    protected List<SQLForeignKey> getForeignKeysInternal(final String parent_db_name_input, final String parent_tbl_name_input, final String foreign_db_name_input, final String foreign_tbl_name_input, boolean allowSql, boolean allowJdo) throws MetaException, NoSuchObjectException {
    ...
    protected List<SQLUniqueConstraint> getUniqueConstraintsInternal(final String db_name_input, final String tbl_name_input, boolean allowSql, boolean allowJdo) throws MetaException, NoSuchObjectException {
        final String db_name = HiveStringUtils.normalizeIdentifier(db_name_input);
        final String tbl_name = HiveStringUtils.normalizeIdentifier(tbl_name_input);
    ...
    private List<SQLUniqueConstraint> getUniqueConstraintsViaJdo(String db_name, String tbl_name) throws MetaException {
        ...
        try {
            ...
            for (Iterator<?> i = constraints.iterator(); i.hasNext(); ) {
                ...
                boolean validate = (enableValidateRely & 2) != 0;
                boolean rely = (enableValidateRely & 1) != 0;
                uniqueConstraints.add(new SQLUniqueConstraint(db_name, tbl_name, currPK.getParentColumn().getCols().get(currPK.getParentIntegerIndex()).getName(), currPK.getPosition(), currPK.getConstraintName(), enable, validate, rely));
            }
            commited = commitTransaction();
        } finally {
            if (!commited) {
    ...
    @Override
    public List<SQLNotNullConstraint> getNotNullConstraints(String db_name, String tbl_name) throws MetaException {
        try {
    ...
    protected List<SQLNotNullConstraint> getNotNullConstraintsInternal(final String db_name_input, final String tbl_name_input, boolean allowSql, boolean allowJdo) throws MetaException, NoSuchObjectException {
        final String db_name = HiveStringUtils.normalizeIdentifier(db_name_input);
        final String tbl_name = HiveStringUtils.normalizeIdentifier(tbl_name_input);
    ...
    private List<SQLNotNullConstraint> getNotNullConstraintsViaJdo(String db_name, String tbl_name) throws MetaException {
        ...
        try {
            ...
            for (Iterator<?> i = constraints.iterator(); i.hasNext(); ) {
                ...
                boolean validate = (enableValidateRely & 2) != 0;
                boolean rely = (enableValidateRely & 1) != 0;
                notNullConstraints.add(new SQLNotNullConstraint(db_name, tbl_name, currPK.getParentColumn().getCols().get(currPK.getParentIntegerIndex()).getName(), currPK.getConstraintName(), enable, validate, rely));
            }
            commited = commitTransaction();
        } finally {
            if (!commited) {
