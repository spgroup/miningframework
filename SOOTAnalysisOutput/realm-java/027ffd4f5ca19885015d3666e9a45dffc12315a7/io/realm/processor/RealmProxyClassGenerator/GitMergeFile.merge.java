...
>>>>>>> YOURS
    ...
    private void emitCopyMethod(JavaWriter writer) throws IOException {
        ...
        for (VariableElement field : fields) {
            ...
            if (typeUtils.isAssignable(field.asType(), realmObject)) {
                writer.emitEmptyLine().emitStatement("%s %s = newObject.%s()", fieldType, fieldName, getters.get(fieldName)).beginControlFlow("if (%s != null)", fieldName).emitStatement("%s cache%s = (%s) cache.get(%s)", fieldType, fieldName, fieldType, fieldName).beginControlFlow("if (cache%s != null)", fieldName).emitStatement("realmObject.%s(cache%s)", setters.get(fieldName), fieldName).nextControlFlow("else").emitStatement("realmObject.%s(%s.copyOrUpdate(realm, %s, update, cache))", setters.get(fieldName), Utils.getProxyClassSimpleName(field), fieldName).endControlFlow().endControlFlow();
            } else if (typeUtils.isAssignable(field.asType(), realmList)) {
    ...
    private void emitUpdateMethod(JavaWriter writer) throws IOException {
        ...
        for (VariableElement field : fields) {
            ...
            if (typeUtils.isAssignable(field.asType(), realmObject)) {
<<<<<<< MINE
                writer.emitStatement("%s %s = newObject.%s()", Utils.getFieldTypeSimpleName(field), fieldName, getters.get(fieldName)).beginControlFlow("if (%s != null)", fieldName).emitStatement("%s cache%s = (%s) cache.get(%s)", Utils.getFieldTypeSimpleName(field), fieldName, Utils.getFieldTypeSimpleName(field), fieldName).beginControlFlow("if (cache%s != null)", fieldName).emitStatement("realmObject.%s(cache%s)", setters.get(fieldName), fieldName).nextControlFlow("else").emitStatement("realmObject.%s(%s.copyOrUpdate(realm, %s, realm.getTable(%s.class).hasPrimaryKey(), cache))", setters.get(fieldName), Utils.getProxyClassSimpleName(field), fieldName, Utils.getFieldTypeSimpleName(field)).endControlFlow().nextControlFlow("else").emitStatement("realmObject.%s(null)", setters.get(fieldName)).endControlFlow();
=======
                writer.emitStatement("%s %s = newObject.%s()", Utils.getFieldTypeSimpleName(field), fieldName, getters.get(fieldName)).beginControlFlow("if (%s != null)", fieldName).emitStatement("realmObject.%s(%s.copyOrUpdate(realm, %s, true))", setters.get(fieldName), Utils.getProxyClassSimpleName(field), fieldName, Utils.getFieldTypeSimpleName(field)).nextControlFlow("else").emitStatement("realmObject.%s(null)", setters.get(fieldName)).endControlFlow();
>>>>>>> YOURS
            } else if (typeUtils.isAssignable(field.asType(), realmList)) {
