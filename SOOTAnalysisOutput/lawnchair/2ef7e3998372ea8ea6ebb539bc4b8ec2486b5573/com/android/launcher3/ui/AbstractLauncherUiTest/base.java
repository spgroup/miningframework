...
public abstract class AbstractLauncherUiTest {
    ...
    public static final long DEFAULT_UI_TIMEOUT = 60000;
    ...
    @Rule
    public ShellCommandRule mDefaultLauncherRule = TestHelpers.isInLauncherProcess() ? ShellCommandRule.setDefaultLauncher() : null;
    ...
    protected TestRule getRulesInsideActivityMonitor() {
        return RuleChain.outerRule(new PortraitLandscapeRunner(this)).around(new FailureWatcher(mDevice));
    }

    @Rule
    public TestRule mOrderSensitiveRules = RuleChain.outerRule(new TestStabilityRule()).around(mActivityMonitor).around(getRulesInsideActivityMonitor());
    ...
    public void verifyLauncherState() {
        try {
            ...
            Log.e(TAG, "Couldn't deinit after a test, exiting tests, see logs for failures that " + "could have caused this", t);
            exit(1);
        }
    }
    ...
    protected void waitForState(String message, LauncherState state) {
        waitForLauncherCondition(message, launcher -> launcher.getStateManager().getCurrentStableState() == state);
    }
    ...
    protected LauncherActivityInfo getSettingsApp() {
        return LauncherAppsCompat.getInstance(mTargetContext).getActivityList("com.android.settings", Process.myUserHandle()).get(0);
    }
    ...
    private void startIntent(Intent intent, BySelector selector, boolean newTask) {
        ...
        assertTrue("App didn't start: " + selector, mDevice.wait(Until.hasObject(selector), DEFAULT_UI_TIMEOUT));
    }

    public static String resolveSystemApp(String category) {
        return getInstrumentation().getContext().getPackageManager().resolveActivity(new Intent(Intent.ACTION_MAIN).addCategory(category), PackageManager.MATCH_SYSTEM_ONLY).activityInfo.packageName;
    }
