...
public class HiveConf extends Configuration {
    ...
    public static enum ConfVars {
        ...
        METASTORE_AUTO_CREATE_SCHEMA("datanucleus.autoCreateSchema", false, "creates necessary schema on a startup if one doesn't exist. set this to false, after creating it once"),
        ...
        HIVEMAPREDMODE("hive.mapred.mode", "strict", "The mode in which the Hive operations are being performed. \n" + "In strict mode, some risky queries are not allowed to run. They include:\n" + "  Cartesian Product.\n" + "  No partition being picked up for a query.\n" + "  Comparing bigints and strings.\n" + "  Comparing bigints and doubles.\n" + "  Orderby without limit."),
        ...
        HIVEQUERYRESULTFILEFORMAT("hive.query.result.fileformat", "TextFile", new StringSet("TextFile", "SequenceFile", "RCfile"), "Default file format for storing result of the query."),
        ...
        HIVE_OPTIMIZE_LIMIT_JOIN_TRANSPOSE("hive.optimize.limitjointranspose", false, "Whether to push a limit through left/right outer join. If the value is true and the size of the outer\n" + "input is reduced enough (as specified in hive.optimize.limitjointranspose.reduction), the limit is pushed\n" + "to the outer input; to remain semantically correct, the limit is kept on top of the join too."),
        HIVE_OPTIMIZE_LIMIT_JOIN_TRANSPOSE_REDUCTION_PERCENTAGE("hive.optimize.limitjointranspose.reductionpercentage", 1.0f, "When hive.optimize.limitjointranspose is true, this variable specifies the minimal reduction of the\n" + "size of the outer input of the join that we should get in order to apply the rule."),
        HIVE_OPTIMIZE_LIMIT_JOIN_TRANSPOSE_REDUCTION_TUPLES("hive.optimize.limitjointranspose.reductiontuples", (long) 0, "When hive.optimize.limitjointranspose is true, this variable specifies the minimal reduction in the\n" + "number of tuples of the outer input of the join that you should get in order to apply the rule."),
        ...
        LLAP_ORC_CACHE_MIN_ALLOC("hive.llap.io.cache.orc.alloc.min", 128 * 1024, "Minimum allocation possible from LLAP low-level cache for ORC. Allocations below that\n" + "will be padded to minimum allocation. Should generally be the same as expected ORC\n" + "compression buffer size, or next lowest power of 2. Must be power of 2."),
        LLAP_ORC_CACHE_MAX_ALLOC("hive.llap.io.cache.orc.alloc.max", 16 * 1024 * 1024, "Maximum allocation possible from LLAP low-level cache for ORC. Should be as large as\n" + "the largest expected ORC compression buffer size. Must be power of 2."),
        LLAP_ORC_CACHE_ARENA_COUNT("hive.llap.io.cache.orc.arena.count", 8, "Arena count for LLAP low-level cache; cache will be allocated in the steps of\n" + "(size/arena_count) bytes. This size must be <= 1Gb and >= max allocation; if it is\n" + "not the case, an adjusted size will be used. Using powers of 2 is recommended."),
        LLAP_ORC_CACHE_MAX_SIZE("hive.llap.io.cache.orc.size", 1024L * 1024 * 1024, "Maximum size for ORC low-level cache; must be a multiple of arena size."),
        LLAP_ORC_CACHE_ALLOCATE_DIRECT("hive.llap.io.cache.direct", true, "Whether ORC low-level cache should use direct allocation."),
        ...
        LLAP_MANAGEMENT_ACL("hive.llap.management.service.acl", "*", "The ACL for LLAP daemon management."),
        ...
        LLAP_DAEMON_YARN_CONTAINER_MB("hive.llap.daemon.yarn.container.mb", -1, "TODO doc. Unused?", "llap.daemon.yarn.container.mb"),
        ...
        LLAP_DAEMON_SERVICE_HOSTS("hive.llap.daemon.service.hosts", "", "Explicitly specified hosts to use for LLAP scheduling. Useful for testing. By default,\n" + "YARN registry is used.", "llap.daemon.service.hosts"),
