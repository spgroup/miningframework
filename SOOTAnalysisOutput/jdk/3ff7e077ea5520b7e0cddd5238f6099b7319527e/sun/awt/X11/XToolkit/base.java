...
public final class XToolkit extends UNIXToolkit implements Runnable {
    ...
    void init() {
        ...
        PrivilegedAction<Void> a = () -> {
            ...
            Thread shutdownThread = new Thread(ThreadGroupUtils.getRootThreadGroup(), r, name, 0, false);
    ...
    public Insets getScreenInsets(GraphicsConfiguration gc) {
        ...
        try {
            ...
            Insets insets = getScreenInsetsManually(root, rootBounds, gc.getBounds(), scale);
            if ((insets.left | insets.top | insets.bottom | insets.right) == 0 && rootBounds != null) {
                root = XlibWrapper.RootWindow(XToolkit.getDisplay(), x11gd.getScreen());
                Insets screenInsets = getInsets(root, rootBounds, scale);
                if (screenInsets != null)
                    return screenInsets;
            }
    ...
    public FileDialogPeer createFileDialog(FileDialog target) {
        ...
        if (!getSunAwtDisableGtkFileDialogs() && (checkGtkVersion(2, 4, 0) || checkGtkVersion(3, 0, 0))) {
    ...
    protected void initializeDesktopProperties() {
        ...
        if (!GraphicsEnvironment.isHeadless()) {
            ...
            if (SunGraphicsEnvironment.isUIScaleEnabled()) {
                addPropertyChangeListener("gnome.Xft/DPI", evt -> localEnv.displayChanged());
            }
        }
    }
