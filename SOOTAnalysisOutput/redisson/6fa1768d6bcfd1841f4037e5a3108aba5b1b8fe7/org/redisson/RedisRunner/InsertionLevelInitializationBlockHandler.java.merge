...
public class RedisRunner {
    ...
    private static RedisProcess runWithOptions(RedisRunner runner, String... options) throws IOException, InterruptedException {
        ...
        ProcessBuilder master = new ProcessBuilder(launchOptions).redirectErrorStream(true).directory(new File(RedissonRuntimeEnvironment.tempDir));
    ...
    public static final class RedisProcess {
        ...
        public int stop() throws InterruptedException {
            if (runner.isNosave() && !runner.isRandomDir()) {
                ...
                connection.async(new RedisStrictCommand<Void>("SHUTDOWN", "NOSAVE", new VoidReplayConvertor())).await(3, TimeUnit.SECONDS);
                c.shutdown();
