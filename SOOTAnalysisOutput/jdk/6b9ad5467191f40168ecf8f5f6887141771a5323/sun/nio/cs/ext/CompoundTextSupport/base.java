...
final class CompoundTextSupport {
    ...
    static boolean getHighBit(byte[] escSequence) {
        Boolean bool = (Boolean) highBitsMap.get(new ControlSequence(escSequence));
        return (bool == Boolean.TRUE);
    }

    static CharsetDecoder getNonStandardDecoder(byte[] escSequence, byte[] encoding) {
        return getDecoder((String) sequenceToEncodingMap.get(new ControlSequence(escSequence, encoding)));
    }
    ...
    static byte[] getEscapeSequence(String encoding) {
        ControlSequence seq = (ControlSequence) encodingToSequenceMap.get(encoding);
        if (seq != null) {
