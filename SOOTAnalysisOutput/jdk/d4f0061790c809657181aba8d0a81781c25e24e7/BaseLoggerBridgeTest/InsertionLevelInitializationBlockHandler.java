...
public class BaseLoggerBridgeTest {
    ...
    static final Level[] mappedLevels = { Level.ALL, Level.TRACE, Level.TRACE, Level.DEBUG, Level.DEBUG, Level.INFO, Level.WARNING, Level.ERROR, Level.OFF };
    ...
    public static interface TestLoggerFinder {
        ...
        public static final class LogEvent implements Cloneable {
            ...
            Object[] toArray(boolean callSupplier) {
                return new Object[] { sequenceNumber, isLoggable, loggerName, level, bundle, thrown, args, callSupplier && supplier != null ? supplier.get() : supplier, msg };
            }
            ...
            public boolean equals(Object obj) {
                return obj instanceof LogEvent && Objects.deepEquals(toArray(callSupplier(obj)), ((LogEvent) obj).toArray(callSupplier(obj)));
            }
            ...
            public static LogEvent of(boolean isLoggable, String name, Level level, ResourceBundle bundle, String key, Throwable thrown) {
            ...
            public static LogEvent of(boolean isLoggable, String name, Level level, Throwable thrown, Supplier<String> supplier) {
                LogEvent evt = new LogEvent();
                evt.isLoggable = isLoggable;
                evt.loggerName = name;
                evt.level = level;
            ...
            public static LogEvent of(boolean isLoggable, String name, Level level, ResourceBundle bundle, String key, Object... params) {
                LogEvent evt = new LogEvent();
                evt.isLoggable = isLoggable;
                evt.loggerName = name;
                evt.level = level;
            ...
            public static LogEvent of(long sequenceNumber, boolean isLoggable, String name, Level level, ResourceBundle bundle, String key, Supplier<String> supplier, Throwable thrown, Object... params) {
    ...
    public static void test(TestLoggerFinder provider, boolean hasRequiredPermissions) {
        ...
        try {
            ...
            loggerDescMap.put(sysLogger1, "jdk.internal.logger.LazyLoggers.getLogger(\"foo\", Thread.class.getModule())");
            if (!hasRequiredPermissions) {
        ...
        if (hasRequiredPermissions) {
            testLogger(provider, loggerDescMap, "foo", null, PlatformLogger.Bridge.convert(sysLogger1), sysSink);
        }
        ...
        testLogger(provider, loggerDescMap, "foo", loggerBundle, PlatformLogger.Bridge.convert(appLogger2), appSink);
        ...
        if (hasRequiredPermissions) {
            testLogger(provider, loggerDescMap, "foo", loggerBundle, PlatformLogger.Bridge.convert(sysLogger2), sysSink);
        }
    }
    ...
    static void checkLogEvent(TestLoggerFinder provider, String desc, TestLoggerFinder.LogEvent expected) {
    ...
    static void checkLogEvent(TestLoggerFinder provider, String desc, TestLoggerFinder.LogEvent expected, boolean expectNotNull) {
        TestLoggerFinder.LogEvent actual = provider.eventQueue.poll();
    ...
    static String logpMessage(ResourceBundle bundle, String className, String methodName, String msg) {
        if (BEST_EFFORT_FOR_LOGP && bundle == null && (className != null || methodName != null)) {
            final String cName = className == null ? "" : className;
            final String mName = methodName == null ? "" : methodName;
    ...
    private static void testLogger(TestLoggerFinder provider, Map<Object, String> loggerDescMap, String name, ResourceBundle loggerBundle, PlatformLogger.Bridge logger, TestLoggerFinder.LoggerImpl sink) {
        ...
        for (Level loggerLevel : Level.values()) {
            ...
            for (sun.util.logging.PlatformLogger.Level messageLevel : julLevels) {
                ...
                TestLoggerFinder.LogEvent expected = TestLoggerFinder.LogEvent.of(sequencer.get(), loggerLevel != Level.OFF && expectedMessageLevel.compareTo(loggerLevel) >= 0, name, expectedMessageLevel, loggerBundle, fooMsg, null, (Throwable) null, (Object[]) null);
        ...
        for (Level loggerLevel : Level.values()) {
            ...
            for (sun.util.logging.PlatformLogger.Level messageLevel : julLevels) {
                ...
                Level expectedMessageLevel = julToSpiMap.get(messageLevel);
                TestLoggerFinder.LogEvent expected = TestLoggerFinder.LogEvent.of(sequencer.get(), loggerLevel != Level.OFF && expectedMessageLevel.compareTo(loggerLevel) >= 0, name, expectedMessageLevel, (ResourceBundle) null, null, supplier, (Throwable) null, (Object[]) null);
        ...
        for (Level loggerLevel : Level.values()) {
            ...
            for (sun.util.logging.PlatformLogger.Level messageLevel : julLevels) {
                ...
                Level expectedMessageLevel = julToSpiMap.get(messageLevel);
                TestLoggerFinder.LogEvent expected = TestLoggerFinder.LogEvent.of(sequencer.get(), loggerLevel != Level.OFF && expectedMessageLevel.compareTo(loggerLevel) >= 0, name, expectedMessageLevel, loggerBundle, format, null, (Throwable) null, arg1, arg2);
        ...
        for (Level loggerLevel : Level.values()) {
            ...
            for (sun.util.logging.PlatformLogger.Level messageLevel : julLevels) {
                ...
                Level expectedMessageLevel = julToSpiMap.get(messageLevel);
                TestLoggerFinder.LogEvent expected = TestLoggerFinder.LogEvent.of(sequencer.get(), loggerLevel != Level.OFF && expectedMessageLevel.compareTo(loggerLevel) >= 0, name, expectedMessageLevel, loggerBundle, fooMsg, null, thrown, (Object[]) null);
        ...
        for (Level loggerLevel : Level.values()) {
            ...
            for (sun.util.logging.PlatformLogger.Level messageLevel : julLevels) {
                ...
                Level expectedMessageLevel = julToSpiMap.get(messageLevel);
                TestLoggerFinder.LogEvent expected = TestLoggerFinder.LogEvent.of(sequencer.get(), loggerLevel != Level.OFF && expectedMessageLevel.compareTo(loggerLevel) >= 0, name, expectedMessageLevel, (ResourceBundle) null, null, supplier, thrown, (Object[]) null);
        ...
        for (Level loggerLevel : Level.values()) {
            ...
            for (sun.util.logging.PlatformLogger.Level messageLevel : julLevels) {
                ...
                TestLoggerFinder.LogEvent expected = isLoggable || loggerBundle != null && BEST_EFFORT_FOR_LOGP ? TestLoggerFinder.LogEvent.of(sequencer.get(), isLoggable, name, expectedMessageLevel, loggerBundle, logpMessage(loggerBundle, sourceClass, sourceMethod, fooMsg), null, (Throwable) null, (Object[]) null) : null;
        ...
        for (Level loggerLevel : Level.values()) {
            ...
            for (sun.util.logging.PlatformLogger.Level messageLevel : julLevels) {
                ...
                Level expectedMessageLevel = julToSpiMap.get(messageLevel);
                boolean isLoggable = loggerLevel != Level.OFF && expectedMessageLevel.compareTo(loggerLevel) >= 0;
                TestLoggerFinder.LogEvent expected = isLoggable ? TestLoggerFinder.LogEvent.ofp(BEST_EFFORT_FOR_LOGP, TestLoggerFinder.LogEvent.of(sequencer.get(), isLoggable, name, expectedMessageLevel, null, null, logpMessage(null, sourceClass, sourceMethod, supplier), (Throwable) null, (Object[]) null)) : null;
        ...
        for (Level loggerLevel : Level.values()) {
            ...
            for (sun.util.logging.PlatformLogger.Level messageLevel : julLevels) {
                ...
                Level expectedMessageLevel = julToSpiMap.get(messageLevel);
                boolean isLoggable = loggerLevel != Level.OFF && expectedMessageLevel.compareTo(loggerLevel) >= 0;
                TestLoggerFinder.LogEvent expected = isLoggable || loggerBundle != null && BEST_EFFORT_FOR_LOGP ? TestLoggerFinder.LogEvent.of(sequencer.get(), loggerLevel != Level.OFF && expectedMessageLevel.compareTo(loggerLevel) >= 0, name, expectedMessageLevel, loggerBundle, logpMessage(loggerBundle, sourceClass, sourceMethod, fooMsg), null, thrown, (Object[]) null) : null;
        ...
        for (Level loggerLevel : Level.values()) {
            ...
            for (sun.util.logging.PlatformLogger.Level messageLevel : julLevels) {
                ...
                Level expectedMessageLevel = julToSpiMap.get(messageLevel);
                boolean isLoggable = loggerLevel != Level.OFF && expectedMessageLevel.compareTo(loggerLevel) >= 0;
                TestLoggerFinder.LogEvent expected = isLoggable ? TestLoggerFinder.LogEvent.ofp(BEST_EFFORT_FOR_LOGP, TestLoggerFinder.LogEvent.of(sequencer.get(), loggerLevel != Level.OFF && expectedMessageLevel.compareTo(loggerLevel) >= 0, name, expectedMessageLevel, null, null, logpMessage(null, sourceClass, sourceMethod, supplier), thrown, (Object[]) null)) : null;
        ...
        for (Level loggerLevel : Level.values()) {
            ...
            for (sun.util.logging.PlatformLogger.Level messageLevel : julLevels) {
                ...
                Level expectedMessageLevel = julToSpiMap.get(messageLevel);
                TestLoggerFinder.LogEvent expected = TestLoggerFinder.LogEvent.of(sequencer.get(), loggerLevel != Level.OFF && expectedMessageLevel.compareTo(loggerLevel) >= 0, name, expectedMessageLevel, bundle, format, null, (Throwable) null, arg1, arg2);
        ...
        for (Level loggerLevel : Level.values()) {
            ...
            for (sun.util.logging.PlatformLogger.Level messageLevel : julLevels) {
                ...
                Level expectedMessageLevel = julToSpiMap.get(messageLevel);
                TestLoggerFinder.LogEvent expected = TestLoggerFinder.LogEvent.of(sequencer.get(), loggerLevel != Level.OFF && expectedMessageLevel.compareTo(loggerLevel) >= 0, name, expectedMessageLevel, bundle, fooMsg, null, thrown, (Object[]) null);
