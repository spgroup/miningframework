...
public class KDC {
    ...
    protected byte[] processTgsReq(byte[] in) throws Exception {
        ...
        try {
            ...
            if (options.containsKey(Option.CHECK_ADDRESSES) && body.kdcOptions.get(KDCOptions.FORWARDED) && body.addresses != null) {
            ...
            EncTicketPart enc = new EncTicketPart(tFlags, key, cname, new TransitedEncoding(1, new byte[0]), new KerberosTime(new Date()), body.from, till, etp.renewTill, body.addresses != null ? body.addresses : etp.caddr, null);
            EncryptionKey skey = keyForUser(service, e3, true);
            if (skey == null) {
                throw new KrbException(Krb5.KDC_ERR_SUMTYPE_NOSUPP);
            }
            Ticket t = new Ticket(System.getProperty("test.kdc.diff.sname") != null ? new PrincipalName("xx" + service.toString()) : service, new EncryptedData(skey, enc.asn1Encode(), KeyUsage.KU_TICKET));
            EncTGSRepPart enc_part = new EncTGSRepPart(key, new LastReq(new LastReqEntry[] { new LastReqEntry(0, new KerberosTime(new Date().getTime() - 10000)) }), body.getNonce(), new KerberosTime(new Date().getTime() + 1000 * 3600 * 24), tFlags, new KerberosTime(new Date()), body.from, till, etp.renewTill, service, body.addresses);
    ...
    protected byte[] processAsReq(byte[] in) throws Exception {
        ...
        try {
            ...
            } else if (till.greaterThan(new KerberosTime(Instant.now().plus(1, ChronoUnit.DAYS)))) {
                till = new KerberosTime(new Date().getTime() + 1000 * DEFAULT_LIFETIME);
            ...
            if (rtime == null && body.kdcOptions.get(KDCOptions.RENEWABLE)) {
                rtime = new KerberosTime(new Date().getTime() + 1000 * DEFAULT_RENEWTIME);
            }
            boolean[] bFlags = new boolean[Krb5.TKT_OPTS_MAX + 1];
            ...
            TicketFlags tFlags = new TicketFlags(bFlags);
            EncTicketPart enc = new EncTicketPart(tFlags, key, body.cname, new TransitedEncoding(1, new byte[0]), new KerberosTime(new Date()), body.from, till, rtime, body.addresses, null);
            Ticket t = new Ticket(service, new EncryptedData(skey, enc.asn1Encode(), KeyUsage.KU_TICKET));
            EncASRepPart enc_part = new EncASRepPart(key, new LastReq(new LastReqEntry[] { new LastReqEntry(0, new KerberosTime(new Date().getTime() - 10000)) }), body.getNonce(), new KerberosTime(new Date().getTime() + 1000 * 3600 * 24), tFlags, new KerberosTime(new Date()), body.from, till, rtime, service, body.addresses);
    ...
    public static KDC startKDC(final String host, final String krbConfFileName, final String realm, final Map<String, String> principals, final String ktab, final KtabMode mode) {
        KDC kdc;
        try {
            ...
            if (principals != null) {
                principals.forEach((name, password) -> {
                    if (password == null || password.isEmpty()) {
                        ...
                        System.out.println(String.format("KDC:add a principal '%s' with '%s' password", name, password));
            ...
            if (ktab != null) {
                ...
                switch(mode) {
                    case APPEND:
                        if (ktabFile.exists()) {
                            System.out.println(String.format("KDC:append keys to an exising keytab " + "file %s", ktab));
                            kdc.appendKtab(ktab);
                        } else {
                        ...
                        break;
                    case EXISTING:
                        System.out.println(String.format("KDC:use an existing keytab file %s", ktab));
                        break;
                    default:
                        throw new RuntimeException(String.format("KDC:unsupported keytab mode: %s", mode));
                }
            }
            System.out.println(String.format("KDC: started on %s:%s with '%s' realm", host, kdc.getPort(), realm));
        } catch (Exception e) {
