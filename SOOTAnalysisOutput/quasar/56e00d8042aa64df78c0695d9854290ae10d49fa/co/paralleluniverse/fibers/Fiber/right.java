...
public class Fiber<V> extends Strand implements Joinable<V>, Serializable, Future<V> {
    ...
    public static boolean checkInstrumentation() {
        ...
        for (int i = 0, k = 0; i < stes.length; i++, k++) {
            ...
            if (skipSTE(ste)) {
                ...
                i--;
            } else if (!ste.getClassName().equals(Fiber.class.getName()) && !ste.getClassName().startsWith(Fiber.class.getName() + '$') && !ste.getClassName().equals(Stack.class.getName())) {
                if (!SuspendableHelper.isWaiver(ste.getClassName(), ste.getMethodName()) && (!SuspendableHelper.isInstrumented(context[k]) || isNonSuspendable(context[k], ste.getMethodName()))) {
                    if (!notInstrumented) {
    ...
    private static boolean skipSTE(StackTraceElement ste) {
        return (ste.getClassName().startsWith("sun.reflect") || ste.getClassName().equals("java.lang.reflect.Method") || ste.getClassName().startsWith("java.lang.invoke."));
    }
