...
public class HiveConf extends Configuration {
    ...
    public static enum ConfVars {
        ...
        SCRATCHDIR("hive.exec.scratchdir", "/tmp/hive", "HDFS root scratch dir for Hive jobs which gets created with 777 permission. " + "For each connecting user, an HDFS scratch dir: ${hive.exec.scratchdir}/<username> is created, " + "with ${hive.scratch.dir.permission}."),
        ...
        HIVE_ZOOKEEPER_QUORUM("hive.zookeeper.quorum", "", "List of ZooKeeper servers to talk to. This is needed for: " + "1. Read/write locks - when hive.lock.manager is set to " + "org.apache.hadoop.hive.ql.lockmgr.zookeeper.ZooKeeperHiveLockManager, " + "2. When HiveServer2 supports service discovery via Zookeeper."),
        HIVE_ZOOKEEPER_CLIENT_PORT("hive.zookeeper.client.port", "2181", "The port of ZooKeeper servers to talk to. " + "If the list of Zookeeper servers specified in hive.zookeeper.quorum," + "does not contain port numbers, this value is used."),
        ...
        HIVE_SERVER2_GLOBAL_INIT_FILE_LOCATION("hive.server2.global.init.file.location", "${env:HIVE_CONF_DIR}", "The location of HS2 global init file (.hiverc).\n" + "If the property is reset, the value must be a valid path where the init file is located."),
        ...
        HIVE_SERVER2_ASYNC_EXEC_SHUTDOWN_TIMEOUT("hive.server2.async.exec.shutdown.timeout", "10s", new TimeValidator(TimeUnit.SECONDS), "Maximum time for which HiveServer2 shutdown will wait for async"),
        ...
        TEZ_DYNAMIC_PARTITION_PRUNING("hive.tez.dynamic.partition.pruning", true, "When dynamic pruning is enabled, joins on partition keys will be processed by sending events from the processing " + "vertices to the tez application master. These events will be used to prune unnecessary partitions."),
