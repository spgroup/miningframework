...
public class Launcher extends Activity implements View.OnClickListener, OnLongClickListener, LauncherModel.Callbacks, View.OnTouchListener, PageSwitchListener, LauncherProviderChangeListener {
    ...
    @Thunk
    final ItemInfo mPendingAddInfo = new ItemInfo();
    ...
    private static final DateFormat sDateFormat = DateFormat.getDateTimeInstance(DateFormat.SHORT, DateFormat.SHORT);
    ...
    private void setupViews() {
        ...
        mAppInfoDropTargetBar = (AppInfoDropTargetBar) mDragLayer.findViewById(R.id.app_info_drop_target_bar);
...
>>>>>>> YOURS
        ...
        if (autoAdvanceRunning != mAutoAdvanceRunning) {
            ...
            if (autoAdvanceRunning) {
                ...
                if (!mWidgetsToAdvance.isEmpty()) {
                    mAutoAdvanceTimeLeft = Math.max(0, ADVANCE_INTERVAL - (System.currentTimeMillis() - mAutoAdvanceSentTime));
                }
                mHandler.removeMessages(ADVANCE_MSG);
    ...
    private void unbindAppWidget(final LauncherAppWidgetInfo widgetInfo, boolean deleteAppWidgetId) {
        ...
        if (deleteAppWidgetId && appWidgetHost != null && !widgetInfo.isCustomWidget() && widgetInfo.isWidgetIdValid()) {
    ...
    protected void onClickWallpaperPicker(View v) {
        ...
        startActivityForResult(new Intent(Intent.ACTION_SET_WALLPAPER).setPackage(getPackageName()).putExtra(WallpaperPickerActivity.EXTRA_WALLPAPER_OFFSET, offset), REQUEST_PICK_WALLPAPER);
        if (mLauncherCallbacks != null) {
    ...
    private void growAndFadeOutFolderIcon(FolderIcon fi) {
        ...
        ObjectAnimator oa = LauncherAnimUtils.ofViewAlphaAndScale(mFolderIconImageView, 0, 1.5f, 1.5f);
        if (Utilities.ATLEAST_LOLLIPOP) {
    ...
    void showOverviewMode(boolean animated) {
        mWorkspace.setVisibility(View.VISIBLE);
        mStateTransitionAnimation.startAnimationToWorkspace(mState, mWorkspace.getState(), Workspace.State.OVERVIEW, animated, null);
        mState = State.WORKSPACE;
    }
    ...
    public Animator startWorkspaceStateChangeAnimation(Workspace.State toState, boolean animated, HashMap<View, Integer> layerViews) {
    ...
    public void enterSpringLoadedDragMode() {
        ...
        mStateTransitionAnimation.startAnimationToWorkspace(mState, mWorkspace.getState(), Workspace.State.SPRING_LOADED, true, null);
        if (isAppsViewVisible()) {
    ...
    void showWorkspaceSearchAndHotseat() {
        ...
        if (mSearchDropTargetBar != null)
            mSearchDropTargetBar.animateToState(SearchDropTargetBar.State.SEARCH_BAR, 0);
    }
    ...
    void hideWorkspaceSearchAndHotseat() {
        ...
        if (mSearchDropTargetBar != null)
            mSearchDropTargetBar.animateToState(SearchDropTargetBar.State.INVISIBLE, 0);
    }
