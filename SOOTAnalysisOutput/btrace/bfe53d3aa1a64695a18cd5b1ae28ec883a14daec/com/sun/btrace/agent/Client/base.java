...
abstract class Client implements CommandListener {
        ...
        if (out == null) {
            if (outputFile.equals("::stdout")) {
                ...
                if (settings.getFileRollMilliseconds() > 0) {
                    out = new PrintWriter(new BufferedWriter(TraceOutputWriter.rollingFileWriter(new File(outputFile), settings)));
                } else {
    ...
    private String templateOutputFileName(String fName) {
        if (fName != null) {
            ...
            fName = fName.replace("${client}", clientName != null ? clientName : "").replace("${ts}", String.valueOf(System.currentTimeMillis())).replace("${agent}", agentName != null ? "." + agentName : "").replace("[default]", "");
