...
>>>>>>> YOURS
        ...
        THREAD_DUMP_ON_FAILURE = threadDumpOnFailure != null ? Boolean.parseBoolean(threadDumpOnFailure) : JenkinsDetector.isOnJenkins();
    ...
    private static void attachFinalRemovalAgent() {
        ...
        new AgentBuilder.Default().disableClassFormatChanges().type(new ElementMatcher<TypeDescription>() {
    ...
    private class TestRepeater extends Statement {
        ...
        public void evaluate() throws Throwable {
            for (int i = 0; i < repeat; i++) {
                if (repeat > 1) {
                    System.out.println(String.format("---> Repeating test [%s:%s], run count [%d]", testMethod.getDeclaringClass().getCanonicalName(), testMethod.getName(), i + 1));
                }
                statement.evaluate();
            }
        }
