...
public final class RegexParser {
    ...
    private Group parse(boolean rootCapture) throws RegexSyntaxException {
        ...
        while (lexer.hasNext()) {
            ...
            token = lexer.next();
            if (token.kind != Token.Kind.quantifier && curTerm != null && curTerm.isBackReference() && curTerm.asBackReference().isNestedOrForwardReference() && !isNestedInLookBehindAssertion(curTerm)) {
                removeCurTerm();
            }
            switch(token.kind) {
                case caret:
